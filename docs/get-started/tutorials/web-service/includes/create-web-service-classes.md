...
1. **솔루션 탐색기**의 **WebServicesTutorial** 프로젝트에서 `WeatherData`라는 새 클래스를 프로젝트에 추가합니다. 그런 다음, **WeatherData.cs**에서 템플릿 코드를 모두 제거하고 다음 코드로 바꿉니다.
    ...
    > [!NOTE]
    > 위의 클래스 정의는 단순화되고, 웹 서비스에서 검색되는 JSON 데이터를 완벽하게 모델링하지 않습니다. 전체 데이터 모델 예제는 [날씨 앱](/samples/xamarin/xamarin-forms-samples/weather/) 샘플을 참조하세요.
