...
또는 `Device.RuntimePlatform` 이 코드에서 설명한 대로 속성을 사용 하 여 각 플랫폼에서 다른 글꼴 이름을 설정할 수 있습니다.
...
`FontSize`모든 글꼴 설정이 XAML에서 문자열 값으로 표현 될 수 있도록 하는 속성에 대 한 기본 제공 변환기가 있습니다. 또한 `FontAttributes` 속성을 사용 하 여 글꼴 특성을 지정할 수 있습니다.
...
[`Device.RuntimePlatform`](~/xamarin-forms/platform/device.md#provide-platform-specific-values)또한 속성은 각 플랫폼에서 다른 글꼴을 렌더링 하기 위해 XAML에서 사용할 수 있습니다. 아래 예제에서는 각 플랫폼에서 다른 글꼴을 사용 합니다.
...
명명 된 글꼴 크기는 XAML과 코드를 모두 통해 설정할 수 있습니다. 또한 `Device.GetNamedSize` 메서드를 호출 하 여 명명 된 글꼴 크기를 나타내는를 반환할 수 있습니다 `double` .
...
> [!IMPORTANT]
> 포함 된 글꼴을 사용 하려면 Xamarin.Forms 4.5.0.530 이상을 사용 해야 합니다.
