...
- **범주** &ndash; 장치가 *방해* 금지 모드에 있을 때와 같이 다양 한 상황에서 알림을 처리 하는 방법을 시스템에 알립니다.
...
이 문서의 뒷부분에 나오는 [기본 알림 외](#beyond-the-basic-notification) 에도 *빅 텍스트*, *수신함*및 *이미지* 알림을 만드는 방법을 설명 합니다.
...
Android Oreo와 함께 설치 되는 **YouTube** 앱은 **다운로드 알림과** **일반 알림**이라는 두 가지 알림 범주를 나열 합니다.
...
알림 채널이 사용자와 상호 작용 하는 방식을 절대적으로 제어 하지는 않습니다 &ndash; . 사용자는 위의 스크린샷에 표시 된 대로 장치에서 알림 채널에 대 한 설정을 수정할 수 있습니다. 그러나 아래에 설명 된 대로 기본값을 구성할 수 있습니다. 이 예제에서 설명 하는 것 처럼 새 알림 채널 기능을 사용 하면 다양 한 종류의 알림을 사용자에 게 세분화 하 여 제어할 수 있습니다.

## <a name="notification-creation"></a>알림 만들기

Android에서 알림을 만들려면 Nuget.exe NuGet 패키지에서 [Notificationcompat. Builder](https://developer.android.com/reference/android/support/v4/app/NotificationCompat.Builder) 클래스를 사용 합니다. [Xamarin.Android.Support.v4](https://www.nuget.org/packages/Xamarin.Android.Support.v4/) 이 클래스를 사용 하면 이전 버전의 Android에서 알림을 만들고 게시할 수 있습니다.
...
활동을 만들 때마다 알림 채널을 만들어야 합니다. 메서드의 경우 `CreateNotificationChannel` `OnCreate` 활동의 메서드에서 호출 해야 합니다.

### <a name="creating-and-publishing-a-notification"></a>알림 만들기 및 게시

Android에서 알림을 생성 하려면 다음 단계를 수행 합니다.

1. `NotificationCompat.Builder` 개체를 인스턴스화합니다.

2. 개체에 대 한 다양 한 메서드 `NotificationCompat.Builder` 를 호출 하 여 알림 옵션을 설정 합니다.

3. 개체의 [빌드](xref:Android.App.Notification.Builder.Build) 메서드를 호출 `NotificationCompat.Builder` 하 여 알림 개체를 인스턴스화합니다.
...
다음 코드 예제에서는를 사용 하 여 기본 알림을 생성 하는 방법을 보여 줍니다 `NotificationCompat.Builder` . 메서드는 `NotificationCompat.Builder` [메서드 체인](https://en.wikipedia.org/wiki/Method_chaining)을 지원 합니다. 즉, 각 메서드는 빌더 개체를 반환 하므로 마지막 메서드 호출의 결과를 사용 하 여 다음 메서드 호출을 호출할 수 있습니다.
...
이 예제에서는 `NotificationCompat.Builder` `builder` 사용할 알림 채널의 ID와 함께 라는 새 개체가 인스턴스화됩니다. 알림의 제목과 텍스트는 설정 되며, 알림 아이콘은 **리소스/그릴 수 있는/ic_notification.png**에서 로드 됩니다. 알림 작성기의 메서드에 대 한 호출은 `Build` 이러한 설정을 사용 하 여 알림 개체를 만듭니다. 다음 단계는 `Notify` 알림 관리자의 메서드를 호출 하는 것입니다. 알림 관리자를 찾으려면 위와 같이를 호출 `GetSystemService` 합니다.

`Notify`메서드는 알림 식별자와 알림 개체의 두 가지 매개 변수를 허용 합니다. 알림 id는 응용 프로그램에 대 한 알림을 식별 하는 고유한 정수입니다. 이 예제에서 알림 식별자는 0으로 설정 됩니다. 그러나 프로덕션 응용 프로그램에서는 각 알림에 고유한 식별자를 지정 하는 것이 좋습니다. 호출에서 이전 식별자 값을 다시 사용 `Notify` 하면 마지막 알림이 덮어쓰여집니다.

이 코드는 Android 5.0 장치에서 실행 되는 경우 다음 예제와 같은 알림을 생성 합니다.

![샘플 코드에 대 한 알림 결과](local-notifications-images/09-hello-world.png)

알림 아이콘은 알림의 왼쪽에 표시 됩니다 &ndash; .이 이미지는 원으로 표시 된 &ldquo; &rdquo; 알파 채널이 있으므로 Android에서 회색 원형 배경을 그릴 수 있습니다. 알파 채널 없이 아이콘을 제공할 수도 있습니다. 사진 이미지를 아이콘으로 표시 하려면이 항목의 뒷부분에 나오는 [크게 아이콘 형식](#large-icon-format) 을 참조 하세요.
...
알림이 소리를 재생 하도록 하려면 알림 작성기의 [Setdefaults](xref:Android.App.Notification.Builder.SetDefaults*) 메서드를 호출 하 고 플래그를 전달 하면 됩니다 `NotificationDefaults.Sound` .
...
알림 개체를 만든 후에는 메서드를 통해 미리 구성 하는 대신 알림 개체에 알림 속성을 설정할 수 있습니다 `NotificationCompat.Builder` . 예를 들어 `SetDefaults` 알림에 대해 진동을 사용 하도록 메서드를 호출 하는 대신 알림의 [기본값](xref:Android.App.Notification.Defaults) 속성의 비트 플래그를 직접 수정할 수 있습니다.
...
게시 된 알림의 콘텐츠를 업데이트 하려는 경우 기존 개체를 다시 사용 `NotificationCompat.Builder` 하 여 새 알림 개체를 만들고이 알림을 마지막 알림의 식별자로 게시할 수 있습니다. 다음은 그 예입니다.
...
이 예제에서 기존 개체는 `NotificationCompat.Builder` 다른 제목 및 메시지를 사용 하 여 새 알림 개체를 만드는 데 사용 됩니다.
...
알림이 &ndash; 알림 서랍에 표시 되는 동안 이전 알림의 본문은 제목만 다시 사용 하 고 알림의 텍스트를 변경 합니다. 제목 텍스트가 "Sample Notification"에서 "업데이트 된 알림"으로 바뀌고 메시지 텍스트는 "Hello World! 이것은 첫 번째 알림입니다. " "이 메시지를 변경 했습니다."

다음 세 가지 중 하나가 발생할 때까지 알림이 계속 표시 됩니다.

- 사용자가 알림을 해제 하거나 [ *모두 지우기*]를 탭 합니다.

- 응용 프로그램은를 호출 하 여 `NotificationManager.Cancel` 알림이 게시 될 때 할당 된 고유한 알림 ID를 전달 합니다.

- 응용 프로그램에서를 호출 `NotificationManager.CancelAll` 합니다.
...
Android에서 *action* &ndash; 사용자가 알림을 누를 때 실행 되는 작업에 대 한 알림이 일반적으로 발생 합니다. 이 활동은 다른 응용 프로그램에 있거나 다른 작업 에서도 존재할 수 있습니다. 알림에 작업을 추가 하려면 [Pendingintent](xref:Android.App.PendingIntent) 개체를 만들고이 알림과 연결 합니다 `PendingIntent` . 는 `PendingIntent` 받는 사람 응용 프로그램에서 보내는 응용 프로그램의 사용 권한으로 미리 정의 된 코드 조각을 실행할 수 있도록 하는 특별 한 유형의 의도입니다. 사용자가 알림을 탭 하면 Android가에 지정 된 작업을 시작 합니다 `PendingIntent` .

다음 코드 조각에서는 `PendingIntent` 원래 앱의 작업을 시작 하는를 사용 하 여 알림을 만드는 방법을 보여 줍니다 `MainActivity` .
...
이 코드는가 `PendingIntent` 알림 개체에 추가 된다는 점을 제외 하 고 이전 섹션의 알림 코드와 매우 비슷합니다. 이 예제에서는 `PendingIntent` 알림 작성기의 [Setcontentintent](xref:Android.App.Notification.Builder.SetContentIntent*) 메서드에 전달 되기 전에 원래 앱의 활동과 연결 됩니다. `PendingIntentFlags.OneShot`플래그는 `PendingIntent.GetActivity` `PendingIntent` 가 한 번만 사용 되도록 메서드에 전달 됩니다. 이 코드가 실행 되 면 다음과 같은 알림이 표시 됩니다.
...
대부분의 경우 알림 작업에서 뒤로 이동 하면 앱에서 사용자를 반환 하 고 홈 화면으로 다시 이동 해야 합니다. 백 스택을 관리 하기 위해 앱은 [Taskstackbuilder](xref:Android.App.TaskStackBuilder) 클래스를 사용 하 여 `PendingIntent` 백 스택으로을 만듭니다.

또 다른 실제적인 고려 사항은 원래 활동에서 알림 활동에 데이터를 보내야 할 수도 있다는 것입니다. 예를 들어 알림에는 문자 메시지가 도착 했음을 나타내고 알림 작업 (메시지 보기 화면)에는 메시지 ID를 사용 하 여 사용자에 게 메시지를 표시할 수 있습니다. 을 만드는 활동은 `PendingIntent` 이 데이터가 알림 활동에 전달 되도록 [의도. putextra](xref:Android.Content.Intent.PutExtra*) 메서드를 사용 하 여 데이터 (예: 문자열)를 의도에 추가할 수 있습니다.

다음 코드 샘플에서는를 사용 하 여 백 스택을 관리 하는 방법을 보여 줍니다 `TaskStackBuilder` . 여기에는 라는 알림 활동에 단일 메시지 문자열을 보내는 방법에 대 한 예제가 포함 되어 있습니다 `SecondActivity` .
...
이 코드 예제에서 앱은 `MainActivity` (위의 알림 코드를 포함 하는) 두 가지 작업으로 구성 되며 `SecondActivity` , 알림을 탭핑 한 후 사용자에 게 표시 되는 화면입니다. 이 코드가 실행 되 면 간단한 알림 (이전 예제와 유사)이 표시 됩니다. 알림을 누르면 사용자가 화면으로 이동 합니다 `SecondActivity` .

![두 번째 활동 스크린샷](local-notifications-images/11-second-activity.png)

문자열 메시지 (의도의 `PutExtra` 메서드로 전달 됨)는 `SecondActivity` 다음 코드 줄을 통해에서 검색 됩니다.
...
위의 스크린샷에 표시 된 것 처럼이 검색 된 메시지 "MainActivity!"의 인사말과 화면에 표시 됩니다 `SecondActivity` . 사용자가 로그인 하는 동안 **뒤로** 단추를 누르면 `SecondActivity` 앱이 시작 되 고 앱이 시작 된 후에 다시 화면으로 이동 합니다.
...
알림은 Android에서 간단한 기본 레이아웃 형식으로 기본 설정 되지만 추가 메서드 호출을 통해이 기본 형식을 향상 시킬 수 있습니다 `NotificationCompat.Builder` . 이 섹션에서는 알림에 긴 사진 아이콘을 추가 하는 방법에 대해 알아보고 확장 된 레이아웃 알림을 만드는 방법에 대 한 예제를 살펴봅니다.
...
다음은 &ndash; 작은 앱 아이콘만 표시 하는 기본 Android 5.0 알림 예제입니다.

![예제 일반 알림](local-notifications-images/13-sample-notification.png)

여기에는 커다란 아이콘이 표시 되도록 수정한 후에는 &ndash; Xamarin 코드 원숭이 이미지에서 만들어진 아이콘이 사용 됩니다.
...
알림에서 이미지를 큼 아이콘으로 사용 하려면 알림 작성기의 [SetLargeIcon](xref:Android.App.Notification.Builder.SetLargeIcon*) 메서드를 호출 하 고 이미지의 비트맵을 전달 합니다. 와 달리는 `SetSmallIcon` `SetLargeIcon` 비트맵만 허용 합니다. 비트맵으로 이미지 파일을 변환 하려면 [BitmapFactory](xref:Android.Graphics.BitmapFactory) 클래스를 사용 합니다. 다음은 그 예입니다.

```csharp
builder.SetLargeIcon (BitmapFactory.DecodeResource (Resources, Resource.Drawable.monkey_icon));
```

이 예제 코드는 **리소스/그릴 수 있는/monkey_icon.png**에서 이미지 파일을 열고, 비트맵으로 변환 하 고, 결과 비트맵을에 전달 `NotificationCompat.Builder` 합니다. 일반적으로 원본 이미지 해상도는 작은 아이콘 보다 크지만 크게 &ndash; 크지 않습니다. 이미지가 너무 크면 알림 게시를 지연 시킬 수 있는 불필요 한 크기 조정 작업이 발생할 수 있습니다.
...
*큰 텍스트* 알림을 만들려면 `NotificationCompat.Builder` 이전과 같은 개체를 인스턴스화한 다음, 개체에 대 한 추가 [textstyle](xref:Android.App.Notification.BigTextStyle) 개체를 인스턴스화하고 추가 `NotificationCompat.Builder` 합니다. 다음은 예제입니다.
...
이 예제에서 메시지 텍스트와 요약 텍스트는 `BigTextStyle` `textStyle` 에 전달 되기 전에 개체 ()에 저장 됩니다. `NotificationCompat.Builder.`
...
알림이 압축 형식으로 표시 되 면 알림 텍스트 (이전에 표시 된 것 처럼 알림 작성기의 메서드에 전달 되는 텍스트)가 표시 됩니다 `SetContentText` . 그러나 이미지를 표시 하도록 알림을 확장 하면 이미지 위에 요약 텍스트가 표시 됩니다.

*이미지* 알림을 만들려면 이전 처럼 개체를 인스턴스화한 다음 개체에 개체에 대 한 개체를 `NotificationCompat.Builder` 만들어 삽입 [BigPictureStyle](xref:Android.App.Notification.BigPictureStyle) `NotificationCompat.Builder` 합니다. 다음은 그 예입니다.
...
`SetLargeIcon`의 메서드와 마찬가지로 `NotificationCompat.Builder` 의 이상 [그림](xref:Android.App.Notification.BigPictureStyle.BigPicture*) 메서드는 `BigPictureStyle` 알림의 본문에 표시 하려는 이미지의 비트맵이 필요 합니다. 이 예제에서 [DecodeResource](xref:Android.Graphics.BitmapFactory.DecodeResource*) 메서드는 `BitmapFactory` **리소스/그릴 수/x_bldg.png** 에 있는 이미지 파일을 읽고 비트맵으로 변환 합니다.
...
미리 이미지 파일의 크기를 모르는 경우에는 예외 처리기에서 [BitmapFactory](xref:Android.Graphics.BitmapFactory.DecodeFile*) 에 대 한 호출을 래핑하는 것이 좋습니다. &ndash; `OutOfMemoryError` 이미지가 너무 커서 크기를 조정할 수 없는 경우 예외가 throw 될 수 있습니다.
...
*받은 편지함* 알림을 만들려면 `NotificationCompat.Builder` 이전과 같은 개체를 인스턴스화하고에 [InboxStyle](xref:Android.App.Notification.InboxStyle) 개체를 추가 `NotificationCompat.Builder` 합니다. 다음은 예제입니다.
...
알림 본문에 새 텍스트 줄을 추가 하려면 개체의 [Addline](xref:Android.App.Notification.InboxStyle.AddLine*) 메서드를 호출 합니다 `InboxStyle` ( *수신함* 알림의 최대 높이는 256 dp). *큰 텍스트* 스타일과 달리 *수신함* 스타일은 알림 본문에서 개별 텍스트 줄을 지원 합니다.

또한 개별 텍스트 줄을 확장 된 형식으로 표시 해야 하는 모든 알림에 대해 *수신함* 스타일을 사용할 수 있습니다. 예를 들어, *받은 편지함* 알림 스타일을 사용 하 여 여러 보류 중인 알림을 요약 알림으로 결합할 수 있습니다 &ndash; . 새 알림 *Inbox* 내용의 줄 바꿈 (위에서 [알림 업데이트](#updating-a-notification) 참조)은 일반적으로 유사한 새로운 알림의 연속 스트림을 생성 하는 대신 업데이트 됩니다.

## <a name="configuring-metadata"></a>메타 데이터 구성

`NotificationCompat.Builder` 에는 우선 순위, 표시 유형, 범주 등 알림에 대 한 메타 데이터를 설정 하기 위해 호출할 수 있는 메서드가 포함 되어 있습니다. Android에서는 사용자 기본 설정과 함께이 정보를 사용 하 여 &mdash; &mdash; 알림을 표시 하는 방법과 시기를 결정 합니다.
...
- `NotificationPriority.High`중요 한 &ndash; 이벤트 (예: 중요 한 전자 메일 또는 실시간 채팅 메시지 도착)를 사용자에 게 알립니다. Android 5.0 이상 장치에서는 높은 우선 순위 알림이 준비 형식으로 표시 됩니다.
...
알림의 우선 순위를 설정 하려면 개체의 [Setpriority](xref:Android.App.Notification.Builder.SetPriority*) 메서드를 호출 하 여 `NotificationCompat.Builder` 우선 순위 수준으로 전달 합니다. 다음은 그 예입니다.
...
알림 표시 여부를 설정 하기 위해 앱 `SetVisibility` 은 개체의 메서드를 호출 하 여 `NotificationCompat.Builder` 표시 유형 설정을 전달 합니다. 예를 들어를 호출 하면 `SetVisibility` 알림이 수행 됩니다 `Private` .
...
이 예제에서 **Notificationslab 래 브** 는 원래 앱의 이름입니다. 이 교정 된 버전의 알림은 잠금 화면이 안전 하 게 보호 되는 경우 (즉, PIN, 패턴 또는 암호를 통해 보안 됨) 잠금 화면 &ndash; 에 안전 하지 않은 경우 잠금 화면에서 전체 알림 콘텐츠를 사용할 수 있는 경우에만 표시 됩니다.
...
알림이 정렬 되 면 알림 우선 순위가 해당 범주 설정 보다 우선 적용 됩니다. 예를 들어 우선 순위가 높은 알림은 범주에 속한 경우에도 준비로 표시 됩니다 `Promo` . 알림의 범주를 설정 하려면 개체의 메서드를 호출 하 여 `SetCategory` `NotificationCompat.Builder` 범주 설정을 전달 합니다. 다음은 그 예입니다.
...
위의 스크린샷에 나와 있는 것 처럼 사용자가 전화 통화를 제외한 모든 인터럽트를 차단 하도록 *방해* 금지를 구성 하는 경우, Android는 `Notification.CategoryCall` 장치가 *방해 금지* 모드로 설정 된 상태에서 범주가 설정 된 알림을 제공할 수 있도록 허용 합니다. `Notification.CategoryAlarm`알림은 *방해* 금지 모드에서 차단 되지 않습니다.

[Localnotifications](/samples/xamarin/monodroid-samples/localnotifications) 샘플에서는를 사용 하 여 `NotificationCompat.Builder` 알림에서 두 번째 활동을 시작 하는 방법을 보여 줍니다. 이 샘플 코드는 [xamarin.ios 연습의 로컬 알림 사용](~/android/app-fundamentals/notifications/local-notifications-walkthrough.md) 에서 설명 합니다.

### <a name="notification-styles"></a>알림 스타일

를 사용 하 여 *빅 텍스트*, *이미지*또는 *수신함* 스타일 알림을 만들려면 `NotificationCompat.Builder` 앱에서 이러한 스타일의 호환성 버전을 사용 해야 합니다. 예를 들어, *큰 텍스트* 스타일을 사용 하려면 다음을 인스턴스화합니다 `NotificationCompat.BigTextstyle` .
...
마찬가지로 앱은 `NotificationCompat.InboxStyle` `NotificationCompat.BigPictureStyle` 각각 및에 *수신함* 및 *이미지* 스타일을 사용할 수 있습니다.

### <a name="notification-priority-and-category"></a>알림 우선 순위 및 범주

`NotificationCompat.Builder``SetPriority`메서드 (Android 4.1부터 사용 가능)를 지원 합니다. 그러나 `SetCategory` *not* `NotificationCompat.Builder` 범주는 Android 5.0에 도입 된 새 알림 메타 데이터 시스템에 포함 되기 때문에에서는 메서드가 지원 되지 않습니다.

을 사용할 수 없는 이전 버전의 Android를 지원 하려면 `SetCategory` `SetCategory` Api 수준이 Android 5.0 (api 레벨 21) 보다 크거나 같은 경우 코드에서 런타임에 api 수준을 검사 하 여 조건적으로 호출할 수 있습니다.
...
이 예제에서는 앱의 **대상 프레임 워크가** android 5.0로 설정 되 고 **최소 android 버전이** **android 4.1 (API 수준 16)** 로 설정 됩니다. `SetCategory`는 api 수준 21 이상에서 사용할 수 있기 때문에이 예제 코드는 `SetCategory` 사용 가능한 경우에만를 호출 하 고 &ndash; `SetCategory` api 수준이 21 보다 작은 경우에는를 호출 하지 않습니다.

### <a name="lock-screen-visibility"></a>잠금 화면 표시 유형

Android 5.0 (API 레벨 21) 이전에는 Android에서 화면 잠금 알림을 지원 하지 않으므로에서 `NotificationCompat.Builder` 이 메서드를 지원 하지 않습니다 `SetVisibility` . 에 대해 위에서 설명한 것 처럼 `SetCategory` 코드는 런타임에 API 수준을 확인 하 고 `SetVisiblity` 사용 가능한 경우에만 호출할 수 있습니다.
...
이 문서에서는 Android에서 로컬 알림을 만드는 방법을 설명 했습니다. 알림 분석에 대해 설명 하 고,을 사용 하 여 `NotificationCompat.Builder` 알림을 만드는 방법, 큰 아이콘의 알림 스타일 지정 방법, *빅 텍스트*, *이미지* 및 *수신함* 형식, 우선 순위, 표시 유형 및 범주와 같은 알림 메타 데이터 설정을 설정 하는 방법, 알림에서 작업을 시작 하는 방법을 설명 했습니다. 또한이 문서에서는 Android 5.0에 도입 된 새로운 준비, 잠금 화면 및 *방해 금지* 기능에서 이러한 알림 설정이 작동 하는 방식을 설명 했습니다. 마지막으로를 사용 하 여 `NotificationCompat.Builder` 이전 버전의 Android와의 호환성을 유지 하는 방법을 배웠습니다.
...
- [NotificationsLab 래 브 (샘플)](/samples/xamarin/monodroid-samples/android50-notificationslab)
- [LocalNotifications (샘플)](/samples/xamarin/monodroid-samples/localnotifications)
