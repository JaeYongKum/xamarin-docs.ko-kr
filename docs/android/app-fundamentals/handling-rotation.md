...
장치를 가로 방향으로 회전 하는 경우 `OnCreate` 아래 스크린샷에 표시 된 것 처럼 활동의 메서드가 다시 호출 되 고 동일한 **Main. axml** 파일이 팽창 됩니다.
...
레이아웃 폴더 외에도 기본적으로 세로를 사용 하 고 이름이 인 폴더를 포함 하 여 *레이아웃-포트* 를 명시적으로 지정할 수 있습니다 `layout-land` . 응용 프로그램은 코드를 변경 하지 않고도 가로에서 필요한 뷰를 정의할 수 있습니다.
...
추가 **Main. axml** 파일이 포함 된 레이아웃 육지 라는 폴더가 프로젝트에 추가 되 면, 않아서는 레이아웃을 가로 방향으로 지정 하면 이제 Android에서 새로 추가 된 **기본. axml** 을 로드 합니다. 다음 코드가 포함 된 **주. axml** 파일의 가로 버전을 고려 합니다. 간단 하 게 하기 위해이 XML은 코드의 기본 세로 버전과 유사 하지만에서 다른 문자열을 사용 합니다 `TextView` .
...
예를 들어, 프로젝트에는 **Resources/그릴** 수 있는 폴더에 Monkey.png 라는 이미지가 포함 되어 있다고 가정 합니다. 여기에서 그릴 수 있는는 `ImageView` 다음과 같이 XML의에서 참조 됩니다.
...
다른 버전의 **Monkey.png** **리소스/그릴**수 있는 위치에 포함 되어 있다고 가정 하겠습니다. 레이아웃 파일을 사용 하는 것과 마찬가지로 장치가 회전 될 때 아래와 같이 그릴 때 지정 된 방향으로 변경 됩니다.

[![세로 및 가로 모드에 표시 되는 Monkey.png의 다양 한 버전](handling-rotation-images/03.png)](handling-rotation-images/03.png#lightbox)
...
예를 들어, `TextView` 다음 코드와 같이에 추가 된 단일 컨트롤로 구성 된 사용자 인터페이스를 살펴보겠습니다 `RelativeLayout` .
...
이 코드는 클래스의 인스턴스를 만들고 `RelativeLayout` 해당 속성을 설정 `LayoutParameters` 합니다. `LayoutParams`클래스는 다시 사용 가능한 방식으로 컨트롤의 위치를 지정 하는 방법을 캡슐화 하는 Android의 방법입니다. 레이아웃의 인스턴스를 만든 후에는 컨트롤을 만들어 추가할 수 있습니다. 컨트롤 `LayoutParameters` 에는이 예제의와 같은도 있습니다 `TextView` . `TextView`가 만들어지면를에 추가 하 `RelativeLayout` 고를 콘텐츠 뷰로 설정 하면 `RelativeLayout` 다음과 같이 응용 프로그램이 표시 됩니다 `TextView` .
...
가 호출 될 때마다 응용 프로그램이 각 방향에 대해 다른 사용자 인터페이스를 로드 하려고 시도 하는 경우 `OnCreate` (장치가 회전 될 때마다 발생 함) 방향을 검색 한 다음 원하는 사용자 인터페이스 코드를 로드 해야 합니다. Android에는 `WindowManager` `WindowManager.DefaultDisplay.Rotation` 아래와 같이 속성을 통해 현재 장치 회전을 결정 하는 데 사용할 수 있는 라는 클래스가 있습니다.
...
이 코드는 다음과 같이를 `TextView` 화면 왼쪽 위에서 100 픽셀 배치 하도록 설정 하 고, 가로로 회전할 때 새 레이아웃에 자동으로 애니메이션 효과를 줍니다.
...
응용 프로그램은에서 모든 항목을 처리 하는 것 외에 `OnCreate` 도 `ConfigurationChanges` 다음과 같이를 설정 하 여 방향이 변경 될 때 활동이 다시 시작 되지 않도록 할 수 있습니다 `ActivityAttribute` .

```csharp
[Activity (Label = "CodeLayoutActivity", ConfigurationChanges=Android.Content.PM.ConfigChanges.Orientation | Android.Content.PM.ConfigChanges.ScreenSize)]
```

이제 장치가 회전 될 때 활동이 다시 시작 되지 않습니다. 이 경우 방향 변경을 수동으로 처리 하기 위해 활동은 메서드를 재정의 `OnConfigurationChanged` 하 고 `Configuration` 아래 활동의 새 구현에서와 같이 전달 되는 개체에서 방향을 결정할 수 있습니다.
...
여기서 `TextView's` 레이아웃 매개 변수는 가로 및 세로 모두에 대해 초기화 됩니다. `TextView`방향이 변경 될 때 활동이 다시 생성 되지 않으므로 클래스 변수에는 자체와 함께 매개 변수가 포함 됩니다. 이 코드에서는 여전히의를 사용 하 여의 `surfaceOrientartion` `OnCreate` 초기 레이아웃을 설정 합니다 `TextView` . 이후에는 `OnConfigurationChanged` 모든 이후 레이아웃 변경 내용을 처리 합니다.
...
이렇게 하려면 프로그래밍 방식 레이아웃에 사용 하는 것과 동일한 절차를 따릅니다. 이전에서 `ConfigurationChanges` `ActivityAttribute` 했던 것 처럼에서를 설정 하면 됩니다 `CodeLayoutActivity` . 방향 변경을 위해 실행 해야 하는 모든 코드는 메서드에서 다시 구현할 수 있습니다 `OnConfigurationChanged` .
...
- [회전 데모 (샘플)](/samples/xamarin/monodroid-samples/applicationfundamentals-rotationdemo)
...
- [빠른 화면 방향 변경](https://android-developers.blogspot.com/2009/02/faster-screen-orientation-change.html)