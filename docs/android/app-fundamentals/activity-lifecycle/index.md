...
1. *중지 됨/Backgrounded* &ndash; 다른 활동에 의해 완전히 가려진 활동은 중지 된 것으로 간주 되거나 백그라운드에서 진행 되는 것으로 간주 됩니다.
    중지 된 활동은 가능한 한 오랫동안 상태 및 멤버 정보를 유지 하려고 시도 하지만 중지 된 활동은 세 가지 상태의 가장 낮은 우선 순위로 간주 됩니다. 따라서 OS는 우선 순위가 높은 활동의 리소스 요구 사항을 충족 하기 위해 먼저이 상태의 활동을 중지 합니다.
...
`OnCreate`이 완료 되 면 Android가를 호출 `OnStart` 합니다.

#### <a name="onstart"></a>OnStart

가 완료 된 후에는 항상 [OnStart](xref:Android.App.Activity.OnStart) 가 시스템에 의해 호출 됩니다 `OnCreate` . 활동 내에서 뷰의 현재 값을 새로 고치는 것과 같이 활동은 활동을 표시 하기 바로 전에 특정 작업을 수행 해야 하는 경우이 메서드를 재정의할 수 있습니다. Android는 `OnResume` 이 메서드 직후에를 호출 합니다.
...
- 마찬가지로, 활동이 대화 상자 또는 경고를 표시 하는 경우 메서드를 사용 하 여 정리 해야 합니다 `.Dismiss()` .
...
[Onrestart](xref:Android.App.Activity.OnRestart) 는 작업이 중지 된 후 다시 시작 되기 전에 호출 됩니다. 이에 대 한 좋은 예는 사용자가 응용 프로그램의 활동에서 홈 단추를 누를 때입니다. 이 경우 `OnPause` `OnStop` 메서드가 호출 되 고 작업은 배경으로 이동 하지만 제거 되지 않습니다. 사용자가 작업 관리자 또는 유사한 응용 프로그램을 사용 하 여 응용 프로그램을 복원 하는 경우 Android는 `OnRestart` 활동의 메서드를 호출 합니다.

에서 구현 해야 하는 논리 종류에 대 한 일반적인 지침은 없습니다 `OnRestart` . 이는 `OnStart` 활동이 만들어지거나 다시 시작 되 고 있는지 여부에 관계 없이 항상 호출 되기 때문입니다. 따라서 활동에 필요한 모든 리소스는가 아니라에서 초기화 되어야 합니다 `OnStart` `OnRestart` .

이후에 호출 되는 다음 수명 주기 메서드는 `OnRestart` 가 됩니다 `OnStart` .
...
응용 프로그램을 백그라운드에 배치 하는 것과 동일한 결과를 표시 하더라도 두 단추 사이에는 약간의 차이가 있습니다. 사용자가 뒤로 단추를 클릭 하면 사용자가 작업을 수행 하는 것을 Android에 지시 하 게 됩니다. Android에서 활동을 제거 합니다. 반면에 사용자가 홈 단추를 클릭 하면 작업은 백그라운드에서 작업을 종료 &ndash; 하지 않습니다.
...
1. `Dictionary`Android에서 상태를 저장 하는 데 사용 하는 [번들](xref:Android.OS.Bundle) 로 알려진의 기본 값을 저장 합니다.
...
- [OnRestoreInstanceState](xref:Android.App.Activity.OnRestoreInstanceState*) &ndash; 메서드가 완료 된 후에 호출 되며 `OnCreate` , 초기화가 완료 된 후 활동에서 해당 상태를 복원할 수 있는 또 다른 기회를 제공 합니다.
...
활동이 중지 될 때 [OnSaveInstanceState](xref:Android.App.Activity.OnSaveInstanceState*) 가 호출 됩니다. 활동에서 해당 상태를 저장할 수 있는 번들 매개 변수를 받게 됩니다. 장치에서 구성 변경이 발생 하는 경우 작업은를 재정의 하 `Bundle` 여 활동 상태를 유지 하기 위해 전달 된 개체를 사용할 수 있습니다 `OnSaveInstanceState` . 예를 들어, 다음 코드를 고려하세요.
...
위의 코드는 라는 단추가 클릭 될 때 라는 정수를 증가 시켜 `c` `incrementCounter` 명명 된에 결과를 표시 `TextView` 합니다 `output` . 구성 변경이 발생 하는 경우, 예를 들어 장치가 회전 되 면 `c` `bundle` `null` 아래 그림에 표시 된 것 처럼 위의 코드는의 값을 잃게 됩니다.

[![이전 값 표시 안 함 표시](images/07-sml.png)](images/07.png#lightbox)

이 예제에서의 값을 유지 하기 위해 `c` 활동은 `OnSaveInstanceState` 아래와 같이 번들에 값을 저장 하 여를 재정의할 수 있습니다.
...
> [!NOTE]
> `OnSaveInstanceState`뷰 계층의 상태를 저장할 수도 있도록 항상의 기본 구현을 호출 하는 것이 중요 합니다.

##### <a name="view-state"></a>상태 보기

재정의 `OnSaveInstanceState` 는 위의 예제에서 카운터와 같이 방향 변경 내용에 따라 활동에 임시 데이터를 저장 하는 데 적합 한 메커니즘입니다. 그러나의 기본 구현은 각 보기 `OnSaveInstanceState` 에 할당 된 ID가 있는 한 모든 보기에 대 한 UI에 임시 데이터를 저장 하는 일을 담당 합니다. 예를 들어 응용 프로그램에 다음과 `EditText` 같이 XML로 정의 된 요소가 있다고 가정 합니다.
...
컨트롤에는 `EditText` 가 할당 되어 있기 때문에 `id` 사용자가 일부 데이터를 입력 하 고 장치를 회전 하는 경우 다음과 같이 데이터가 계속 표시 됩니다.

[![데이터는 가로 모드로 유지 됩니다.](images/08-sml.png)](images/08.png#lightbox)

#### <a name="onrestoreinstancestate"></a>OnRestoreInstanceState

[OnRestoreInstanceState](xref:Android.App.Activity.OnRestoreInstanceState*) 는 후에 호출 됩니다 `OnStart` . 이전을 실행 하는 동안 번들에 이전에 저장 된 모든 상태를 복원할 수 있는 작업을 제공 합니다 `OnSaveInstanceState` . 그러나이는에 제공 되는 것과 동일한 번들입니다 `OnCreate` .

다음 코드에서는에서 상태를 복원 하는 방법을 보여 줍니다 `OnRestoreInstanceState` .
...
이 메서드는 상태를 복원 해야 하는 경우에 대 한 몇 가지 유연성을 제공 하기 위해 존재 합니다. 경우에 따라 인스턴스 상태를 복원 하기 전에 모든 초기화가 완료 될 때까지 대기 하는 것이 더 적절할 수 있습니다. 또한 기존 작업의 서브 클래스는 인스턴스 상태의 특정 값만 복원 하면 됩니다. `OnRestoreInstanceState`대부분의 작업은에 제공 된 번들을 사용 하 여 상태를 복원할 수 있으므로 대부분의 경우를 재정의할 필요가 없습니다 `OnCreate` .

을 사용 하 여 상태를 저장 하는 예제는 `Bundle` [연습-작업 상태 저장](saving-state.md)을 참조 하세요.

#### <a name="bundle-limitations"></a>번들 제한 사항

`OnSaveInstanceState`에서는 임시 데이터를 쉽게 저장할 수 있지만 다음과 같은 몇 가지 제한 사항이 있습니다.

- 모든 경우에 호출 되지 않습니다. 예를 들어 **Home** 또는 **Back** 을 눌러 활동을 종료 하면이 호출 되지 않습니다 `OnSaveInstanceState` .

- 에 전달 된 번들은 `OnSaveInstanceState` 이미지와 같은 커다란 개체를 위해 설계 되지 않았습니다. 개체가 클 경우 아래에 설명 된 대로 [OnRetainNonConfigurationInstance](xref:Android.App.Activity.OnRetainNonConfigurationInstance) 에서 개체를 저장 하는 것이 좋습니다.

- 번들을 사용 하 여 저장 된 데이터는 serialize 되며이로 인해 지연이 발생할 수 있습니다.

번들 상태는 많은 메모리를 사용 하지 않는 단순 데이터에 유용 하지만 *구성 되지 않은 인스턴스 데이터* 는 보다 복잡 한 데이터 또는 검색 비용이 많이 드는 데이터 (예: 웹 서비스 호출 또는 복잡 한 데이터베이스 쿼리)에 유용 합니다. 구성 되지 않은 인스턴스 데이터는 필요에 따라 개체에 저장 됩니다. 다음 섹션에서는 `OnRetainNonConfigurationInstance` 구성 변경을 통해 더 복잡 한 데이터 형식을 유지 하는 방법을 소개 합니다.

### <a name="persisting-complex-data"></a>복합 데이터 유지

Android는 번들에 데이터를 유지 하는 것 외에도 [OnRetainNonConfigurationInstance](xref:Android.App.Activity.OnRetainNonConfigurationInstance) 를 재정의 하 고 유지할 데이터가 포함 된의 인스턴스를 반환 하 여 데이터를 저장 하도록 지원 `Java.Lang.Object` 합니다. 를 사용 하 여 상태를 저장 하는 두 가지 주요 이점이 있습니다 `OnRetainNonConfigurationInstance` .

- 에서 반환 되는 개체는 `OnRetainNonConfigurationInstance` 메모리가이 개체를 유지 하기 때문에 더 크고 복잡 한 데이터 형식에서 잘 작동 합니다.

- `OnRetainNonConfigurationInstance`메서드는 요청 시 및 필요한 경우에만 호출 됩니다. 이 방법은 수동 캐시를 사용 하는 것 보다 경제적입니다.

를 사용 하면 `OnRetainNonConfigurationInstance` 웹 서비스 호출 등에서 데이터를 여러 번 검색 하는 데 비용이 많이 드는 시나리오에 적합 합니다. 예를 들어 Twitter를 검색 하는 다음 코드를 살펴보겠습니다.
...
구성 변경이 발생 하는 경우, 예를 들어 장치가 회전 되 면 코드가 프로세스를 반복 합니다. 원래 검색 된 결과를 다시 사용 하 고 불필요 하 게 중복 된 네트워크 호출을 발생 시 키 지 않고 다음과 같이를 사용 하 여 결과를 저장할 수 있습니다 `OnRetainNonconfigurationInstance` .
...
이제 장치를 회전할 때 원래 결과가 속성에서 검색 됩니다 `LastNonConfiguartionInstance` . 이 예제에서 결과는 `string[]` 포함 하는 트 윗 구성 됩니다. 에서 `OnRetainNonConfigurationInstance` 를 반환 해야 하므로 `Java.Lang.Object` `string[]` `Java.Lang.Object` 아래와 같이는 서브 클래스가 되는 클래스에 래핑됩니다.
...
예를 들어에서 반환 된 개체로를 사용 하려고 하면 `TextView` `OnRetainNonConfigurationInstance` 아래 코드에 나와 있는 것 처럼 활동이 누출 됩니다.
...
이 섹션에서는를 사용 하 여 간단한 상태 데이터를 유지 하는 방법과를 사용 하 여 `Bundle` 더 복잡 한 데이터 형식을 유지 하는 방법을 배웠습니다 `OnRetainNonConfigurationInstance` .
