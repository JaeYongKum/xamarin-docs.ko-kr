...
재질 테마에서 파생 하 여 사용자 고유의 테마를 만들고 일부 또는 모든 색 특성을 재정의할 수 있습니다. 예를 들어에서 파생 되는 테마를 만들 수 `Theme.Material.Light` 있지만, 앱 바 색을 재정의 하 여 브랜드 색과 일치 시킵니다. 또한 개별 보기의 스타일을 지정할 수 있습니다. 예를 들어 모퉁이가 더 둥글고 더 진한 배경색을 사용 하는 [CardView](~/android/user-interface/controls/card-view.md) 에 대 한 스타일을 만들 수 있습니다.
...
응용 프로그램 테마가로 설정 된 경우 `@android:style/Theme.Material.Light` *MyApp* 의 모든 작업은를 사용 하 여 표시 됩니다 `Theme.Material.Light` .

### <a name="theming-an-activity"></a>활동 테마

활동에 테마를 `Theme` `[Activity]` 지정 하려면 활동 선언 위의 특성에 설정을 추가 하 고 사용 하려는 `Theme` 재질 테마 버전에 할당 합니다. 다음 예제에서는를 사용 하 여 작업을 테마에 사용 합니다 `Theme.Material.Light` .
...
브랜드 색으로 앱에 스타일을 지정 하는 사용자 지정 테마를 만들어 브랜드를 향상할 수 있습니다 &rsquo; . 사용자 지정 테마를 만들려면 기본 제공 재질 테마 버전에서 파생 되는 새 스타일을 정의 하 여 변경 하려는 색 특성을 재정의 합니다. 예를 들어에서 파생 되는 사용자 지정 테마를 정의 하 `Theme.Material.Light.DarkActionBar` 고 화면 배경색을 흰색이 아닌 베이 지로 변경할 수 있습니다.

재질 테마는 사용자 지정을 위해 다음과 같은 레이아웃 특성을 제공 합니다.

- `colorPrimary`&ndash;앱 바의 색입니다.

- `colorPrimaryDark`&ndash;상태 표시줄 및 상황별 앱 막대의 색입니다. 일반적으로이는의 진한 버전입니다 `colorPrimary` .
...
기본적으로 `statusBarColor` 는의 값으로 설정 됩니다 `colorPrimaryDark` . 단색으로 설정 `statusBarColor` 하거나로 설정 하 여 상태 표시줄을 투명 하 게 `@android:color/transparent` 만들 수 있습니다. 을로 설정 하 여 탐색 모음을 투명 하 게 만들 수도 있습니다 `navigationBarColor` `@android:color/transparent` .
...
- 이 시점에서 *Mycustomtheme* 을 사용 하는 앱은 `Theme.Material.Light` 사용자 지정 없이 스톡 테마를 표시 합니다.
...
이러한 변경 내용을 적용 하면 *Mycustomtheme* 을 사용 하는 앱은의 및 UI 컨트롤에 앱 바 색을 표시 `my_blue` `my_purple` 하지만 `Theme.Material.Light` 다른 모든 위치에서 색 구성표를 사용 합니다.
...
이 예제에서 *Mycustomtheme* `Theme.Material.Light` 은 배경색, 상태 표시줄 및 텍스트 색에 대해 색을 활용 앱 바의 색을로 변경 하 `my_blue` 고 라디오 단추의 색을로 설정 `my_purple` 합니다.
...
다음 스크린샷은 `CardView` `CardView` 사용자 지정 `CardView.MyBlue` 테마 (오른쪽에 표시 됨)로 스타일이 지정 된와 비교 하 여 왼쪽에 표시 되는 기본값 (왼쪽에 표시 됨)의 예를 제공 합니다.

[![기본 CardView 및 사용자 지정 CardView의 예](material-theme-images/custom-cardview-sml.png)](material-theme-images/custom-cardview.png#lightbox)

이 예제에서 사용자 지정은 `CardView` 배경색과 `my_blue` 18 dp 모퉁이 반지름이 표시 됩니다.
