...
이 가이드의 모든 코드와 XML은이 가이드의 [SplashScreen](/samples/xamarin/monodroid-samples/splashscreen) 샘플 프로젝트에서 찾을 수 있습니다.
...
그러면 `layer-list` 시작 이미지가 리소스로 지정 된 배경색으로 가운데 맞춤 `@color/splash_background` 됩니다. 샘플 응용 프로그램은 **Resources/values/colors.xml** 파일에이 색을 정의 합니다.
...
개체에 대 한 자세한 내용은 `Drawable` [Android에서 사용할 Google 설명서](https://developer.android.com/reference/android/graphics/drawable/Drawable)를 참조 하세요.
...
**Mytheme.** 부흥 시킴는 &ndash; 창 배경을 선언 하 고, 창에서 제목 표시줄을 명시적으로 제거 하 고, 전체 화면 임을 선언 합니다. 활동이 첫 번째 레이아웃을 늘어납니다 하기 전에 앱의 UI를 에뮬레이트하는 시작 화면을 만들려면 `windowContentOverlay` 스타일 정의 대신를 사용할 수 있습니다 `windowBackground` . 이 경우 UI의 에뮬레이션을 표시 하도록 **splash_screen.xml** 그릴 수 있도록 수정 해야 합니다.
...
`OnCreate`테마에서 그릴 수 있는를 배경으로 선언 하므로 레이아웃을 로드할 필요가 없습니다.

작업이 뒤로 스택에서 제거 되도록 특성을 설정 하는 것이 중요 합니다 `NoHistory=true` . 뒤로 단추가 시작 프로세스를 취소 하는 것을 방지 하기 위해를 재정의 하 `OnBackPressed` 고 아무 작업도 수행 하지 않을 수도 있습니다.

```csharp
public override void OnBackPressed() { }
```

시작 작업은에서 비동기적으로 수행 됩니다 `OnResume` . 이는 시작 작업으로 인해 시작 화면의 속도가 느려지거나 지연 되지 않도록 하기 위해 필요 합니다. 작업이 완료 되 면이 `SplashActivity` 시작 되 `MainActivity` 고 사용자가 앱과 상호 작용을 시작할 수 있습니다.

이 새 `SplashActivity` 는 특성을로 설정 하 여 응용 프로그램에 대 한 시작 관리자 활동으로 설정 됩니다 `MainLauncher` `true` . `SplashActivity`는 이제 시작 관리자 작업 이므로에서 특성을 편집 하 `MainActivity.cs` 고 제거 해야 합니다 `MainLauncher` `MainActivity` .
...
6. 값 수정-가로 버전의 시작 화면에 사용할 색을 구성 하려면 **육지/colors.xml** 를 수정 합니다. 이 예제에서는 가로 모드의 경우 시작 배경색이 blue로 변경 됩니다.
...
가로 모드 시작 화면을 사용 하는 경우에는 항상 원활한 환경을 제공 하지는 않습니다. 기본적으로 Android는 앱을 세로 모드로 시작 하 고 장치가 이미 가로 모드에 있는 경우에도 가로 모드로 전환 합니다. 따라서 장치가 가로 모드에 있는 동안 앱이 시작 되 면, 장치는 세로로 세로 시작 화면을 표시 한 다음 세로에서 가로 시작 화면으로 회전에 애니메이션 효과를 적용 합니다. 아쉽게도이 초기 세로-가로 전환은 `ScreenOrientation = Android.Content.PM.ScreenOrientation.Landscape` 시작 작업의 플래그에가 지정 된 경우에도 발생 합니다. 이 제한을 해결 하는 가장 좋은 방법은 세로 및 가로 모드에서 올바르게 렌더링 되는 단일 시작 화면 이미지를 만드는 것입니다.
...
- [SplashScreen (샘플)](/samples/xamarin/monodroid-samples/splashscreen)
