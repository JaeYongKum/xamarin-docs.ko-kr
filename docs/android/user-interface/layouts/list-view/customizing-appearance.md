...
그런 다음 표준 컨트롤 식별자를 참조 하 여 뷰의 속성을 설정할 수 `Text1` 있으며 `Text2` , `Icon` 아래 `Android.Resource.Id` (뷰에 포함 되지 않은 속성을 설정 하지 않거나 예외가 throw 됨).
...
그러면 `Text1` 위에서 설명한 대로 표준 및 컨트롤 식별자를 참조 하 여 그룹 보기 및 자식 뷰에 대 한 속성을 설정할 수 있습니다 `Text2` . SimpleExpandableListItem 스크린 샷 (위에 표시 됨)에서는 한 줄 그룹 보기 (SimpleExpandableListItem1) 및 2 줄 자식 보기 (SimpleExpandableListItem2)의 예를 제공 합니다. 또는 두 줄 (SimpleExpandableListItem2)에 대해 그룹 보기를 구성 하 고 자식 보기를 한 줄 (SimpleExpandableListItem1)에 대해 구성 하거나, 두 그룹 보기와 자식 보기 모두의 줄 수를 지정할 수 있습니다. 
...
이러한 액세서리 중 하나를 표시 하려면 필요한 레이아웃 리소스 ID를 어댑터에 전달 하 고, 필요한 행에 대 한 선택 상태를 수동으로 설정 합니다. 이 코드 줄에서는 `Adapter` 다음 레이아웃 중 하나를 사용 하 여를 만들고 할당 하는 방법을 보여 줍니다.

```csharp
ListAdapter = new ArrayAdapter<String>(this, Android.Resource.Layout.SimpleListItemChecked, items);
```

`ListView`자체는 표시 되는 액세서리에 관계 없이 다양 한 선택 모드를 지원 합니다. 혼동을 피하려면 보조 프로그램에서 선택 모드를 사용 하 고 스타일을 사용 하 여 `Single` `SingleChoice` 또는 모드를 사용 `Checked` `Multiple` `MultipleChoice` 합니다. 선택 모드는의 속성에 의해 제어 됩니다 `ChoiceMode` `ListView` .
...
' 선택 ' 된 항목을 메서드를 사용 하 여 수동으로 설정 `SetItemChecked` (여러 번 선택 하는 경우 여러 번 호출 될 수 있음):
...
또한 코드는 여러 선택 항목과 다르게 단일 선택을 검색 해야 합니다. 모드에서 선택 된 행을 확인 하려면 `Single` 정수 속성을 사용 합니다 `CheckedItemPosition` .

```csharp
FindViewById<ListView>(Android.Resource.Id.List).CheckedItemPosition
```

모드에서 선택 된 행을 확인 하려면를 `Multiple` 반복 해야 `CheckedItemPositions` `SparseBooleanArray` 합니다. 스파스 배열은 값이 변경 된 항목만 포함 하는 사전과 유사 하므로 `true` 다음 코드 조각에 나와 있는 것 처럼 목록에서 선택 된 항목을 확인 하려면 값을 검색 하는 전체 배열을 트래버스 해야 합니다.
...
- `Activity`는이 아니라에서 상속 `ListActivity` 됩니다. 모든 항목에 대 한 행을 사용자 지정할 수 있지만 `ListView` 다른 컨트롤은 `Activity` 레이아웃 (예: 머리글, 단추 또는 기타 사용자 인터페이스 요소)에 포함 될 수도 있습니다. 이 예제에서는 `ListView` 을 설명 하기 위해 위의 제목을 추가 합니다.
...
- `Adapter`구현은 재정의에서 사용자 지정 레이아웃을 반환 합니다 `GetView` .

- `ItemClick` 는 다르게 선언 해야 합니다. 이벤트 처리기는에서 재정의 되는 대신에 연결 됩니다 `ListView.ItemClick`  `OnListItemClick` `ListActivity` .
...
에서는 `HomeScreen` 더 이상 상속 되지 않으므로 `ListActivity` 기본 뷰가 없으므로 HomeScreen 보기에 대 한 레이아웃 axml 파일을 만들어야 합니다. 이 예의 경우 보기에는를 사용 하 여 데이터를 표시 하는 머리글이 포함 됩니다 `TextView` `ListView` . 레이아웃은 다음에 표시 된 **Resources/layout/HomeScreen** 파일에서 정의 됩니다.
...
사용자 지정 레이아웃을 사용 하 여를 사용 하는 경우의 장점 `Activity` (대신 `ListActivity` )은이 예의 머리글과 같이 화면에 컨트롤을 더 추가할 수 `TextView` 있습니다.
...
사용자 지정 어댑터 예의 구현은에 `HomeScreenAdapter.cs` 있습니다. 키 메서드는 `GetView` 리소스 ID를 사용 하 여 사용자 지정 AXML을 로드 한 `Resource.Layout.CustomView` 다음 반환 하기 전에 뷰의 각 컨트롤에 대 한 속성을 설정 하는 위치입니다. 전체 어댑터 클래스가 표시 됩니다.
...
`HomeScreen`이제 클래스는에서 상속 되므로 `Activity` `ListView` axml에 선언 된 컨트롤에 대 한 참조를 포함 하기 위해 클래스에서 필드가 선언 됩니다.

```csharp
ListView listView;
```

그런 다음 클래스는 메서드를 사용 하 여 작업의 사용자 지정 레이아웃 AXML을 로드 해야 합니다 `SetContentView` . 그런 다음 `ListView` 레이아웃에서 컨트롤을 찾은 다음 어댑터를 만들어 할당 하 고 클릭 처리기를 할당 합니다. OnCreate 메서드에 대 한 코드가 다음과 같이 표시 됩니다.
...
마지막으로 `ItemClick` 처리기를 정의 해야 합니다 .이 경우에는 다음과 같이 메시지를 표시 하기만 하면 됩니다 `Toast` .
...
강조 표시 동작을 다시 사용 하도록 설정 하 고 사용 되는 색을 사용자 지정 하려면 배경 특성을 사용자 지정 선택기로 설정 합니다. 이 선택기는 기본 배경색과 강조 색을 모두 선언 합니다. 파일 **리소스/그릴 ** 때/CustomSelector.xml에는 다음 선언이 포함 됩니다.
...
선택한 행과 해당 메시지는 `Toast` 이제 다음과 같이 표시 됩니다.
...
Android는 레이아웃 정보를 캐싱하여 스크롤 성능을 향상 시 키 려 고 `ListView` 합니다. 데이터의 스크롤 목록이 긴 경우 `android:cacheColorHint` `ListView` 작업의 axml 정의에서 선언에 대 한 속성을 사용자 지정 행 레이아웃의 배경과 동일한 색 값으로 설정 해야 합니다. 사용자가 사용자 지정 행 배경색이 있는 목록을 스크롤할 때이 힌트를 포함 하지 않으면 ' 깜박임 '이 발생할 수 있습니다.
