...
ms.locfileid: "91457330"
---
# <a name="xamarinandroid-gridlayout"></a>Xamarin Android GridLayout

는 `GridLayout` `ViewGroup` 아래와 같이 HTML 테이블과 비슷하게 2d 그리드에서 보기 레이아웃을 지 원하는 새 하위 클래스입니다.
...
다음 XML은 `TextView` *GridLayout*에 여러 개의 컨트롤을 추가 합니다.
...
위의 XML에서 각는 `TextView` 행 이나 열을 지정 하지 않습니다. 이러한 지정을 지정 하지 않으면는 `GridLayout` 방향에 따라 각 자식 뷰를 순서 대로 할당 합니다. 예를 들어 GridLayout의 방향을 기본값인 가로에서 다음과 같이 세로로 변경 하겠습니다.
...
에서 자식 뷰의 위치를 명시적으로 제어 하려는 경우 `GridLayout` 해당 및 특성을 설정할 수 있습니다 `layout_row` `layout_column` . 예를 들어 다음 XML은 방향에 관계 없이 첫 번째 스크린샷 (위에 표시 됨)에 표시 되는 레이아웃을 생성 합니다.
...
의 자식 보기 사이에 간격을 지정 하는 몇 가지 옵션이 있습니다 `GridLayout` . `layout_margin`아래와 같이 특성을 사용 하 여 각 자식 보기의 여백을 직접 설정할 수 있습니다.
...
또한 Android 4에서는 이제 라는 새로운 범용 간격 뷰를 `Space` 사용할 수 있습니다. 이를 사용 하려면 자식 뷰로 추가 하기만 하면 됩니다.
예를 들어 아래 XML은를 3으로 설정 하 여에 추가 행을 추가 하 `GridLayout` `rowcount` 고 사이에 `Space` 간격을 제공 하는 뷰를 추가 합니다 `TextViews` .
...
이 XML은 아래와 같이에 간격을 만듭니다 `GridLayout` .
...
새 보기를 사용 하면 `Space` 간격을 허용 하 고 모든 자식 보기에서 특성을 설정할 필요가 없다는 이점도 있습니다.

### <a name="spanning-columns-and-rows"></a>열 및 행 확장

에서는 `GridLayout` 여러 열과 행에 걸쳐 있는 셀도 지원 합니다. 예를 들어 아래와 같이 단추를 포함 하는 다른 행을에 추가 한다고 가정 합니다 `GridLayout` .
...
그러면 다음과 같이의 첫 번째 열이 `GridLayout` 단추의 크기에 맞게 늘어납니다.
...
이렇게 하면 `TextViews` 아래와 같이의 아래쪽에 단추가 추가 된 상태로 이전에 있던 레이아웃과 유사한의 레이아웃이 만들어집니다 `GridLayout` .
...
- [GridLayoutDemo (샘플)](/samples/xamarin/monodroid-samples/gridlayoutdemo)