...
이 가이드에서는 `CardView` Xamarin Android 프로젝트에 패키지를 추가 하는 방법, 레이아웃에 추가 하는 방법 `CardView` 및 앱에서의 모양을 사용자 지정 하는 방법을 설명 합니다 `CardView` . 또한이 가이드에서는 `CardView` `CardView` Android 5.0 롤리팝 이전의 android 버전에서 사용 하는 데 도움이 되는 특성을 포함 하 여 변경할 수 있는 특성의 자세한 목록을 제공 합니다.

<a name="requirements"></a>

## <a name="requirements"></a>요구 사항

다음은 `CardView` Xamarin 기반 앱에서 새로운 Android 5.0 이상 기능 (포함)을 사용 하는 데 필요 합니다.
...
- **JAVA JDK 1.8** &ndash; 구체적으로 API 레벨 23이 하를 대상으로 하는 경우 JDK 1.7을 사용할 수 있습니다. JDK 1.8은 [Oracle](https://www.oracle.com/technetwork/java/javase/downloads/jdk8-downloads-2133151.html)에서 사용할 수 있습니다.

앱에도 패키지를 포함 해야 합니다 `Xamarin.Android.Support.v7.CardView` . `Xamarin.Android.Support.v7.CardView`Mac용 Visual Studio에서 패키지를 추가 하려면 다음을 수행 합니다.
...
`Xamarin.Android.Support.v7.CardView`Visual Studio에서 패키지를 추가 하려면 다음을 수행 합니다.
...
기본값은 `CardView` 모퉁이가 둥근 모퉁이가 작은 흰색 카드와 약간의 그림자와 유사 합니다. 다음 예제 **Main. axml** 레이아웃은 `CardView` 를 포함 하는 단일 위젯을 표시 합니다 `TextView` .
...
이 예제에서는 앱 스타일을 밝은 재질 테마 ()로 설정 하 여 `Theme.Material.Light` `CardView` 그림자와 가장자리를 더 쉽게 볼 수 있도록 합니다. Android 5.0 앱에 테마를 표시 하는 방법에 대 한 자세한 내용은 [재질 테마](~/android/user-interface/material-theme.md)를 참조 하세요. 다음 섹션에서는 `CardView` 응용 프로그램에 대해 사용자 지정 하는 방법을 배웁니다.

## <a name="customizing-cardview"></a>CardView 사용자 지정

기본 특성을 수정 `CardView` 하 여 앱에서의 모양을 사용자 지정할 수 있습니다 `CardView` . 예를 들어의 상승을 `CardView` 늘려 더 큰 그림자를 캐스팅할 수 있습니다 (카드를 배경 위에 배치 하는 것 처럼 보이게 함). 또한 카드의 모퉁이가 더 둥글게 되도록 모퉁이 반경을 늘릴 수 있습니다.

다음 레이아웃 예제에서는 사용자 지정 된를 `CardView` 사용 하 여 인쇄 사진 ("스냅숏")의 시뮬레이션을 만듭니다. 는 `ImageView` `CardView` 이미지를 표시 하기 위해에 추가 되 고는 `TextView` `ImageView` 이미지의 제목을 표시 하기 위해 아래에 배치 됩니다.
이 예제 레이아웃에서에는 `CardView` 다음과 같은 사용자 지정이 있습니다.

- `cardElevation`큰 그림자를 캐스트 하기 위해가 4dp로 증가 합니다.
- `cardCornerRadius`모퉁이가 더 둥근 모양으로 보이도록가 5dp로 증가 합니다.

`CardView`는 Android v7 지원 라이브러리에서 제공 되기 때문에 네임 스페이스에서 해당 특성을 사용할 수 없습니다 `android:` . 따라서 고유한 XML 네임 스페이스를 정의 하 고 해당 네임 스페이스를 특성 접두사로 사용 해야 합니다 `CardView` . 아래 레이아웃 예제에서는이 줄을 사용 하 여 라는 네임 스페이스를 정의 합니다 `cardview` .

```xml
    xmlns:cardview="http://schemas.android.com/apk/res-auto"
```

이 네임 스페이스를 호출 `card_view` 하거나 `myapp` (이 파일의 범위 내 에서만 액세스할 수 있음)를 선택할 수도 있습니다. 이 네임 스페이스를 호출 하도록 선택 하는 경우에는이 네임 스페이스를 사용 하 여 수정 하려는 특성에 접두사를 사용 해야 합니다 `CardView` . 이 레이아웃 예제에서 `cardview` 네임 스페이스는 및에 대 한 접두사입니다 `cardElevation` `cardCornerRadius` .
...
이 레이아웃 예제를 사용 하 여 사진 보기 앱에서 이미지를 표시 하는 경우에는 `CardView` 다음 스크린샷에 표시 된 것 처럼에 사진 스냅숏의 모양이 있습니다.
...
- `contentPadding`의 &ndash; 자식 뷰와 `CardView` 카드의 모든 가장자리 사이의 안쪽 안쪽 여백입니다.

- `contentPaddingBottom`&ndash;의 자식 뷰와 `CardView` 카드의 아래쪽 가장자리 사이의 안쪽 안쪽 여백입니다.

- `contentPaddingLeft`&ndash;의 자식 뷰와 `CardView` 카드의 왼쪽 가장자리 사이의 안쪽 안쪽 여백입니다.

- `contentPaddingRight`&ndash;의 자식 뷰와 `CardView` 카드의 오른쪽 가장자리 사이의 안쪽 안쪽 여백입니다.

- `contentPaddingTop`&ndash;의 자식 뷰와 `CardView` 카드의 위쪽 가장자리 사이의 안쪽 안쪽 여백입니다.

콘텐츠 패딩 특성은 콘텐츠 영역 내에 있는 지정 된 위젯이 아니라 콘텐츠 영역의 경계를 기준으로 합니다.
예를 들어 `contentPadding` 사진 보기 앱에서 충분히 늘어난 경우는 `CardView` 카드에 표시 된 이미지와 텍스트를 모두 자릅니다.
...
- `cardElevation`&ndash;의 상승 `CardView` (Z 축 나타냄)입니다.

- `cardMaxElevation`&ndash;상승의 최대값입니다 `CardView` .

값이 클수록 `cardElevation` `CardView` 배경 위에 더 큰 부동으로 보이도록 그림자 크기가 증가 합니다. `cardElevation`특성은 겹치는 보기의 그리기 순서를 결정 합니다. 즉, `CardView` 높은 권한 상승을 사용 하는 다른 겹치는 뷰로 그리고 낮은 권한 설정으로 겹치는 보기 위에 그려집니다.
`cardMaxElevation`설정은 앱에서 권한 상승을 동적으로 변경 하는 경우 &ndash; 이 설정을 사용 하 여 정의 하는 한도를 초과 하 여 섀도를 확장할 수 없는 경우에 유용 합니다.
...
- `cardCornerRadius`&ndash;의 모든 모퉁이에 대 한 모퉁이 반경입니다 `CardView` .

- `cardBackgroundColor`&ndash;의 배경색입니다 `CardView` .

이 다이어그램에서 `cardCornerRadius` 는 보다 반올림 된 10dp로 설정 되 고 `cardBackgroundColor` 는 `"#FFFFCC"` (연한 노랑)로 설정 됩니다.
...
- `cardPreventCornerOverlap`&ndash; `true` 이전 Android 버전에서 앱을 실행 하는 경우이 특성을로 설정 합니다 (API 레벨 20 및 이전 버전). 이 설정은 `CardView` 콘텐츠가 모퉁이가 둥근 모퉁이와 교차 하지 않도록 `CardView` 합니다.

- `cardUseCompatPadding`&ndash;앱이 Android 버전 이상에서 실행 되는 경우에는이 특성을로 설정 하 여 `true` 패딩을 추가 합니다. `CardView`롤리팝 이전 장치에서를 사용 하 고 롤리팝에서 동일 하 게 보이도록 하려면이 특성을로 설정 `true` 합니다. 이 특성을 사용 하는 경우에는 `CardView` 미리 롤리팝 장치에서 실행 될 때 그림자를 그리기 위해 안쪽 여백을 추가 합니다. 이렇게 하면 이전 버전의 프로그래밍 그림자 구현이 적용 될 때 도입 된 패딩의 차이를 극복 하는 데 도움이 됩니다.
...
이 가이드에서는 `CardView` Android 5.0 (롤리팝)에 포함 된 새로운 위젯을 소개 했습니다. 기본 `CardView` 모양을 보여주고 `CardView` , 높이, 모퉁이 원형율, 콘텐츠 안쪽 여백 및 배경색을 변경 하 여 사용자 지정 하는 방법을 설명 했습니다. `CardView`레이아웃 특성 (참조 다이어그램 포함)이 나열 되 고 `CardView` Android 5.0 롤리팝 이전의 android 장치에서를 사용 하는 방법에 대해 설명 했습니다. 에 대 한 자세한 내용은 `CardView` [CardView 클래스 참조](https://developer.android.com/reference/android/support/v7/widget/CardView.html)를 참조 하세요.
