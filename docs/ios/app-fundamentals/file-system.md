...
Xamarin.ios를 사용 하면 `System.IO` ios에서 파일 시스템 작업에 대 한 .net 클래스를 사용할 수 있습니다.
...
전체 네임 스페이스를 사용 하 `System.Xml` 는 작업은이 문서에서 다루지 않지만 다음 코드 조각과 같은 StreamReader를 사용 하 여 파일 시스템에서 XML 문서를 쉽게 deserialize 할 수 있습니다.
...
자세한 내용은 [System.Xml](xref:System.Xml) 및 [serialization](xref:System.Xml.Serialization)에 대 한 설명서를 참조 하세요. 링커에 대 한 [xamarin.ios 설명서](~/ios/deploy-test/linker.md) 를 참조 하세요. `[Preserve]` 직렬화 하려는 클래스에 특성을 추가 해야 하는 경우가 많습니다.

### <a name="creating-files-and-directories"></a>파일 및 디렉터리 만들기

이 샘플에서는 클래스를 사용 하 여 `Environment` 파일 및 디렉터리를 만들 수 있는 문서 폴더에 액세스 하는 방법을 보여 줍니다.
...
[![응용 프로그램 프로젝트에 NuGet 패키지 추가](file-system-images/json01.png)](file-system-images/json01.png#lightbox)
...
마지막으로 클래스의 인스턴스를 만들고 `Account` , json 데이터로 serialize 하 고, 파일에 씁니다.
...
이러한 혼동 때문에 `System.IO.Path.Combine` 특정 경로 구분 기호를 하드 코딩 하는 대신 현재 플랫폼에 맞게 조정 하는 메서드를 사용 하는 것이 좋습니다. 이는 코드를 다른 플랫폼으로 이식할 수 있게 해 주는 간단한 단계입니다.
...
[![이 스크린샷은 Finder 창의 디렉터리 구조를 보여줍니다.](file-system-images/08-library-directory.png)](file-system-images/08-library-directory.png#lightbox)

### <a name="accessing-other-directories-programmatically"></a>프로그래밍 방식으로 다른 디렉터리 액세스

이전 디렉터리 및 파일 예제는 디렉터리에 액세스 `Documents` 했습니다. 다른 디렉터리에 쓰려면 다음과 같이 "..." 구문을 사용 하 여 경로를 생성 해야 합니다.
...
및 디렉터리에 대 한 경로는 `Caches` `tmp` 다음과 같이 생성할 수 있습니다.
...
iOS 11에는 사용자가 iCloud에서 파일을 보고 상호 작용 하 고이를 지 원하는 응용 프로그램 에서도 저장할 수 있도록 하는 iOS 용 파일 **브라우저 파일이 도입** 되었습니다. 사용자가 앱의 파일에 직접 액세스할 수 있도록 하려면 **info.plist** 파일에 새 부울 키를 만들고 `LSSupportsOpeningDocumentsInPlace` 다음과 같이로 설정 합니다 `true` .
...
사용자는 다음과 같이 `Info.plist` 소스 뷰에서 iTunes 공유 () 항목을 **지 원하는 응용** 프로그램을 편집 하 고 만들어 응용 프로그램의 문서 디렉터리에 있는 파일에 액세스할 수 있습니다 `UIFileSharingEnabled` . **Source**
...
이러한 파일은 장치가 연결 되어 있고 사용자가 탭을 선택할 때 iTunes에서 액세스할 수 있습니다 `Apps` . 예를 들어 다음 스크린샷은 iTunes를 통해 공유 되는 선택 된 앱의 파일을 보여 줍니다.
...
- 또는에서 쉽게 다시 만들거나 다시 다운로드할 수 있는 다른 데이터를 저장 `Library/Caches` `tmp` 합니다 .이 데이터는 백업 되지 않으며 ' 치료 ' 될 수 있습니다.
- 또는 폴더에 적합 한 파일이 `Library/Caches` `tmp` 있지만 ' 치료 ' 하지 않으려는 경우 다른 곳에 저장 하 `Library/YourData` 고 ' 백업 안 함 ' 특성을 적용 하 여 파일이 iCloud 백업 대역폭과 저장소 공간을 사용 하지 않도록 합니다. 이 데이터는 장치에서 공간을 계속 사용 하므로 신중 하 게 관리 하 고 가능한 경우 삭제 해야 합니다.

' 백업 안 함 ' 특성은 클래스를 사용 하 여 설정 됩니다 `NSFileManager` . 클래스가 `using Foundation` 이 고 다음과 같이를 호출 하는지 확인 합니다 `SetSkipBackupAttribute` .
...
`SetSkipBackupAttribute`이 `true` 파일이 저장 된 디렉터리 (디렉터리에도 상관 없음)에 관계 없이 백업 되지 않습니다 `Documents` . 메서드를 사용 하 여 특성을 쿼리하면 `GetSkipBackupAttribute` `SetSkipBackupAttribute` `false` 다음과 같이를 사용 하 여 메서드를 호출 하 여이를 다시 설정할 수 있습니다.
...
> [!IMPORTANT]
> 반환 된 그룹 경로가 인 경우 `null` 자격 및 프로 비전 프로필의 구성이 올바른지 확인 하 고 올바른지 확인 합니다.
