...
description: '이 가이드에서는 ios 디자이너에서 iOS 자동 레이아웃 제약 조건을 만드는 대신 c # 코드에서 작업 하는 방법을 보여 줍니다.'
ms.prod: xamarin
...
_이 가이드에서는 ios 디자이너에서 iOS 자동 레이아웃 제약 조건을 만드는 대신 c # 코드에서 작업 하는 방법을 보여 줍니다._
...
일반적으로 iOS에서 자동 레이아웃으로 작업할 때 iOS 디자이너를 사용 하 여 UI 항목에 레이아웃 제약 조건을 그래픽으로 배치 합니다. 그러나 c # 코드에서 제약 조건을 만들고 적용 해야 하는 경우가 있을 수 있습니다. 예를 들어 동적으로 생성 된 UI 요소를에 추가 하는 경우 `UIView` 입니다.

이 가이드에서는 iOS 디자이너에서 그래픽을 만드는 대신 c # 코드를 사용 하 여 제약 조건을 만들고 작업 하는 방법을 보여 줍니다.
...
- [레이아웃 앵커](#Layout-Anchors) -이 API는 `TopAnchor` `BottomAnchor` `HeightAnchor` 제한 되는 UI 항목의 앵커 속성 (예: 또는)에 대 한 액세스를 제공 합니다.
- [레이아웃 제약 조건](#Layout-Constraints) -클래스를 사용 하 여 직접 제약 조건을 만들 수 있습니다 `NSLayoutConstraint` .
...
클래스를 사용 하 여 `NSLayoutAnchor` 제한 되는 UI 항목의 앵커 속성을 기반으로 제약 조건을 만들기 위한 흐름 인터페이스가 있습니다. 예를 들어 뷰 컨트롤러의 위쪽 및 아래쪽 레이아웃 안내선은 `TopAnchor` , `BottomAnchor` 및 `HeightAnchor` 앵커 속성을 노출 하는 반면 뷰는 가장자리, 가운데, 크기 및 기준 속성을 노출 합니다.

> [!IMPORTANT]
> 표준 앵커 속성 집합 외에도 iOS 보기에는 `LayoutMarginsGuides` 및 `ReadableContentGuide` 속성이 포함 됩니다. 이러한 속성 `UILayoutGuide` 은 각각 보기의 여백 및 읽을 수 있는 콘텐츠 가이드를 사용 하기 위한 개체를 노출 합니다.

레이아웃 앵커는 읽기 쉬운 간단한 형식으로 제약 조건을 만들 수 있는 몇 가지 메서드를 제공 합니다.

- **ConstraintEqualTo** - `first attribute = second attribute + [constant]` 선택적으로 제공 되는 오프셋 값을 사용 하는 관계를 정의 `constant` 합니다.
...
여기서 c # 코드의 부분은 다음과 같이 수식의 지정 된 부분에 해당 합니다.
...
지정 된 레이아웃 제약 조건 수식을 해결 하는 데 필요한 매개 변수만 제공 하는 것 외에도 각 레이아웃 앵커 메서드는 전달 된 매개 변수의 형식 안전성을 적용 합니다. 따라서 또는와 같은 가로 제약 조건 앵커는 `LeadingAnchor` `TrailingAnchor` 다른 수평선 앵커 유형에만 사용할 수 있으며, 승수는 크기 제약 조건에만 제공 됩니다.
...
여기서 `NSLayoutAttribute` 열거형은 뷰의 여백 값을 정의 하 고 `LayoutMarginsGuide` , 및와 같은 속성에 해당 하며 `Left` , `Right` `Top` `Bottom` 열거형은 지정 된 `NSLayoutRelation` 특성 간에 생성 되는 관계를, 또는로 정의 합니다 `Equal` `LessThanOrEqual` `GreaterThanOrEqual` .

레이아웃 앵커 API와 달리 `NSLayoutConstraint` 생성 메서드는 특정 제약 조건의 중요 한 측면을 강조 하지 않으며 제약 조건에 대해 수행 되는 컴파일 시간 검사는 없습니다. 따라서 런타임에 예외를 throw 하는 잘못 된 제약 조건을 쉽게 생성할 수 있습니다.
...
1. `NSDictionary`뷰 개체 및 레이아웃 안내선과 형식을 정의할 때 사용할 문자열 키를 포함 하는을 만듭니다.
2. 필요에 따라 `NSDictionary` `NSNumber` 제약 조건에 대 한 상수 값으로 사용 되는 키 및 값 () 집합을 정의 하는을 만듭니다.
3. 단일 열 또는 항목 행을 레이아웃 하는 형식 문자열을 만듭니다.
4. `FromVisualFormat`클래스의 메서드를 호출 `NSLayoutConstraint` 하 여 제약 조건을 생성 합니다.
5. `ActivateConstraints`클래스의 메서드를 호출 `NSLayoutConstraint` 하 여 제약 조건을 활성화 하 고 적용 합니다.
...
이 가이드에서는 c #에서 자동 레이아웃 제약 조건을 만들고 사용 하는 것을 iOS 디자이너에서 그래픽으로 만드는 것이 아니라, 먼저 레이아웃 앵커 ()를 사용 하 여 자동 레이아웃을 처리 하는 방법을 살펴보았습니다 `NSLayoutAnchor` . 다음으로 레이아웃 제약 조건 ()을 사용 하는 방법을 살펴보았습니다 `NSLayoutConstraint` . 마지막으로, 자동 레이아웃에 대 한 시각적 서식 언어를 사용 하 여 표시 됩니다.
