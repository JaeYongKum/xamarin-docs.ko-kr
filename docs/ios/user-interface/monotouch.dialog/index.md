...
Monotouch.dialog. 대화 상자를 MT 라고 합니다. D의 경우는 개발자가 뷰 컨트롤러, 테이블 등의 번거로움을 아니라 정보를 사용 하 여 응용 프로그램 화면과 탐색을 빌드할 수 있게 해 주는 신속한 UI 개발 도구 키트입니다. 따라서 UI 개발 및 코드 감소를 매우 단순화 합니다. 예를 들어 다음 스크린샷에서를 살펴보세요.
 ...
 [![예를 들어이 스크린샷](images/image1.png)](images/image1.png#lightbox)
...
*Dialogviewcontroller*또는 short에 대 한 *dvc* 는에서 상속 `UITableViewController` 되므로 테이블이 있는 화면을 나타냅니다. DVCs는 일반 UITableViewController와 마찬가지로 탐색 컨트롤러에 푸시할 수 있습니다.
...
섹션은 테이블의 셀 그룹입니다. 일반 테이블 섹션과 마찬가지로, 다음 스크린샷에 나와 있는 것 처럼 텍스트 또는 사용자 지정 보기를 포함할 수 있는 머리글 및 바닥글을 선택적으로 포함할 수 있습니다.
 ...
 [![일반 테이블 섹션과 마찬가지로,이 스크린샷에서와 같이 텍스트 또는 사용자 지정 보기를 포함할 수 있는 머리글 및 바닥글을 선택적으로 포함할 수 있습니다.](images/image2.png)](images/image2.png#lightbox)

### <a name="element"></a>요소

요소는 테이블의 실제 셀을 나타냅니다. 휴먼. D는 다양 한 데이터 형식 또는 다른 입력을 나타내는 다양 한 요소로 압축 됩니다. 예를 들어 다음 스크린샷에서는 사용 가능한 몇 가지 요소를 보여 줍니다.

 [![예를 들어이 스크린샷에서는 사용 가능한 몇 가지 요소를 보여 줍니다.](images/image3.png)](images/image3.png#lightbox)
...
Add 메서드를 호출 하거나 c # 4 이니셜라이저 구문을 사용 하 여 섹션을 추가 합니다.
...
속성과 결합 된 `Element.Value` 콜백은 다른 요소에 설정 된 값을 검색할 수 있습니다. 예를 들어, 다음 코드를 고려하세요.
...
이 코드는 아래와 같이 UI를 만듭니다. 이 예제에 대 한 전체 연습은 [요소 API 연습](~/ios/user-interface/monotouch.dialog/elements-api-walkthrough.md) 자습서를 참조 하세요.
 ...
 [![다른 요소에 설정 된 값을 가져올 수 있습니다. 값 속성과 함께 사용 됩니다.](images/image6.png)](images/image6.png#lightbox)

사용자가 아래쪽 테이블 셀을 누르면 익명 함수의 코드가 실행 되어 인스턴스의 값이 `element` Mac용 Visual Studio의 **응용 프로그램 출력** 패드에 기록 됩니다.
...
사용자 입력을 캡처하는 데 사용 되는 요소는 언제 `Value` 든 지 요소의 현재 값을 보유 하는 public 속성을 노출 합니다. 사용자가 응용 프로그램을 사용 하면 자동으로 업데이트 됩니다.
...
는 `StringElement` 표 셀의 왼쪽에 캡션을 표시 하 고 셀의 오른쪽에 문자열 값을 표시 합니다.
 ...
 [![StringElement는 표 셀의 왼쪽에 캡션을 표시 하 고 셀의 오른쪽에 있는 문자열 값을 표시 합니다.](images/image7.png)](images/image7.png#lightbox)

를 단추로 사용 하려면 `StringElement` 대리자를 제공 합니다.
...
```
 ...
 [![StringElement를 단추로 사용 하려면 대리자를 제공 합니다.](images/image8.png)](images/image8.png#lightbox)
...
에서는 `StyledStringElement` 기본 제공 테이블 셀 스타일 또는 사용자 지정 서식 지정을 사용 하 여 문자열을 표시할 수 있습니다.
 ...
 [![StyledStringElement를 사용 하면 기본 제공 테이블 셀 스타일이 나 사용자 지정 서식 지정을 사용 하 여 문자열을 표시할 수 있습니다.](images/image9.png)](images/image9.png#lightbox)

`StyledStringElement`클래스는에서 파생 `StringElement` 되지만 개발자가 글꼴, 텍스트 색, 배경 셀 색, 줄 바꿈 모드, 표시할 줄 수 및 액세서리를 표시할지 여부와 같은 몇 가지 속성을 사용자 지정할 수 있습니다.

### <a name="multiline-element"></a>Multiline 요소

 [![Multiline 요소](images/image10.png)](images/image10.png#lightbox)

### <a name="entry-element"></a>Entry 요소

`EntryElement`이름에서 알 수 있듯이은 사용자 입력을 가져오는 데 사용 됩니다. 문자를 숨기는 일반 문자열 또는 암호를 지원 합니다.

 [![EntryElement는 사용자 입력을 가져오는 데 사용 됩니다.](images/image11.png)](images/image11.png#lightbox)
...
어떤 지점에서 든 해당 Value 속성에 액세스 하면의 값을 검색할 수 있습니다 `EntryElement` .
...
### <a name="boolean-element"></a>부울 요소
 ...
 [![부울 요소](images/image12.png)](images/image12.png#lightbox)

### <a name="checkbox-element"></a>Checkbox 요소

 [![Checkbox 요소](images/image13.png)](images/image13.png#lightbox)

### <a name="radio-element"></a>Radio 요소

를 `RadioElement` 사용 하려면에를 `RadioGroup` 지정 해야 합니다 `RootElement` .
...
의 그룹 이름은 `RadioGroup` 포함 하는 페이지 (있는 경우)에서 선택한 값을 표시 하는 데 사용 되 고,이 경우 0 인 값은 첫 번째 선택 된 항목의 인덱스입니다.

### <a name="badge-element"></a>배지 요소

 [![배지 요소](images/image15.png)](images/image15.png#lightbox)

### <a name="float-element"></a>Float 요소

 [![Float 요소](images/image16.png)](images/image16.png#lightbox)

### <a name="activity-element"></a>Activity 요소

 [![Activity 요소](images/image17.png)](images/image17.png#lightbox)

### <a name="date-element"></a>Date 요소

 ![Date 요소](images/image18.png)

DateElement에 해당 하는 셀을 선택 하면 날짜 선택이 다음과 같이 표시 됩니다.

 [![DateElement에 해당 하는 셀을 선택 하면 다음과 같이 날짜 선택이 표시 됩니다.](images/image19.png)](images/image19.png#lightbox)

### <a name="time-element"></a>Time 요소

 [![Time 요소](images/image20.png)](images/image20.png#lightbox)

TimeElement에 해당 하는 셀을 선택 하면 다음과 같이 시간 선택이 표시 됩니다.

 [![TimeElement에 해당 하는 셀을 선택 하면 다음과 같이 시간 선택이 표시 됩니다.](images/image21.png)](images/image21.png#lightbox)

### <a name="datetime-element"></a>DateTime 요소

 [![DateTime 요소](images/image22.png)](images/image22.png#lightbox)

DateTimeElement에 해당 하는 셀을 선택 하면 다음과 같이 datetime 선택이 표시 됩니다.

 [![DateTimeElement에 해당 하는 셀을 선택 하면 datetime 선택이 다음과 같이 표시 됩니다.](images/image23.png)](images/image23.png#lightbox)

### <a name="html-element"></a>HTML 요소

 [![HTML 요소](images/image24.png)](images/image24.png#lightbox)

는 `HTMLElement` 테이블 셀에 속성의 값을 표시 합니다 `Caption` . 선택 선택 된 경우 `Url` 요소에 할당 된는 `UIWebView` 아래와 같이 컨트롤에 로드 됩니다.

 [![선택 선택 된 경우 요소에 할당 된 Url은 아래와 같이 UIWebView 보기 컨트롤에 로드 됩니다.](images/image25.png)](images/image25.png#lightbox)

### <a name="message-element"></a>Message 요소

 [![Message 요소](images/image26.png)](images/image26.png#lightbox)
...
`UIActivity`표시기가 애니메이션 효과를 시작 하 고 사용자가 셀을 탭 한 다음 생성자에 전달 된가 실행 될 때 로드 캡션이 표시 됩니다 `NSAction` . 의 코드가 완료 되 면 `NSAction` `UIActivity` 표시기가 애니메이션 효과를 중지 하 고 일반 캡션이 다시 표시 됩니다.

### <a name="uiview-element"></a>UIView 요소

또한를 사용 하 여 모든 사용자 지정을 `UIView` 표시할 수 있습니다 `UIViewElement` .

### <a name="owner-drawn-element"></a>소유자가 그린 요소

이 요소는 추상 클래스 이므로 서브클래싱 되어야 합니다. `Height(RectangleF bounds)`요소의 높이를 반환 해야 하는 메서드와 `Draw(RectangleF bounds, CGContext context, UIView view)` 컨텍스트 및 뷰 매개 변수를 사용 하 여 지정 된 범위 내에서 사용자 지정 된 모든 그리기를 수행 해야 하는 메서드를 재정의 해야 합니다. 이 요소는를 서브클래싱 하 `UIView` 고 반환 될 셀에 배치 하 여 두 개의 간단한 재정의를 구현 하기만 하면 됩니다. 파일의 샘플 앱에서 더 나은 샘플 구현을 볼 수 있습니다 `DemoOwnerDrawnElement.cs` .
...
는 `JsonElement` `RootElement` `RootElement` 로컬 또는 원격 url에서 중첩 된 자식의 콘텐츠를 로드할 수 있도록를 확장 하는의 서브 클래스입니다.

는 `JsonElement` `RootElement` 두 가지 형식으로 인스턴스화할 수 있는입니다. 한 버전은 `RootElement` 요청 시 콘텐츠를 로드 하는을 만듭니다. 이러한 매개 변수는 끝 부분에서 추가 인수를 사용 하는 생성자를 사용 하 여 만들고 `JsonElement` , 콘텐츠를 로드 하는 데 사용 되는 url입니다.

```csharp
var je = new JsonElement ("Dynamic Data", "https://tirania.org/tmp/demo.json");
```

다른 양식은 `System.Json.JsonObject` 이미 구문 분석 한 로컬 파일 또는 기존에서 데이터를 만듭니다.
...
대화 상자에 자동으로 끌어오기-새로 고침 지원을 추가 하려면 다음 두 가지 작업을 수행 해야 합니다. 즉, 사용자가 데이터를 끌어올 때 알림을 받도록 이벤트 처리기를 연결 하 고 데이터를 로드 하 여 `DialogViewController` 기본 상태로 되돌릴 때에 게 알립니다.

알림을 후크 하는 작업은 간단 합니다. 다음과 같이의 이벤트에 연결 하기만 하면 됩니다 `RefreshRequested` `DialogViewController` .

```csharp
dvc.RefreshRequested += OnUserRequestedRefresh;
```

그런 다음 메서드에서 `OnUserRequestedRefresh` 일부 데이터 로드를 큐에 대기 시키고, net에서 일부 데이터를 요청 하거나, 데이터를 계산 하기 위해 스레드를 회전 합니다. 데이터가 로드 되 면에 `DialogViewController` 새 데이터가 있음을 알리고 다음을 호출 하 여 뷰를 기본 상태로 복원 해야 합니다 `ReloadComplete` .
...
검색을 지원 하려면 `EnableSearch` 에서 속성을 설정 `DialogViewController` 합니다. `SearchPlaceholder`검색 표시줄의 워터 마크 텍스트로 사용 하도록 속성을 설정할 수도 있습니다.
...
이미지 로더는 클래스에서 구현 되며, 메서드를 호출 하기만 하면 `ImageLoader` `DefaultRequestImage` 로드 하려는 이미지에 대 한 Uri를 제공 하 고 `IImageUpdated` 이미지를 로드할 때 호출 되는 인터페이스의 인스턴스를 제공 해야 합니다.

예를 들어 다음 코드는 Url에서로 이미지를 로드 합니다 `BadgeElement` .
...
Linq는 LINQ 및 c #의 초기화 구문을 사용 하 여 요소 계층 구조를 만드는 데 사용할 수 있습니다. 예를 들어 다음 코드는 일부 문자열 배열에서 화면을 만들고 각각에 전달 되는 익명 함수를 통해 셀 선택을 처리 합니다 `StringElement` .
...
요소에 가변 크기를 사용할 수 있는 경우 `IElementSizing` 하나의 메서드를 포함 하는 인터페이스를 구현 해야 합니다.
...
`GetCell`반환 된 셀을 호출 하 고 사용자 지정 하 여 메서드를 구현 하려는 경우에는 `base.GetCell(tv)` `CellKey` 다음과 같이 요소에 고유한 키를 반환 하도록 속성을 재정의 해야 합니다.
...
이는 대부분의 요소에 대해 작동 하지만 및의 경우에는 작동 하지 않으며 `StringElement` `StyledStringElement` 다양 한 렌더링 시나리오에 고유한 키 집합을 사용 합니다. 이러한 클래스에서 코드를 복제 해야 합니다.

### <a name="dialogviewcontrollers-dvcs"></a>DialogViewControllers (DVCs)

Reflection과 Elements API는 모두 동일한를 사용 `DialogViewController` 합니다. 보기의 모양을 사용자 지정 하려는 경우 또는 `UITableViewController` ui의 기본 생성을 벗어나는의 일부 기능을 사용 하려는 경우가 있습니다.

는 `DialogViewController` 단순히의 서브 클래스 `UITableViewController` 이며,를 사용자 지정 하는 것과 같은 방법으로 사용자 지정할 수 있습니다 `UITableViewController` .

예를 들어 목록 스타일을 또는로 변경 하려는 경우 `Grouped` `Plain` 다음과 같이 컨트롤러를 만들 때 속성을 변경 하 여이 값을 설정할 수 있습니다.
...
배경을 설정 하는 등의 고급 사용자 지정 `DialogViewController` 작업을 수행 하려면 아래 예제에 표시 된 것 처럼 해당 메서드를 하위 클래스로 지정 하 고 적절 한 메서드를 재정의 합니다.
...
이 메서드는 셀이 균일 하 게 크기가 지정 된 경우의 서브 클래스를 반환 `DialogViewController.Source` 하거나 `DialogViewController.SizingSource` 셀이 불균형 하지 않은 경우의 서브 클래스를 반환 해야 합니다.

이 재정의를 사용 하 여 메서드를 캡처할 수 있습니다 `UITableViewSource` . 예를 들어 [TweetStation](https://github.com/migueldeicaza/TweetStation) 는이를 사용 하 여 사용자가 위쪽으로 스크롤 한 시기를 추적 하 고 읽지 않은 트 윗의 수를 적절 하 게 업데이트 합니다.
...
사용자에 게 오류를 알리는 방법은 응용 프로그램 마다 다릅니다. 를 팝 `UIAlertView` 하거나 힌트를 표시할 수 있습니다.
