...
- **C #을 사용 하** 는 경우 코드를 사용 하 여 프로그래밍 방식으로 컨트롤을 생성 하 고 뷰 계층 구조에 추가할 수도 있습니다.

IOS 프로젝트를 마우스 오른쪽 단추로 클릭 하 고 **새 파일 > 추가**...를 선택 하 여 새 스토리 보드 및 XIB 파일을 추가할 수 있습니다.

어떤 방법을 사용 하 든 컨트롤 속성 및 이벤트는 응용 프로그램 논리에서 c #을 사용 하 여 계속 조작할 수 있습니다.
...
C #을 사용 하 여 프로그래밍 방식으로 사용자 인터페이스 개체를 만들도록 결정 한 경우 (예: 보기 또는 보기 컨트롤러에서) 다음 단계를 수행 합니다.

- 사용자 인터페이스 개체에 대 한 클래스 수준 필드를 선언 합니다. 예를 들어 컨트롤 자체를 한 번 만듭니다 `ViewDidLoad` . 그런 다음 뷰 컨트롤러의 수명 주기 메서드 (예:
`ViewWillAppear`).
- `CGRect`컨트롤의 프레임 (너비와 높이 뿐만 아니라 화면에서 X 및 Y 좌표)을 정의 하는을 만듭니다. `using CoreGraphics`이에 대 한 지시문이 있는지 확인 해야 합니다.
- 생성자를 호출 하 여 컨트롤을 만들고 할당 합니다.
- 속성 또는 이벤트 처리기를 설정 합니다.
- `Add()`를 호출 하 여 뷰 계층 구조에 컨트롤을 추가 합니다.

다음은 `UILabel` c #을 사용 하 여 뷰 컨트롤러에서를 만드는 간단한 예입니다.
...
## <a name="using-c-and-storyboards"></a>C # 및 Storyboard 사용
...
는 `ControlsViewController.designer.cs` partial 클래스를 포함 하는 생성 된 코드입니다. Mac용 Visual Studio의 디자인 화면에서 컨트롤의 이름을 설정 하거나 Xcode에서 콘센트가 나 작업을 만들면 designer.cs (디자이너) 파일에 해당 속성 또는 부분 메서드가 추가 됩니다. 아래 코드에서는 두 개의 단추와 텍스트 뷰에 대해 생성 된 코드의 예제를 보여 줍니다. 여기에는 단추 중 하나에도 `TouchUpInside` 이벤트가 있습니다.
...
파일을 수동으로 편집 하면 안 됩니다 `designer.cs` . IDE (Mac용 Visual Studio 또는 Visual Studio)는 스토리 보드와 동기화 된 상태를 유지 해야 합니다.

사용자 인터페이스 개체가 또는에 프로그래밍 방식으로 추가 되는 경우 `View` `ViewController` 개체 참조를 직접 인스턴스화하고 관리 하므로 디자이너 파일이 필요 하지 않습니다.
