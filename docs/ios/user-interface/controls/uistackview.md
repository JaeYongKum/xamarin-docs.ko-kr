...
[![스택 뷰 레이아웃 다이어그램](uistackview-images/stacked01.png)](uistackview-images/stacked01.png#lightbox)

Xamarin.ios 앱에서를 사용 하는 경우 `UIStackView` 개발자는 IOS 디자이너의 스토리 보드 내에서 또는 c # 코드에서 하위 뷰를 추가 및 제거 하 여 하위 뷰을 정의할 수 있습니다.
...
컨트롤에 대 한 간략 한 소개로 `UIStackView` , 사용자가 1 ~ 5의 등급을 입력할 수 있는 간단한 인터페이스를 만들게 됩니다. 두 개의 스택 뷰를 사용 합니다. 하나는 장치 화면에서 수직으로 인터페이스를 정렬 하는 것이 고 다른 하나는 화면 전체에서 1-5 등급 아이콘을 가로로 정렬 하는 것입니다.
...
[![뷰 컨트롤러에서 단일 세로 스택 뷰를 끌어 옵니다.](uistackview-images/quick01.png)](uistackview-images/quick01.png#lightbox)
...
[![스택 뷰 UI 레이아웃](uistackview-images/quick03.png)](uistackview-images/quick03.png#lightbox)

다음 옵션을 사용 하 여 가로 스택 보기를 구성 합니다.

[![가로 스택 보기 옵션 구성](uistackview-images/quick04.png)](uistackview-images/quick04.png#lightbox)
...
[![스택 뷰 콘센트 및 작업](uistackview-images/quick05.png)](uistackview-images/quick05.png#lightbox)
...
[![샘플 앱 실행](uistackview-images/intro01.png)](uistackview-images/intro01.png#lightbox)
...
이제 컨트롤의 정의와 작동 방식을 전반적으로 파악 했으므로 `UIStackView` 몇 가지 기능 및 세부 정보를 자세히 살펴보겠습니다.
...
`BaselineRelativeArrangement`속성이 인 경우 `true` **위쪽**, 아래쪽 또는 **가운데**Y 위치를 사용 하는 대신 첫 번째 또는 마지막 **하위** 뷰의 기준선을 기준으로 하위 뷰을 배치 합니다 -  **Y** . 이러한 항목은 스택 뷰의 콘텐츠에서 다음과 같이 계산 됩니다.
...
스택 뷰에서는 `ArrangedSubviews` 속성이 항상 `Subviews` 다음 규칙을 사용 하 여 해당 속성의 하위 집합이 되도록 합니다.

- 하위 뷰가 컬렉션에 추가 되는 경우 컬렉션에 `ArrangedSubviews` 자동으로 추가 됩니다 `Subviews` (이미 해당 컬렉션의 일부가 아닌 경우).
- 하위 뷰가 컬렉션에서 제거 되 면 `Subviews` (표시에서 제거 됨) 컬렉션 에서도 제거 됩니다 `ArrangedSubviews` .
- 컬렉션에서 하위 뷰를 제거 `ArrangedSubviews` 해도 컬렉션에서 하위 뷰가 제거 되지는 않습니다 `Subviews` . 따라서 더 이상 스택 뷰로 레이아웃 되지 않지만 화면에 계속 표시 됩니다.

`ArrangedSubviews`컬렉션은 항상 컬렉션의 하위 집합 `Subview` 이지만 각 컬렉션 내의 개별 하위 뷰 순서는 별개 이며 다음에 의해 제어 됩니다.

- 컬렉션 내에서 하위 뷰의 순서는 `ArrangedSubviews` 스택 내에서의 표시 순서를 결정 합니다.
- 컬렉션 내에서 하위 뷰의 순서는 `Subview` 뷰 내에서 Z 순서 (또는 레이어)를 맨 앞으로 다시 결정 합니다.
...
이 문서에서는 `UIStackView` xamarin.ios 앱에서 가로 또는 세로로 정렬 된 스택에서 하위 뷰 집합을 관리 하기 위해 새로운 컨트롤인 (iOS 9의 경우)에 대해 살펴보았습니다.
