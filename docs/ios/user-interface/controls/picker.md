...
문서에서는 및 컨트롤을 구현 하 고 사용 하는 방법을 설명 합니다 `UIPickerView` `UIDatePicker` .

## <a name="uipickerview"></a>UIPickerView

### <a name="implementing-a-picker"></a>선택 구현

새를 인스턴스화하여 선택기를 구현 합니다 `UIPickerView` .
...
[`UIPickerViewModel`](xref:UIKit.UIPickerViewModel)기본 클래스는 두 개의 인터페이스를 구현 합니다.[`IUIPickerDataSource`](xref:UIKit.IUIPickerViewDataSource)
및는 [`IUIPickerViewDelegate`](xref:UIKit.IUIPickerViewDelegate) 선택기의 데이터를 지정 하는 다양 한 메서드와 상호 작용을 처리 하는 방법을 선언 합니다.
...
클래스 또는의 [`GetView`](xref:UIKit.UIPickerViewModel.GetView(UIKit.UIPickerView,System.nint,System.nint,UIKit.UIView)) 및 메서드를 재정의 [`GetRowHeight`](xref:UIKit.UIPickerViewModel.GetRowHeight(UIKit.UIPickerView,System.nint)) `UIPickerViewModel` 합니다.

## <a name="uidatepicker"></a>UIDatePicker

### <a name="implementing-a-date-picker"></a>날짜 선택 구현

를 인스턴스화하여 날짜 선택을 구현 합니다 `UIDatePicker` .
...
[`MinimumDate`](xref:UIKit.UIDatePicker.MinimumDate)[`MaximumDate`](xref:UIKit.UIDatePicker.MaximumDate)날짜 선택에서 사용할 수 있는 날짜 범위를 제한 합니다. 예를 들어 다음 코드는 날짜 선택을 현재 시간 까지의 60 년으로 제한 합니다.
...
> [!TIP]
> 을로 명시적으로 캐스팅할 수 있습니다 `DateTime` `NSDate` .
>
> ```csharp
...
선택기의 순서는 기본적으로 시스템 로캘을 사용 하는 날짜 선택의 로캘에 따라 달라 집니다. 위의 이미지는 로캘의 선택기 레이아웃을 보여 `en_US` 주지만, 다음은이 순서를 Day로 변경 합니다. | 월 | 연도가
...
와 마찬가지로 [`UIDatePickerMode.Date`](#uidatepickermodedate) 선택기의 순서와 12 또는 24 시간 시계를 사용 하는 것은 날짜 선택의 로캘에 따라 다릅니다.

> [!TIP]
> `Date`모드, 또는에서 날짜 선택기의 값을 캡처하려면 속성을 사용 `UIDatePickerMode.Time` 합니다 `UIDatePickerMode.Date` `UIDatePickerMode.DateAndTime` . 이 값은로 저장 됩니다 `NSDate` .
...
`CountDownDuration`속성은 모드에서 날짜 선택기의 값을 캡처합니다 `UIDatePickerMode.CountDownTimer` . 예를 들어 현재 날짜에 카운트다운 값을 추가 하려면 다음을 수행 합니다.
...
형식을 지정 하려면를 `NSDate` 사용 [`NSDateFormatter`](xref:Foundation.NSDateFormatter) 합니다.

를 사용 하려면 `NSDateFormatter` 해당 메서드를 호출 [`ToString`](xref:Foundation.NSDateFormatter.ToString(Foundation.NSDate)) 합니다. 다음은 그 예입니다.
...
[`DateFormat`](xref:Foundation.NSDateFormatter.DateFormat)의 속성 (문자열)은 `NSDateFormatter` 사용자 지정 가능한 날짜 형식 지정을 허용 합니다.
...
[`TimeStyle`](xref:Foundation.NSDateFormatter.TimeStyle)속성 (의는 [`NSDateFormatterStyle`](xref:Foundation.NSDateFormatterStyle) `NSDateFormatter` 미리 결정 된 스타일을 기준으로 시간 형식을 지정 합니다.
...
[`DateStyle`](xref:Foundation.NSDateFormatter.DateStyle)의 속성 ( `NSDateFormatterStyle` )은 `NSDateFormatter` 미리 결정 된 스타일을 기준으로 날짜 형식을 지정 합니다.
...
- `NSDateFormatterStyle.Full`: 2017 년 8 월 2 일 수요일, 7:48 PM
...
> [!NOTE]
> `DateFormat`및 `DateStyle` / `TimeStyle` 는 날짜 및 시간 형식을 지정 하는 다양 한 방법을 제공 합니다. 가장 최근에 설정 된 속성은 날짜 포맷터의 출력을 결정 합니다.

## <a name="related-links"></a>관련 링크

- [PickerControl (샘플)](/samples/xamarin/ios-samples/pickercontrol)