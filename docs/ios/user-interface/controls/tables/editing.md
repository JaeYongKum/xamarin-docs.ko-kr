...
서브 클래스의 메서드를 재정의 하 여 테이블 편집 기능을 사용할 수 있습니다 `UITableViewSource` . 가장 간단한 편집 동작은 단일 메서드 재정의를 사용 하 여 구현할 수 있는 살짝 밀기-삭제 제스처입니다.
...
삭제로 살짝 밀기 기능은 사용자가 필요로 하는 iOS에서 자연 스러운 제스처입니다. 
 ...
 [![삭제할 살짝 밀기의 예](editing-images/image10.png)](editing-images/image10.png#lightbox)
...
이 예제에서는 `UITableViewSource` `List<TableItem>` 컬렉션에서 항목을 추가 및 삭제 하는 것을 지원 하기 때문에 문자열 배열 대신를 데이터 소스로 사용 하도록 업데이트 되었습니다.
...
**Tableeditmode** 샘플은 다음과 같이 이러한 기능을 구현 합니다.
 ...
 [![TableEditMode 샘플은 다음과 같이 이러한 기능을 구현 합니다.](editing-images/image11.png)](editing-images/image11.png#lightbox)

에는 `UITableViewSource` 테이블의 편집 모드 동작에 영향을 주는 여러 가지 방법이 있습니다.
...
- **EditingStyleForRow** – 테이블이 편집 모드에 있는 경우이 메서드의 반환 값은 셀에 빨간색 삭제 아이콘이 표시 되는지 아니면 녹색 추가 아이콘이 표시 되는지를 결정 합니다. `UITableViewCellEditingStyle.None`행을 편집할 수 없으면를 반환 합니다. 
...
`MoveRow`구현은 새 순서와 일치 하도록 기본 데이터 구조를 변경 해야 하기 때문에 좀 더 복잡 합니다. 데이터는로 구현 되기 때문에 `List` 아래 코드는 이전 위치에 있는 데이터 항목을 삭제 하 고 새 위치에 삽입 합니다. 데이터가 ' order ' 열 (예:)을 사용 하 여 SQLite 데이터베이스 테이블에 저장 된 경우이 메서드는 대신 해당 열의 숫자를 다시 정렬 하기 위해 일부 SQL 작업을 수행 해야 합니다.
...
마지막으로, 테이블을 편집 모드로 가져오려면 **편집** 단추가 다음과 같이 호출 해야 합니다. `SetEditing`
...
테이블 내에서 행 삽입은 일반적이 지 않은 사용자 인터페이스입니다. 표준 iOS 앱의 주요 예는 **연락처 편집** 화면입니다. 이 스크린샷에서는 행 삽입 기능의 작동 방식을 보여 줍니다. 편집 모드에서 데이터에 추가 행을 삽입 하는 추가 행이 있습니다. 편집이 완료 되 면 임시 **(새 추가)** 행이 제거 됩니다.
 ...
 [![편집이 완료 되 면 임시 새 행 추가가 제거 됩니다.](editing-images/image12.png)](editing-images/image12.png#lightbox)
...
이 행 삽입 UI 패턴은 자주 사용 되지 않지만 `UITableView.BeginUpdates` 및 메서드를 사용 `EndUpdates` 하 여 테이블의 셀 삽입 또는 제거에 애니메이션 효과를 줄 수도 있습니다. 이러한 메서드를 사용 하는 규칙은 및 호출 사이에서 반환 되는 값의 차이가 `RowsInSection` `BeginUpdates` `EndUpdates` 및 메서드와 함께 추가/삭제 된 셀의 순 수와 일치 해야 한다는 `InsertRows` 것입니다 `DeleteRows` . 기본 데이터 원본이 테이블 뷰의 삽입/삭제와 일치 하도록 변경 되지 않은 경우 오류가 발생 합니다.
