...
- Mono **프레임 워크** – Mono 4.3 ProtectedData는 mono 4.2에서 데이터 암호를 해독 하지 못합니다. 따라서 `Data unprotection failed` 보호 된 nuget 원본을 구성할 때 오류와 함께 nuget 패키지를 복원 하지 못합니다.
  - **해결 방법** – Mac용 Visual Studio에서 암호 인증을 사용 하는 모든 NuGet 패키지 소스를 다시 추가 해야 패키지 복원을 다시 시도할 수 있습니다.
- **Mac용 Visual Studio w/f # 추가 기능** – Windows에서 f # Android 템플릿을 만들 때 발생 하는 오류입니다. 이는 Mac에서 제대로 작동 해야 합니다.
- **Xamarin.ios** – 대상 프레임 워크가로 설정 된 xamarin.ios 통합 템플릿 프로젝트를 실행 하는 경우 `Unsupported` 팝업이 `Could not connect to the debugger` 표시 될 수 있습니다.
...
`size`나열 된 예제에서 두 라이브러리 간의 차이점은 위에서 실행 됩니다. `ENABLE_BITCODE`이 크기 문제에 대 한 해결 방법으로 bitcode enabled (Xcode 설정)를 사용 하 여 Xcode archive 빌드에서 라이브러리를 생성 해야 합니다.
...
2. `Assets.car`최종 응용 프로그램 번들의 [아이콘 및 이미지 작업](~/ios/tvos/app-fundamentals/icons-images.md) 설명서에서 파일을 포함 했는지 확인 합니다.
...
이 솔루션은 Siri 원격 ()에 대 한 지원을 `GCMicroGamepad` 앱의 파일에 추가 하는 것입니다 `Info.plist` . Siri 원격을 대상으로 하는 마이크로 게임 컨트롤러 프로필이 Apple에서 추가 되었습니다. 예를 들어 다음 키를 포함 합니다.
...
이 문제를 해결 하려면 다음 내용으로 이라는 XML 파일을 추가 `Xamarin.TVOS.xml` 합니다.
