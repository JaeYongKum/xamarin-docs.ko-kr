...
이 가이드의 모든 태스크는 부록 샘플 응용 프로그램에서 사용할 수 있습니다.
 ...
 [![함께 제공 되는 샘플 응용 프로그램 화면](eventkit-images/01.png)](eventkit-images/01.png#lightbox)
...
EventKit를 사용 하는 경우 공용 클래스 및 사용을 이해 하는 것이 중요 합니다. 이러한 클래스는 모두 및에서 찾을 수 있습니다 `EventKit` `EventKitUI` `EKEventEditController` .
...
`EventStore`는 데이터베이스 엔진과 비슷하지만 응용 프로그램 인스턴스의 수명 동안 가능한 한 거의 필요 하지 않은 상태로 생성 되 고 제거 되어야 한다는 것을 의미 합니다. 실제로 응용 프로그램에서 하나의 인스턴스를 만든 후에는 `EventStore` 다시 필요 하지 않을 경우 응용 프로그램의 전체 수명 동안 해당 참조를 유지 하는 것이 좋습니다. 또한 모든 호출은 단일 인스턴스로 이동 해야 합니다 `EventStore` . 따라서 단일 인스턴스를 사용 가능 하 게 유지 하기 위해 Singleton 패턴을 사용 하는 것이 좋습니다.

#### <a name="creating-an-event-store"></a>이벤트 저장소 만들기

다음 코드는 클래스의 단일 인스턴스를 만들고 `EventStore` 응용 프로그램 내에서 정적으로 사용할 수 있도록 하는 효율적인 방법을 보여 줍니다.
...
위의 코드는 Singleton 패턴을 사용 하 여 `EventStore` 응용 프로그램이 로드 될 때의 인스턴스를 인스턴스화합니다. `EventStore`그러면 응용 프로그램 내에서 다음과 같이 전역으로에 액세스할 수 있습니다.

```csharp
App.Current.EventStore;
```

여기의 모든 예제에서는이 패턴을 사용 하므로 via를 참조 `EventStore` `App.Current.EventStore` 합니다.

#### <a name="requesting-access-to-calendar-and-reminder-data"></a>일정 및 미리 알림 데이터에 대 한 액세스 요청

응용 프로그램은 EventStore를 통해 데이터에 액세스 하도록 허용 하기 전에 먼저 필요한 항목에 따라 일정 이벤트 데이터 또는 미리 알림 데이터에 대 한 액세스를 요청 해야 합니다. 이를 용이 하 게 하기 위해은 전달 된에 `EventStore` `RequestAccess` 따라 응용 프로그램이 일정 데이터 또는 미리 알림 데이터에 대 한 액세스를 요청 하 고 있음을 알리는 경고 보기를 사용자에 게 표시 하는 라는 메서드를 노출 합니다 `EKEntityType` . 호출은 경고 보기를 발생 시키므로 비동기 호출이 고, 두 개의 매개 변수를 받는 (또는 람다)로 전달 되는 완료 처리기를 호출 합니다 `NSAction` . 여기에는 액세스 권한이 부여 되었는지 여부에 대 한 부울과 `NSError` null이 아닌 경우 요청에 오류 정보가 포함 됩니다. 예를 들어 다음 코딩 된는 일정 이벤트 데이터에 대 한 액세스를 요청 하 고 요청이 부여 되지 않은 경우 경고 보기를 표시 합니다.
...
또한 완료 처리기는 개별 (비 UI) 스레드에서 호출 되기 때문에 완료 처리기의 UI에 대 한 모든 업데이트는를 통해 호출 해야 합니다 `InvokeOnMainThread` . 그렇지 않으면 예외가 throw 되 고 catch 되지 않은 경우 응용 프로그램의 작동이 중단 됩니다.
 ...
 *EKCalendar* 은 달력 이벤트의 그룹을 포함 하는 달력을 나타냅니다. 일정은 로컬, *iCloud*, *Exchange Server* , *Google*등의 타사 공급자 위치에 저장 하는 등의 다양 한 위치에 저장 될 수 있습니다. 여러 번 `EKCalendar` 사용 하 여 `EventKit` 이벤트를 검색할 위치 또는 저장 위치를 확인할 수 있습니다.

### <a name="ekeventeditcontroller"></a>EKEventEditController

 *EKEventEditController* 는 네임 스페이스에서 찾을 수 있으며 `EventKitUI` , 달력 이벤트를 편집 하거나 만드는 데 사용할 수 있는 기본 제공 컨트롤러입니다. 기본 제공 카메라 컨트롤러와 마찬가지로 UI를 `EKEventEditController` 표시 하 고 저장을 처리 하는 데 많은 도움이 됩니다.

### <a name="ekevent"></a>EKEvent

 *Ekevent* 는 달력 이벤트를 나타냅니다. 및는 모두 `EKEvent` `EKReminder` 에서 상속 되며, 등의 필드를 `EKCalendarItem` 포함 `Title` `Notes` 합니다.
...
사용자가 장치에 구성한 일정을 열거 하려면에서를 호출 하 `GetCalendars` `EventStore` 고 받으려는 일정 유형 (미리 알림 또는 이벤트)을 전달 합니다.
...
일정 응용 프로그램을 사용할 때 사용자에 게 표시 되는 것과 동일한 UI를 사용 하 여 이벤트를 만들거나 편집 하려는 경우 *EKEventEditViewController* 는 많은 작업을 수행 합니다.
 ...
 [![Calendar 응용 프로그램을 사용할 때 사용자에 게 표시 되는 UI입니다.](eventkit-images/02.png)](eventkit-images/02.png#lightbox)
...
그런 다음 시작 하려면이를 인스턴스화하고,에 대 한 참조를 제공 하 `EventStore` 고, *EKEventEditViewDelegate* 대리자를 연결 하 고, 다음을 사용 하 여 표시 합니다 `PresentViewController` .
...
대리자는 메서드를 재정의 해야 합니다 `Completed` .이 메서드는 사용자가 대화 상자에서 작업을 완료할 때 컨트롤러에서 호출 됩니다.
...
필요에 따라 대리자에서 메서드의 *작업* 을 확인 `Completed` 하 여 이벤트를 수정 하 고 다시 저장 하거나 취소 된 경우 다른 작업을 수행할 수 있습니다. 등:
...
코드에서 이벤트를 만들려면 클래스에서 *Fromstore* 팩터리 메서드를 사용 하 `EKEvent` 고 데이터를 설정 합니다.
...
이벤트를 저장 하려면에서 *SaveEvent* 메서드를 호출 합니다 `EventStore` .
...
저장 하려면에서 *SaveReminder* 메서드를 호출 합니다 `EventStore` .
...
ID로 이벤트를 검색 하려면에서 *EventFromIdentifier* 메서드를 사용 하 여 `EventStore` 이벤트에서 끌어온를 전달 합니다 `EventIdentifier` .

```csharp
EKEvent mySavedEvent = App.Current.EventStore.EventFromIdentifier ( newEvent.EventIdentifier );
```

이벤트의 경우 두 가지 다른 식별자 속성이 있지만이 속성은 `EventIdentifier` 이에 대해 작동 하는 유일한 속성입니다.

### <a name="retrieving-a-reminder-by-id"></a>ID로 미리 알림 검색

미리 알림을 검색 하려면에서 *Getcalendaritem* 메서드를 사용 하 `EventStore` 고 *calendaritemidentifier*를 전달 합니다.

```csharp
EKCalendarItem myReminder = App.Current.EventStore.GetCalendarItem ( reminder.CalendarItemIdentifier );
```

`GetCalendarItem`는를 반환 하므로 `EKCalendarItem` `EKReminder` 미리 알림 데이터에 액세스 하거나 인스턴스를 나중에 사용 해야 하는 경우로 캐스팅 해야 합니다 `EKReminder` .

`GetCalendarItem`작성 시와 마찬가지로 달력 이벤트에는 사용 하지 마세요.

### <a name="deleting-an-event"></a>이벤트 삭제

일정 이벤트를 삭제 하려면에서 *Removeevent* 를 호출 하 `EventStore` 고 이벤트에 대 한 참조와 적절 한를 전달 합니다 `EKSpan` .
...
그러나 이벤트가 삭제 된 후에는 이벤트 참조가가 됩니다 `null` .

### <a name="deleting-a-reminder"></a>미리 알림 삭제

미리 알림을 삭제 하려면에서 *RemoveReminder* 를 호출 하 `EventStore` 고 미리 알림에 대 한 참조를 전달 합니다.
...
위의 코드에서는를 `EKReminder` `GetCalendarItem` 검색 하는 데 사용 되기 때문에로 캐스팅 되었습니다.

### <a name="searching-for-events"></a>이벤트 검색

일정 이벤트를 검색 하려면에서 *PredicateForEvents* 메서드를 통해 *NSPredicate* 개체를 만들어야 합니다 `EventStore` . 는 `NSPredicate` iOS에서 일치 항목을 찾는 데 사용 하는 쿼리 데이터 개체입니다.
...
을 만든 후에 `NSPredicate` 는에서 *EventsMatching* 메서드를 사용 합니다 `EventStore` .
...
이 문서는 EventKit 프레임 워크의 중요 한 부분과 가장 일반적인 몇 가지 작업에 대 한 개요를 제공 했습니다. 그러나 EventKit 프레임 워크는 매우 크고 강력 하며 여기에 소개 되지 않은 기능 (예: batch 업데이트, 경보 구성, 이벤트에 대 한 되풀이 구성, 일정 데이터베이스의 변경 내용 등록 및 수신 대기, 지역 구분 설정 등)을 포함 합니다.  자세한 내용은 Apple의 [일정 및 미리 알림 프로그래밍 가이드](https://developer.apple.com/library/prerelease/ios/#documentation/DataManagement/Conceptual/EventKitProgGuide/Introduction/Introduction.html)를 참조 하세요.
