...
iOS 10은 시스템이 적절 한 시간에 자동으로 유용한 정보를 사용자에 게 자동으로 제공할 수 있도록 하 여 앱에 대 한 참여를 유도 하는 새로운 방법을 제공 합니다. IOS 9에서 스포트라이트, 전달 및 Siri 제안을 사용 하 여 앱에 심층 검색을 추가 하는 기능을 제공 하는 것 처럼 ( [새 검색 api](~/ios/platform/search/index.md)참조) 앱은 다음 위치에서 시스템에 의해 사용자에 게 제공할 수 있는 기능을 노출할 수 있습니다.
...
앱은 `NSUserActivity` , 웹 태그, 핵심 스포트라이트, MapKit, Media Player 및 UIKit와 같은 기술의 컬렉션을 사용 하 여 시스템에이 기능을 노출 합니다. 또한 앱에 대 한 사전 제안 지원을 제공 하 여 더 심층적인 Siri 통합을 무료로 제공 합니다.

## <a name="location-based-suggestions"></a>위치 기반 제안

IOS 10의 새로운 기능으로, 클래스에는 `NSUserActivity` `MapItem` 개발자가 다른 컨텍스트에서 사용할 수 있는 위치 정보를 제공할 수 있는 속성이 포함 되어 있습니다. 예를 들어 앱에서 식당 리뷰를 표시 하는 경우 개발자는 `MapItem` 사용자가 앱에서 보고 있는 식당의 위치로 속성을 설정할 수 있습니다. 사용자가 Maps 앱으로 전환 하는 경우 식당의 위치를 자동으로 사용할 수 있습니다.

앱이 앱 검색을 지 원하는 경우 클래스의 새 주소 구성 요소를 사용 `CSSearchableItemAttributesSet` 하 여 사용자가 방문할 수 있는 위치를 지정할 수 있습니다. `MapItem`속성을 설정 하면 다른 속성이 자동으로 채워집니다.

주소 구성 요소 속성의 및를 설정 하는 것 외에도 `Latitude` `Longitude` 앱에서 및 속성을 제공 하는 것이 좋습니다 `NamedLocation` `PhoneNumbers` . 그러면 siri가 해당 위치에 대 한 호출을 시작할 수 있습니다.

## <a name="web-markup-based-suggestions"></a>웹 태그 기반 제안

iOS 9는 사용자에 게 스포트라이트 및 Safari 검색 결과에 표시 되는 콘텐츠를 강화 웹 사이트에 구조화 된 데이터 태그를 포함 하는 기능을 추가 했습니다 ( [웹 마크업으로 검색](~/ios/platform/search/web-markup.md)참조). iOS 10은 사용자 환경을 더욱 향상 시키기 위해 위치 기반 태그 (예: [Schema.org](https://schema.org/)에 정의 된 [PostalAddress](https://schema.org/PostalAddress) )를 포함 하는 기능을 추가 합니다. 예를 들어 사용자가 웹 사이트에서 페이지가 표시 된 위치를 보는 경우 맵을 열 때 시스템에서 동일한 위치를 제안할 수 있습니다.
...
예를 들어 사용자가 표시 된 텍스트 필드에 텍스트를 입력 하는 경우 `UITextContentType.FullStreetAddress` 시스템은 사용자가 최근에 보고 있던 위치로 필드를 자동으로 채우는 것을 제안할 수 있습니다.
...
[MKDirectionsRequest](xref:MapKit.MKDirectionsRequest) API를 사용 하는 사용자가 공유 하는 앱이 있는 경우, iOS 10은 사용자가 원하는 경우에 앱 전환기에서 옵션으로 표시 합니다. 또한 해당 `MKDirectionsModeRideShare` 파일에 [MKDirectionsApplicationSupportedModes](https://developer.apple.com/library/content/documentation/General/Reference/InfoPlistKeyReference/Articles/iPhoneOSKeys.html) 키에 대 한를 지정 하 여 해당 앱을 타는 공유 앱으로 등록 해야 합니다 `Info.plist` .
...
- ** `NSUserActivity` 및 Schema.org**  -  `NSUserActivity` 시스템에서 사용자가 현재 화면에서 작업 중인 정보를 이해 하는 데 도움이 됩니다. Schema.org는 웹 페이지에 비슷한 기능을 추가 합니다.
...
- **핸드 오프**  -  `NSUserActivity` 을 (를) 지원 하기 위해 iOS 8에서 추가 되었습니다 .이를 통해 개발자는 한 장치에서 작업을 시작 하 고 다른 장치에서 작업을 계속할 수 있습니다 ( [핸드 오프 소개](~/ios/platform/handoff.md)참조).
- **스포트라이트 검색** -iOS 9는를 사용 하 여 스포트라이트 검색 결과 내에서 앱 콘텐츠를 승격 하는 기능을 추가 했습니다 `NSUserActivity` ( [Core 스포트라이트를 사용 하 여 검색](~/ios/platform/search/corespotlight.md)참조).
- IOS 10의 **상황별 Siri 미리 알림** - `NSUserActivity` siri가 나중에 앱에서 현재 보고 있는 콘텐츠를 확인 하는 미리 알림을 신속 하 게 만들 수 있도록 확장 되었습니다.
- **위치 제안** -iOS 10은 `NSUserActivity` 앱 내에서 볼 수 있는 캡처 위치를 개선 하 고 시스템 전체의 여러 위치에서 승격 합니다.
- **컨텍스트 Siri 요청**  -  `NSUserActivity` 앱 내에서 Siri에 게 제공 되는 정보에 대 한 컨텍스트를 제공 하 여 사용자가 응용 프로그램 내에서 Siri를 호출 하는 방향을 가져오거나 호출을 수행할 수 있도록 합니다.
- **연락처 상호 작용** -iOS 10의 새로운 기능으로, `NSUserActivity` 통신 앱은 연락처 카드 (연락처 앱)에서 대체 통신 방법으로 승격할 수 있습니다.
...
1. 사용자가 앱에서 작업할 때 `NSUserActivity` 나중에 앱의 상태를 다시 만들기 위해가 만들어집니다.
...
[![NSUserActivity 세부 정보](proactive-suggestions-images/activity03.png)](proactive-suggestions-images/activity03.png#lightbox)

여기서 앱은를 만들고 `NSUserActivity` 나중에 상태를 다시 만들기 위한 정보로 채워져 있습니다. 앱에는 위치 이름 및 주소와 같은 메타 데이터도 포함 되어 있습니다. 이 활동을 만든 후 앱에서 iOS에서 사용자의 현재 상태를 나타내는지 알 수 있습니다.
...
활동을 만들기 전에 활동 유형 식별자를 생성 하 여 해당 활동을 식별 해야 합니다. 활동 유형 식별자는 `NSUserActivityTypes` `Info.plist` 지정 된 사용자 활동 유형을 고유 하 게 식별 하는 데 사용 되는 앱 파일의 배열에 추가 되는 짧은 문자열입니다. 앱이 지원 하 고 앱 검색에 노출 하는 각 작업에 대해 배열에 하나의 항목이 있습니다. 자세한 내용은 [활동 형식 식별자 참조 만들기](~/ios/platform/search/nsuseractivity.md) 를 참조 하세요.
...
개발자가 위에서 만든 활동과 동일한 활동 유형 식별자 () 인지 확인 해야 합니다 `com.xamarin.platform` . 앱은에 저장 된 정보를 사용 하 여 `NSUserActivity` 사용자가 남은 위치로 상태를 다시 복원 합니다.
...
위의 식당 검색 앱 예제를 예로 들어 보겠습니다. `NSUserActivity`메타 데이터와 특성을 모두 구현 하 고 올바르게 채우면 사용자가 다음을 수행할 수 있습니다.
...
5. 이는 타사 앱 (지원 되는)에도 적용 `NSUserActivity` 되므로 사용자가 자동으로 실행 되는 앱으로 전환할 수 있으며 식당의 주소도 자동으로 대상으로 제안 됩니다.
...
앱이 MapKit를 사용 하는 경우 현재 지도를 활동에 추가 하는 것 만큼 간단 합니다 `MKMapItem` .
...
담당자는를 사용 하 여 앱에서 구현 되며 `NSUserActivity` iOS 10에 도입 된 새로운 의도 프레임 워크를 사용 합니다. 의도로 작업 하는 방법에 대 한 자세한 내용은 [Sirikit 개념 이해](~/ios/platform/sirikit/understanding-sirikit.md) 및 [Sirikit 구현](~/ios/platform/sirikit/implementing-sirikit.md) 가이드를 참조 하세요.
...
상호 작용이 완전히 채워지면 메서드를 호출 하 여 `DonateInteraction` 상호 작용을 사용할 수 있다는 사실을 시스템에 알립니다.

사용자가 연락처 카드에서 앱과 상호 작용할 때 상호 작용은와 함께 제공 `NSUserActivity` 되며,이는 앱을 시작 하는 데 사용 됩니다.

[![상호 작용은 앱을 시작 하는 데 사용 되는 NSUserActivity와 함께 제공 됩니다.](proactive-suggestions-images/activity05.png)](proactive-suggestions-images/activity05.png#lightbox)
...
이 코드를 자세히 살펴보면의 인스턴스를 만들고 채웁니다 `NSUserActivity` . 위의 [작업 만들기](#creating-an-activity) 섹션에서 볼 수 있습니다. 그런 다음 `INSendMessageIntent` 에서 상속 되는의 인스턴스를 만들고 `INIntent` 전송 중인 메시지의 세부 정보로 채웁니다.

```csharp
var intent = new INSendMessageIntent (to, text, "", "MonkeyChat", from);
```

`INSendMessageIntentResponse`가 만들어지고 위에서 만든를 전달 합니다 `NSUserActivity` .

```csharp
var response = new INSendMessageIntentResponse (INSendMessageIntentResponseCode.Success, activity);
```

는 `INInteraction` 전송 메시지 의도 ( `INSendMessageIntent` ) 및 응답 ()을 통해 작성 됩니다 `INSendMessageIntentResponse` .
...
- `NeedsSave`지연 페이로드 업데이트에 사용 합니다.
...
위와 같이를 사용 하면 `NSUserActivity` 시스템에서 사용자가 현재 화면에서 작업 하 고 있는 정보를 이해할 수 있습니다. Schema.org는 웹 페이지에 비슷한 기능을 추가 합니다.
...
- 개발자는 웹 사이트에서 구현 하 여 네이티브 앱에서를 사용 하는 몇 가지 이점을 얻을 수 있습니다 `NSUserActivity` .

스키마는 *식당*등의 특정 형식이 *로컬 비즈니스*와 같은 보다 일반적인 형식에서 상속 하는 구조와 같은 트리로 정렬 됩니다. 자세한 내용은 [Schema.org](https://schema.org)를 참조 하세요.
...
다음은 정적 클래스의 개발자가 사용할 수 있는 몇 가지 일반적인 형식입니다 `UITextContentType` .
...
인 경우 `MKDirectionsRequest` URL에서을 만듭니다.
...
개발자는 iOS 10에 간단한 API 호출을 포함 하 여 미디어 앱의 사용자에 게 더욱 매력적인 잠금 화면 환경을 만들 수 있습니다. `MPPlayableContentManager`미디어 재생을 관리 하기 위해 클래스를 사용 하 여 전체 미디어 컨트롤 (음악 앱에서 제공 하는 것과 같은)이 앱의 잠금 화면에 표시 됩니다.
