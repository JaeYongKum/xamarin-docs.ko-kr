...
또한 MonkeyCall VOIP 호출을 받을 때 iPhone의 잠금이 해제 된 경우 기본 제공 Phone 앱의 동일한 전체 화면, 기본 UI 및 표준 살짝 밀기-응답 및 탭-거절 기능이 표시 되 고 MonkeyCall에 사용자 지정 벨 소리를 재생할 수 있는 옵션이 제공 됩니다.

CallKit는 MonkeyCall에 추가 기능을 제공 하 여, 해당 VOIP 호출이 다른 형식의 호출과 상호 작용할 수 있도록 하 고, 기본 제공 최근 및 즐겨찾기 목록에 표시 하 고, 기본 제공 되는 기능 및 차단 기능을 사용 하 고, Siri에서 MonkeyCall 호출을 시작 하 고, 사용자가 연락처 앱의 사용자에 게 MonkeyCall 호출을 할당할 수 있습니다
...
[![CallKit 서비스 스택](callkit-images/callkit01.png)](callkit-images/callkit01.png#lightbox)

위 다이어그램의 MonkeyCall 앱에 대해 자세히 살펴보겠습니다. 앱은 자체 네트워크와 통신 하는 모든 코드를 포함 하며 자체 사용자 인터페이스를 포함 합니다. 다음은 시스템과 통신 하는 CallKit의 링크입니다.

[![MonkeyCall 앱 아키텍처](callkit-images/callkit02.png)](callkit-images/callkit02.png#lightbox)
...
앱은 다음에 대해를 사용 해야 합니다 `CXProvider` .

- 시스템에 대 한 들어오는 호출을 보고 합니다.
- 발신 전화가 시스템에 연결 되어 있음을 보고 합니다.
- 원격 사용자가 시스템에 대 한 호출을 종료 하는 것을 보고 합니다.

앱은 시스템과 통신 하려고 할 때 클래스를 사용 하 `CXCallUpdate` 고 시스템에서 앱과 통신 해야 하는 경우 클래스를 사용 합니다 `CXAction` .

[![CXProvider를 통해 시스템과 통신](callkit-images/callkit03.png)](callkit-images/callkit03.png#lightbox)

### <a name="the-cxcallcontroller"></a>CXCallController

를 `CXCallController` 사용 하면 앱에서 VOIP 통화를 시작 하는 사용자와 같은 로컬 사용자 작업을 시스템에 알릴 수 있습니다. 앱은를 구현 하 여 `CXCallController` 시스템에서 다른 형식의 호출로 상호 작용를 가져옵니다. 예를 들어, 진행 중인 활성 전화 통신 호출이 이미 있는 경우에서 `CXCallController` voip 앱이 해당 호출을 대기 중으로 설정 하 고 voip 호출을 시작 하거나 답변할 수 있습니다.

앱은 다음에 대해를 사용 해야 합니다 `CXCallController` .

- 사용자가 시스템에 대 한 나가는 호출을 시작 했을 때 보고 합니다.
- 사용자가 시스템에 대 한 들어오는 호출에 응답 하는 경우 보고 합니다.
- 사용자가 시스템에 대 한 호출을 종료 하는 경우 보고 합니다.

응용 프로그램은 로컬 사용자 동작을 시스템에 전달 하려는 경우 클래스를 사용 합니다 `CXTransaction` .

[![CXCallController를 사용 하 여 시스템에 보고](callkit-images/callkit04.png)](callkit-images/callkit04.png#lightbox)
...
이는 시뮬레이션 전용 이므로는 `ActiveCallManager` 개체의 컬렉션을 유지 관리 `ActiveCall` 하 고 해당 속성에 의해 지정 된 호출을 찾기 위한 루틴이 있습니다 `UUID` . 또한 나가는 호출의 보류 상태를 시작, 종료 및 변경 하는 메서드도 포함 합니다. 자세한 내용은 아래의 [발신 호출 처리](#handling-outgoing-calls) 섹션을 참조 하십시오.

### <a name="the-providerdelegate-class"></a>ProviderDelegate 클래스

위에서 설명한 것 처럼는 `CXProvider` 대역 외 알림을 위해 앱과 시스템 간의 양방향 통신을 제공 합니다. 개발자는 `CXProviderDelegate` 대역 외 CallKit 이벤트를 처리 하기 위해 사용자 지정을 제공 하 고 `CXProvider` 앱에 대 한를에 연결 해야 합니다. MonkeyCall은 다음을 사용 합니다 `CXProviderDelegate` .
...
이 대리자의 인스턴스를 만들면 `ActiveCallManager` 호출 활동을 처리 하는 데 사용할이 전달 됩니다. 다음으로는가 응답 하는 핸들 형식 ()을 정의 합니다 `CXHandleType` `CXProvider` .
...
이러한 값은를 구성 하는 데 사용 되는에 번들로 제공 `CXProviderConfiguration` 됩니다 `CXProvider` .
...
그런 다음이 대리자는 `CXProvider` 이러한 구성을 사용 하 여 새를 만들어 자신에 게 연결 합니다.
...
또한 메서드를 사용 `DidDeactivateAudioSession` 하 여 시스템 제공 오디오 세션에 대 한 연결을 마무리 하 고 해제 합니다.
...
MonkeyCall은 AppDelegate를 사용 하 여 앱 전체에서 사용 되는 및 인스턴스를 유지 합니다 `ActiveCallManager` `CXProviderDelegate` .
...
[![원격 사용자가 VOIP 대화를 시작 했습니다.](callkit-images/callkit05.png)](callkit-images/callkit05.png#lightbox)

1. 앱은 들어오는 VOIP 호출이 있는 통신 네트워크에서 알림을 가져옵니다.
2. 앱은를 사용 하 여 `CXProvider` `CXCallUpdate` 호출을 알리는를 시스템에 보냅니다.
3. 시스템은 CallKit를 사용 하 여 시스템 UI, 시스템 서비스 및 기타 모든 VOIP 앱에 대 한 호출을 게시 합니다.

예를 들어에서 `CXProviderDelegate` 다음을 수행 합니다.
...
이 코드는 새 인스턴스를 만들고 `CXCallUpdate` 호출자를 식별할 핸들을 여기에 연결 합니다. 그런 다음 클래스의 메서드를 사용 하 여 `ReportNewIncomingCall` `CXProvider` 호출을 시스템에 알립니다. 성공적으로 실행 되 면 앱의 활성 호출 컬렉션에 호출이 추가 됩니다. 그렇지 않으면 사용자에 게 오류를 보고 해야 합니다.

### <a name="user-answering-incoming-call"></a>들어오는 호출에 대 한 사용자 응답

사용자가 들어오는 VOIP 통화에 응답 하려는 경우 다음 작업이 수행 됩니다.

[![사용자가 들어오는 VOIP 통화에 응답 합니다.](callkit-images/callkit06.png)](callkit-images/callkit06.png#lightbox)

1. 시스템 UI는 사용자가 VOIP 호출에 응답 하려고 함을 시스템에 알립니다.
2. 시스템은 `CXAnswerCallAction` 응답 의도를 알려 주는를 앱에 보냅니다 `CXProvider` .
...
이 코드는 먼저 활성 호출 목록에서 지정 된 호출을 검색 합니다. 호출을 찾을 수 없는 경우 시스템에 알림이 표시 되 고 메서드가 종료 됩니다. 이 항목이 발견 되 면 `AnswerCall` 클래스의 메서드를 `ActiveCall` 호출 하 여 호출을 시작 하 고 시스템이 성공 하거나 실패 하면 정보를 제공 합니다.
...
이 코드는 먼저 활성 호출 목록에서 지정 된 호출을 검색 합니다. 호출을 찾을 수 없는 경우 시스템에 알림이 표시 되 고 메서드가 종료 됩니다. 이 항목이 발견 되 면 `EndCall` 클래스의 메서드를 `ActiveCall` 호출 하 여 호출을 종료 하 고 시스템이 성공 하거나 실패 하면 정보를 제공 합니다. 성공 하면 활성 호출의 컬렉션에서 호출이 제거 됩니다.
...
위의 상황에서 시스템은 `CXTransaction` 여러 작업 (예: 및)의 목록을 포함 하는를 앱에 보냅니다 `CXEndCallAction` `CXAnswerCallAction` . 이러한 모든 작업은 개별적으로 수행 해야 하므로 시스템에서 UI를 적절 하 게 업데이트할 수 있습니다.
...
를 만들고이를 사용 하 여 `CXHandle` `CXStartCallAction` `CXTransaction` `RequestTransaction` 클래스의 메서드를 사용 하 여 시스템으로 전송 `CXCallController` 되는에 번들로 제공 되는를 구성 합니다. 시스템은 메서드를 호출 하 여 `RequestTransaction` 새 호출이 시작 되기 전에 원본 (Phone 앱, FaceTime, VOIP 등)에 관계 없이 기존 호출을 보류 중 상태로 둘 수 있습니다.

나가는 VOIP 통화를 시작 하는 요청은 Siri와 같은 여러 다른 원본 (연락처 앱에서 연락처 앱에 있는 항목) 또는 최근 목록 (Phone 앱)에서 가져올 수 있습니다. 이러한 상황에서 앱은 내에서 시작 호출 의도를 보내고 `NSUserActivity` AppDelegate는이를 처리 해야 합니다.
...
여기서 `CallHandleFromActivity` 도우미 클래스의 메서드는 `StartCallRequest` 호출 되는 사용자에 대 한 핸들을 가져오는 데 사용 됩니다 (위의 [Startcallrequest 클래스](#the-startcallrequest-class) 참조).

`PerformStartCallAction`이제 [Providerdelegate 클래스](#the-providerdelegate-class) 의 메서드를 사용 하 여 실제 나가는 호출을 시작 하 고 시스템의 수명 주기를 알립니다.
...
`ActiveCall`진행 중인 호출에 대 한 정보를 저장 하기 위해 클래스의 인스턴스를 만들고 호출 되는 사용자로 채웁니다. `StartingConnectionChanged`및 `ConnectedChanged` 이벤트는 나가는 통화 수명 주기를 모니터링 하 고 보고 하는 데 사용 됩니다. 호출이 시작 되 고 시스템에서 작업이 완료 되었음을 알립니다.
...
3. 응용 프로그램은 오류를 시스템에 알리기 위해 **실패** 메시지를 다시 시작 호출 작업 ()으로 보내야 *합니다* `Action.Fail()` .
...
`CXProviderDelegate`이러한 시간 제한 상황을 정상적으로 처리 하기 위해 재정의 해야 하는 공급자 대리자 ()에는 여러 메서드가 있습니다.
...
[![호출 시작 작업 순서](callkit-images/callkit09.png)](callkit-images/callkit09.png#lightbox)

1. 들어오는 호출에 응답 하기 위해 앱에서 호출 시작 작업을 수신 합니다.
2. 응용 프로그램에서이 작업을 수행 하기 전에에는에 필요한 구성이 제공 됩니다 `AVAudioSession` .
3. 앱은 작업이 수행 되었음을 시스템에 알립니다.
4. 호출을 연결 하기 전에 CallKit는 `AVAudioSession` 앱이 요청한 구성과 일치 하는 높은 우선 순위를 제공 합니다. 의 메서드를 통해 앱에 알림이 제공 됩니다 `DidActivateAudioSession` `CXProviderDelegate` .
...
그러면 `CallDirectoryHandler.cs` 다음과 같은 클래스를 프로젝트에 추가 합니다.
...
`BeginRequest`필요한 기능을 제공 하려면 호출 디렉터리 처리기의 메서드를 수정 해야 합니다. 위의 샘플에서는 VOIP 앱의 연락처 데이터베이스에서 차단 및 사용 가능한 숫자 목록을 설정 하려고 합니다. 어떤 이유로 든 실패 한 요청이 있으면를 만들어 `NSError` 오류를 설명 하 고 `CancelRequest` 클래스의 메서드를 전달 합니다 `CXCallDirectoryExtensionContext` .

차단 된 번호를 설정 하려면 `AddBlockingEntry` 클래스의 메서드를 사용 합니다 `CXCallDirectoryExtensionContext` . 메서드에 제공 되는 숫자는 숫자순으로 _오름차순 이어야 합니다_ . 전화 번호가 많은 경우 최적의 성능과 메모리 사용을 위해 지정 된 시간에 숫자 하위 집합을 로드 하 고 autorelease pool을 사용 하 여 로드 된 각 일괄 처리에서 할당 된 개체를 해제 하는 것만 고려 하십시오.

VOIP 앱에 알려진 연락처 번호의 연락처에 알리기 위해 클래스의 메서드를 사용 하 `AddIdentificationEntry` `CXCallDirectoryExtensionContext` 고 숫자와 식별 레이블을 모두 제공 합니다. 또한 메서드에 제공 되는 숫자는 숫자로 오름차순으로 정렬 _되어야_ 합니다. 전화 번호가 많은 경우 최적의 성능과 메모리 사용을 위해 지정 된 시간에 숫자 하위 집합을 로드 하 고 autorelease pool을 사용 하 여 로드 된 각 일괄 처리에서 할당 된 개체를 해제 하는 것만 고려 하십시오.

## <a name="summary"></a>요약

이 문서에서는 iOS 10에서 Apple에서 릴리스된 새 CallKit API와 Xamarin.ios VOIP 앱에서이 API를 구현 하는 방법에 대해 설명 했습니다. 이는 CallKit에서 앱을 iOS 시스템에 통합 하는 방법, 기본 제공 앱과 함께 기능 패리티를 제공 하는 방법 (예: 전화) 및 Siri 상호 작용 및 연락처 앱을 통해 잠금 및 홈 화면과 같은 위치에서 iOS 전체에 앱의 가시성을 향상 시키는 방법을 보여 줍니다.
