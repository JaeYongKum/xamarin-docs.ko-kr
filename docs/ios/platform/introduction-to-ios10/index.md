...
Apple의 트루 톤 표시 기술은 iOS 장치에서 주변 광원 센서를 사용 하 여 현재 조명 조건과 일치 하도록 디스플레이의 색 및 강도를 동적으로 조정 합니다. iOS 10은 앱의 파일에 추가할 수 있는 새 [UIWhitePointAdaptivityStyle](https://developer.apple.com/library/prerelease/content/documentation/General/Reference/InfoPlistKeyReference/Articles/iPhoneOSKeys.html#//apple_ref/doc/uid/TP40009252-SW31) 키를 제공 하 `Info.plist` 고, 참 톤이 표준 컬러 시프트를 적용 하는 방식을 제어 합니다. 
...
- `DocumentInputMode`클래스의 새 속성은 `UITextDocumentProxy` 문서의 입력 언어를 결정 하 고 해당 언어에 맞게 키보드 확장을 허용할 수 있습니다.
...
또한 PassKit 프레임 워크는 외부 Apple Pay를 지원 하도록 확장 되었으며 `UIKit` 카드 발급자가 자신의 앱 내에서 카드를 제공할 수 있도록 합니다.
...
따라서 iOS 10 (또는 이후 버전)에서 실행 되는 앱 `Info.plist` 은 사용자에 게 액세스 권한을 얻으려고 하는 이유를 설명 하는 개인 정보 보호 키를 하나 이상 입력 하 여 특정 기능 또는 사용자 정보에 액세스 하는 의도를 정적으로 선언 해야 합니다.
...
음성 인식에는 Apple 서버에서 데이터의 전송 및 임시 저장소가 필요 하므로 앱은 파일 _must_ 에 키를 포함 하 `NSSpeechRecognitionUsageDescription` `Info.plist` 고 메서드를 호출 하 여 인식을 수행할 수 있는 사용자의 권한을 요청 해야 합니다 `SFSpeechRecognizer.RequestAutorization` .
...
- 이제 [UIColor](xref:UIKit.UIColor) 는 sRGB 색 공간을 사용 하 고 값을 to 범위에 더 이상 클램프 하지 않습니다 `0.0` `1.0` . 앱이 이전 클램프 동작에 의존 하는 경우에는 iOS 10에 대해 수정 해야 합니다.
- IPad Pro에서 사용자 지정 그리기를 수행 하는 경우에는 sRGB 색 공간에 대해 그리기 환경이 구성 됩니다 `UIView` .
...
- `UILocalNotification` `UIMutableUserNotificationAction` `UIMutableUserNotificationCategory` `UIUserNotificationAction` `UIUserNotificationCategory` Uikit의,,, 및 클래스는 `UIUserNotificationSettings` 더 이상 사용 되지 않습니다. 대신 [사용자 알림](#user-notifications) 프레임 워크를 사용 합니다.
- `HandleActionForLocalNotification`, `HandleActionForRemoteNotification` `DidReceiveLocalNotification` 및 `DidReceiveRemoteNotification` WatchKit 메서드는 더 이상 사용 되지 않습니다. `HandleActionForNotification`대신 및 메서드를 사용 `DidReceiveNotification` 합니다.
- `DidReceiveLocalNotification` `DidReceiveRemoteNotification` [WKExtensionDelegate](https://developer.apple.com/reference/watchkit/wkextensiondelegate) 의 및 메서드는 더 이상 사용 되지 않습니다. 적절 한 메서드를 구현 하 고 Unusernotificationcenter 개체의 속성에 할당 하는 [Unusernotificationcenter 대리자](https://developer.apple.com/reference/usernotifications/unusernotificationcenterdelegate) 의 인스턴스를 만듭니다 `Delegate` . [UNUserNotificationCenter](https://developer.apple.com/reference/usernotifications/unusernotificationcenter)
