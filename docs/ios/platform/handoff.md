...
[![핸드 오프 작업을 수행 하는 예제](handoff-images/handoff02.png)](handoff-images/handoff02.png#lightbox)
...
`NSUserActivity`클래스 (및의 일부 작은 변경 내용 포함 `UIKit` `AppKit` )는 사용자의 다른 장치에서 잠재적으로 계속 될 수 있는 사용자 활동을 정의 하는 기능을 제공 합니다.

활동을 사용자의 다른 장치로 전달 하려면 인스턴스에 캡슐화 하 고, `NSUserActivity` _현재 활동_으로 표시 되는 페이로드 집합 (연속 작업을 수행 하는 데 사용 되는 데이터)이 있고, 활동을 해당 장치로 전송 해야 합니다.

전달은 작업을 계속 하도록 정의 하기 위해 최소 정보를 전달 하 고, iCloud를 통해 더 큰 데이터 패킷을 동기화 합니다.

수신 장치에서 사용자는 활동을 연속으로 사용할 수 있다는 알림을 받게 됩니다. 사용자가 새 장치에서 작업을 계속 하도록 선택 하면 지정 된 앱이 시작 되 고 (아직 실행 중이 아닌 경우)의 페이로드가 `NSUserActivity` 활동을 다시 시작 하는 데 사용 됩니다.

[![사용자 활동을 계속 하는 개요](handoff-images/handoffinteractions.png)](handoff-images/handoffinteractions.png#lightbox)
...
수신 앱은의 사전에 있는 정보를 사용 하 여 `NSUserActivity` `UserInfo` 사용자 인터페이스를 구성 하 고, 지정 된 활동의 상태를 복원 하 여 최종 사용자에 게 원활한 전환이 표시 되도록 합니다.

연속 작업에를 통해 효율적으로 보낼 수 있는 것 보다 많은 정보가 필요한 경우 `NSUserActivity` 다시 시작 하는 앱은 원래 앱에 대 한 호출을 보내고 필요한 데이터를 전송 하는 스트림을 하나 이상 설정할 수 있습니다. 예를 들어 작업이 여러 이미지를 포함 하는 많은 텍스트 문서를 편집 하는 경우 수신 장치에서 작업을 계속 하는 데 필요한 정보를 전송 하려면 스트리밍이 필요 합니다. 자세한 내용은 아래의 [연속 스트림 지원](#supporting-continuation-streams) 섹션을 참조 하세요.
...
클래스는 전달 `NSUserActivity` 교환의 주 개체 이며 연속에 사용할 수 있는 사용자 활동의 상태를 캡슐화 하는 데 사용 됩니다. 앱은 지원 되는 `NSUserActivity` 모든 활동에 대 한의 복사본을 인스턴스화하고 다른 장치에서 계속 진행 하려고 합니다. 예를 들어 문서 편집기는 현재 열려 있는 각 문서에 대 한 활동을 만듭니다. 그러나 맨 앞의 창이 나 탭에 표시 되는 맨 앞의 문서만 _현재 작업_ 이며 연속에 사용할 수 있는 줄임으로써.

인스턴스는 `NSUserActivity` 해당 및 속성으로 식별 됩니다 `ActivityType` `Title` . `UserInfo`사전 속성은 활동의 상태에 대 한 정보를 전달 하는 데 사용 됩니다. `NeedsSave` `true` 의 대리자를 통해 상태 정보를 지연 로드 하려는 경우 속성을로 설정 합니다 `NSUserActivity` . `AddUserInfoEntries` `UserInfo` 작업 상태를 유지 하기 위해 필요에 따라 메서드를 사용 하 여 다른 클라이언트의 새 데이터를 사전에 병합 합니다.

### <a name="the-nsuseractivitydelegate-class"></a>NSUserActivityDelegate 클래스

는 `NSUserActivityDelegate` 의 사전 정보를 최신 상태로 유지 `NSUserActivity` 하 `UserInfo` 고 활동의 현재 상태와 동기화 하는 데 사용 됩니다. 시스템에서 업데이트할 활동의 정보 (예: 다른 장치에서 계속 하기 전에)가 필요한 경우 대리자의 메서드를 호출 합니다 `UserActivityWillSave` .

메서드를 구현 하 `UserActivityWillSave` 고 (예:, 등)를 변경 하 여 `NSUserActivity` `UserInfo` `Title` 현재 활동의 상태를 계속 반영 해야 합니다. 시스템에서 메서드를 호출 하면 `UserActivityWillSave` `NeedsSave` 플래그가 지워집니다. 활동의 데이터 속성을 수정할 경우를 다시로 설정 해야 `NeedsSave` `true` 합니다.

위에서 설명한 방법을 사용 하는 대신 `UserActivityWillSave` 선택적 `UIKit` `AppKit` 으로 사용자 활동을 자동으로 보유 하거나 관리할 수 있습니다. 이렇게 하려면 응답자 개체의 속성을 설정 하 `UserActivity` 고 메서드를 구현 `UpdateUserActivityState` 합니다. 자세한 내용은 아래의 [응답자의 지원](#supporting-handoff-in-responders) 전달 섹션을 참조 하세요.

### <a name="app-framework-support"></a>앱 프레임 워크 지원

`UIKit`(IOS) 및 `AppKit` (OS X)는 모두 `NSDocument` , 응답자 ( `UIResponder` / `NSResponder` ) 및 클래스에 대 한 기본 제공 지원을 제공 `AppDelegate` 합니다. 각 OS는 더 약간 다르게 이송을 구현 하지만 기본 메커니즘과 Api는 동일 합니다.

#### <a name="user-activities-in-document-based-apps"></a>문서 기반 앱의 사용자 활동

문서 기반 iOS 및 OS X 앱은 자동으로 전달 지원을 기본 제공 합니다. 이 지원을 활성화 하려면 `NSUbiquitousDocumentUserActivityType` `CFBundleDocumentTypes` 앱의 **info.plist** 파일에서 각 항목에 대 한 키 및 값을 추가 해야 합니다.

이 키가 있는 경우 및는 `NSDocument` `UIDocument` 지정 된 `NSUserActivity` 유형의 iCloud 기반 문서에 대 한 인스턴스를 자동으로 만듭니다. 앱에서 지 원하는 각 문서 형식에 대 한 활동 유형을 제공 해야 하며 여러 문서 유형은 동일한 활동 유형을 사용할 수 있습니다. `NSDocument`및 `UIDocument` 는 모두의 속성을 해당 `UserInfo` `NSUserActivity` `FileURL` 속성 값으로 자동으로 채웁니다.
...
`UIKit`및 `AppKit` 는 모두 응답자 개체의 속성으로 설정 하는 경우 사용자 활동을 자동으로 관리할 수 있습니다 `UserActivity` . 상태가 수정 된 경우 `NeedsSave` 응답자의 속성 `UserActivity` 을로 설정 해야 `true` 합니다. `UserActivity`필요한 경우 해당 메서드를 호출 하 여 상태를 업데이트 하는 응답자 시간을 제공한 후 시스템에서 자동으로를 저장 합니다 `UpdateUserActivityState` .

여러 응답 자가 단일 인스턴스를 공유 하는 경우 `NSUserActivity` `UpdateUserActivityState` 시스템에서 사용자 활동 개체를 업데이트할 때 콜백을 받게 됩니다. `AddUserInfoEntries` `NSUserActivity` `UserInfo` 이 시점에서 현재 활동 상태를 반영 하도록의 사전을 업데이트 하려면 응답자는 메서드를 호출 해야 합니다. `UserInfo`각 호출 전에 사전이 지워집니다 `UpdateUserActivityState` .

활동에서 자신을 분리 하기 위해 응답자는 `UserActivity` 속성을로 설정할 수 있습니다 `null` . 앱 프레임 워크 관리 되는 `NSUserActivity` 인스턴스에 더 이상 연결 된 응답자 또는 문서가 없으면 자동으로 무효화 됩니다.
...
앱 `AppDelegate` 은 핸드 오프 연속을 처리할 때 기본 진입점입니다. 사용자가 핸드 오프 알림에 응답 하면 적절 한 앱이 시작 되 고 (아직 실행 되지 않은 경우) `WillContinueUserActivityWithType` 의 메서드가 호출 됩니다 `AppDelegate` . 이 시점에서 앱은 연속 작업이 시작 되 고 있음을 사용자에 게 알려 주어 야 합니다.

`NSUserActivity` `AppDelegate` 의 메서드가 호출 되 면 인스턴스가 전달 됩니다 `ContinueUserActivity` . 이때 앱의 사용자 인터페이스를 구성 하 고 지정 된 작업을 계속 합니다.
...
4. 지정 된 ID에 대해 **iCloud** 서비스를 확인 했는지 확인 합니다.
    ...
    [![지정 된 ID에 대해 iCloud 서비스를 사용 하도록 설정](handoff-images/provision01.png)](handoff-images/provision01.png#lightbox)
5. 변경 내용을 저장합니다.
6. 프로 **비전 프로필**  >  **개발** 을 클릭 하 고 앱에 대 한 새 개발 프로 비전 프로필을 만듭니다.

    [![앱에 대 한 새 개발 프로 비전 프로필 만들기](handoff-images/provision02.png)](handoff-images/provision02.png#lightbox)
...
8. Xamarin.ios 프로젝트 옵션을 편집 하 고 방금 만든 프로 비전 프로필을 사용 하 고 있는지 확인 합니다.
    ...
    [![앞서 만든 프로 비전 프로필을 선택 합니다.](handoff-images/provision03.png)](handoff-images/provision03.png#lightbox)
9. 그런 다음 **info.plist** 파일을 편집 하 고 프로 비전 프로필을 만드는 데 사용 된 앱 ID를 사용 하 고 있는지 확인 합니다.

    [![앱 ID 설정](handoff-images/provision04.png)](handoff-images/provision04.png#lightbox)
10. **배경 모드** 섹션으로 스크롤하고 다음 항목을 확인 합니다.

    [![필요한 백그라운드 모드 사용](handoff-images/provision05.png)](handoff-images/provision05.png#lightbox)
...
동일한 개발자 팀 ID로 서명 된 앱 간에 사용자 활동을 계속 하 고 동일한 활동 유형을 지원할 수 있습니다. Xamarin.ios 앱에 전달 기능을 구현 하려면 또는에서 사용자 활동 개체를 만들고 `UIKit` `AppKit` , 활동을 추적 하기 위해 개체의 상태를 업데이트 하 고, 수신 장치에서 활동을 계속 해야 합니다.
...
_활동 유형 식별자_ 는 `NSUserActivityTypes` 지정 된 사용자 활동 유형을 고유 하 게 식별 하는 데 사용 되는 앱의 **info.plist** 파일 배열에 추가 된 짧은 문자열입니다. 앱에서 지 원하는 각 작업에 대 한 항목은 배열에 하나씩 있습니다. Apple은 충돌을 방지 하기 위해 활동 형식 식별자에 대 한 역방향 DNS 스타일 표기법을 사용 하는 것을 제안 합니다. 예를 들어 `com.company-name.appname.activity` 특정 앱 기반 활동의 경우 또는 `com.company-name.activity` 여러 앱에서 실행할 수 있는 활동의 경우입니다.
...
[![Info.plist 편집기의 NSUserActivityTypes 키 및 필수 식별자](handoff-images/type01.png)](handoff-images/type01.png#lightbox)
...
클래스의 새 인스턴스를 만들 때 `NSUserActivity` `NSUserActivityDelegate` 활동의 상태에 대 한 변경 내용을 추적 하는 인스턴스를 지정 합니다. 예를 들어 다음 코드를 사용 하 여 상태 변경을 추적할 수 있습니다.
...
`UserActivityReceivedData`연속 스트림이 송신 장치에서 데이터를 수신 하면 메서드가 호출 됩니다. 자세한 내용은 아래의 [연속 스트림 지원](#supporting-continuation-streams) 섹션을 참조 하세요.
...
`UserActivityWillSave`메서드는 활동에 대 한 변경 내용이 저장 되 고 로컬에서 사용 가능한 장치에 동기화 되기 전에 호출 됩니다. 이 메서드를 사용 하 여 `UserInfo` 인스턴스를 보내기 전에 인스턴스의 속성에 대 한 마지막 분 변경 작업을 수행할 수 있습니다 `NSUserActivity` .

### <a name="creating-a-nsuseractivity-instance"></a>NSUserActivity 인스턴스 만들기

앱에서 다른 장치를 계속 진행할 수 있는 가능성을 제공 하고자 하는 각 활동은 인스턴스에 캡슐화 되어야 합니다 `NSUserActivity` . 앱은 필요한 만큼의 작업을 만들 수 있으며 해당 활동의 특성은 해당 앱의 기능 및 기능에 따라 달라 집니다. 예를 들어 전자 메일 앱은 새 메시지를 만들기 위한 활동 하 나와 메시지를 읽는 다른 활동을 만들 수 있습니다.

예제 앱의 경우 `NSUserActivity` 사용자가 탭 웹 브라우저 보기 중 하나에 새 URL을 입력할 때마다 새이 만들어집니다. 다음 코드는 지정 된 탭의 상태를 저장 합니다.
...
`NSUserActivity`위에서 만든 사용자 활동 유형 중 하나를 사용 하 여 새를 만들고 활동에 대해 사람이 읽을 수 있는 제목을 제공 합니다. 위의 만든 인스턴스에 연결 하 여 `NSUserActivityDelegate` 상태 변경을 감시 하 고이 사용자 활동이 현재 활동 임을 iOS에 알립니다.
...
핸드 오프는 비연속적 사용자 활동의 가용성을 위해 원래 장치에 물리적으로 근접 하 고 동일한 iCloud 계정에 로그인 한 로컬 iOS 및 OS X 장치를 자동으로 알립니다. 사용자가 새 장치에서 작업을 계속 하도록 선택 하는 경우 시스템은 적절 한 앱 (팀 ID 및 활동 유형에 따라)을 시작 하 고 해당 연속 작업을 `AppDelegate` 수행 해야 한다는 정보를 설정 합니다.

먼저 `WillContinueUserActivityWithType` 메서드가 호출 되어 사용자에 게 연속 작업이 시작 되는 것을 알릴 수 있습니다. 예제 앱의 **AppDelegate.cs** 파일에서 다음 코드를 사용 하 여 연속 시작을 처리 합니다.
...
`ContinueUserActivity`의는 `AppDelegate` 실제로 지정 된 작업을 계속 하기 위해 호출 됩니다. 이번에는 앱 예제에서 다음을 수행 합니다.
...
문서 기반 앱의 경우 메서드를 구현 하지 `ContinueUserActivity` 않거나 반환 하거나 `false` 를 반환 `UIKit` 하거나 작업을 `AppKit` 자동으로 다시 시작할 수 있습니다. 자세한 내용은 아래의 [문서 기반 앱에서 전달 지원](#supporting-handoff-in-document-based-apps) 섹션을 참조 하세요.
...
제공 된를 사용 `NSError` 하 여 오류에 대 한 정보를 사용자에 게 제공 해야 합니다.
...
원래 앱이 웹 인터페이스 (및 지정 된 항목을 계속 진행 하는 데 필요한 구문)의 URL을 알고 있는 경우 인스턴스의 속성에서이 정보를 인코딩할 수 있습니다 `WebpageURL` `NSUserActivity` . 수신 장치에 연속 작업을 처리할 적절 한 네이티브 앱이 설치 되어 있지 않은 경우 제공 된 웹 인터페이스를 호출할 수 있습니다.

## <a name="web-browser-to-native-app-handoff"></a>웹 브라우저에서 네이티브 앱 전달

사용자가 원래 장치에서 웹 기반 인터페이스를 사용 하 고 있고 수신 장치의 네이티브 앱이 속성의 도메인 부분을 클레임 하는 경우 시스템은 해당 앱을 사용 하 여 연속 작업을 `WebpageURL` 처리 합니다. 새 장치는 `NSUserActivity` 활동 유형을로 표시 하는 인스턴스를 수신 하 `BrowsingWeb` 고,는 `WebpageURL` 사용자가 방문한 URL을 포함 하 게 됩니다 `UserInfo` . 사전이 비어 있게 됩니다.

앱이이 형식의 전달에 참여 하려면 `com.apple.developer.associated-domains` 형식 `<service>:<fully qualified domain name>` (예:)을 사용 하 여 도메인에 자격을 요청 해야 합니다. `activity continuation:company.com`
...
앱은 해당 도메인의 자격을 가진 모든 활동을 수신 합니다 `WebpageURL` `com.apple.developer.associated-domains` . `http`및 `https` 프로토콜만 지원 되며 다른 프로토콜은 예외를 발생 시킵니다.
...
자동으로 생성 된 사용자 활동 개체 (문서의 속성을 통해 사용 가능 `UserActivity` )는 앱의 다른 개체에서 참조 하 고 연속 시 상태를 복원 하는 데 사용할 수 있습니다. 예를 들어 항목 선택 및 문서 위치를 추적 합니다. `NeedsSave` `true` 상태를 변경 하 고 메서드에서 사전을 업데이트할 때마다이 활동 속성을로 설정 해야 `UserInfo` `UpdateUserActivityState` 합니다.

`UserActivity`속성은 모든 스레드에서 사용할 수 있으며 KVO (키-값 관찰) 프로토콜을 준수 하므로 iCloud에서 이동할 때 문서를 동기화 상태로 유지 하는 데 사용할 수 있습니다. `UserActivity`문서를 닫으면 속성이 무효화 됩니다.
...
원래 장치에서 사용자 활동 대리자는 해당 메서드를 호출 하 여 다시 `DidReceiveInputStream` 시작 장치에서 사용자 활동을 계속 하는 데 필요한 데이터를 제공 함으로써 스트림을 받습니다.

을 사용 하 여 `NSInputStream` 스트림 데이터에 읽기 전용 액세스를 제공 하 고 `NSOutputStream` 쓰기 전용 액세스를 제공 합니다. 스트림은 수신 앱에서 더 많은 데이터를 요청 하 고 원래 앱에서 제공 하는 요청 및 응답 방식으로 사용 해야 합니다. 따라서 원래 장치의 출력 스트림에 기록 된 데이터를 계속 해 서 장치의 입력 스트림에서 읽고 그 반대의 경우도 마찬가지입니다.
...
- 앱에서 사용자 활동의 상태 ()를 업데이트 해야 하는 경우 `NSUserActivity` 속성을 `NeedsSave` 로 설정 `true` 합니다. 적절 한 시간에 전달은 대리자의 메서드를 호출 `UserActivityWillSave` 하므로 필요에 따라 사전을 업데이트할 수 있습니다 `UserInfo` .
- 전달 프로세스는 수신 장치에서 즉시 초기화 되지 않을 수 있기 때문에를 구현 하 `AppDelegate` `WillContinueUserActivity` 고 연속 작업이 시작 됨을 사용자에 게 알립니다.
...
[![핸드 오프 앱 예제](handoff-images/handoff01.png)](handoff-images/handoff01.png#lightbox)

사용자의 장치 중 다른 사용자의 장치에 **Monkeybrowser** 앱이 설치 되어 있는 경우, 동일한 사용자 계정을 사용 하 여 iCloud에 로그인 하 고, 동일한 네트워크에 있고, 위의 장치에 가까운 위치에 있는 경우 전달 활동은 홈 화면에 표시 됩니다 (왼쪽 하단 모서리).

[![왼쪽 아래 모퉁이의 홈 화면에 표시 되는 핸드 오프 활동](handoff-images/handoff02.png)](handoff-images/handoff02.png#lightbox)

사용자가 핸드 위로 아이콘을 끌면 앱이 시작 되 고에 지정 된 사용자 활동이 `NSUserActivity` 새 장치에서 계속 됩니다.

[![새 장치에서 계속 되는 사용자 작업](handoff-images/handoff03.png)](handoff-images/handoff03.png#lightbox)

사용자 활동이 다른 Apple 장치로 성공적으로 전송 되 면 송신 장치는 `NSUserActivity` 해당에 대 한 메서드에 대 한 호출을 수신 `UserActivityWasContinued` `NSUserActivityDelegate` 하 여 사용자 활동이 다른 장치에 성공적으로 전송 되었음을 알 수 있습니다.
