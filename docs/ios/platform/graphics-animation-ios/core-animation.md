...
예를 들어,를 사용 하 여 컨트롤러를 사용 하는 경우 `UIImageView` 이미지를 누르는 것은 두 번째를 표시 하는 것 `UIImageView` 입니다. 두 번째 이미지 뷰로 전환 하기 위해 이미지 뷰의 슈퍼 뷰에 애니메이션 효과를 주려면를 호출 하는 것 만큼 간단 하 `UIView.Transition` `toView` 고 아래와 같이 및를 전달 합니다 `fromView` .
...
UIKit는 다음을 포함 하 여 클래스에서 다양 한 속성에 대 한 애니메이션 효과를 무료로 지원 합니다 `UIView` .
...
계층에 대 한 암시적 애니메이션을 추가 하려면 단순히의 속성 변경 내용을 래핑합니다 `CATransaction` . 이렇게 하면 아래와 같이 보기 애니메이션 (예: 및)으로 애니메이션 효과 되지 않는 속성에 애니메이션을 적용할 수 있습니다 `BorderWidth` `BorderColor` .
...
이 코드는 계층의 좌표에 있는 `Position` 왼쪽 위에서 측정 된 계층의 앵커 지점 위치인 계층에도 애니메이션 효과를 적용 합니다. 계층의 앵커 지점은 계층 좌표계 내의 정규화 된 점입니다.
...
암시적 애니메이션 외에도 핵심 애니메이션에는에서 상속 되는 다양 한 클래스가 포함 되어 있으므로 `CAAnimation` 계층에 명시적으로 추가 되는 애니메이션을 캡슐화 할 수 있습니다. 애니메이션의 시작 값을 수정 하 고, 애니메이션을 그룹화 하 고, 비선형 경로를 허용 하기 위해 키 프레임을 지정 하는 등 애니메이션을 보다 세부적으로 제어할 수 있습니다.

다음 코드에서는 앞에 표시 된 계층에 대해를 사용 하는 명시적 애니메이션의 예를 보여 줍니다 `CAKeyframeAnimation` (암시적 애니메이션 단원).
...
이 코드는 `Position` 키 프레임 애니메이션을 정의 하는 데 사용 되는 경로를 만들어 계층의를 변경 합니다. 계층의 `Position` 가 애니메이션에서의 최종 값으로 설정 된 것을 확인할 수 `Position` 있습니다. 이를 사용 하지 않으면 `Position` 애니메이션은 표시 값만 변경 하 고 실제 모델 값은 변경 하지 않기 때문에 애니메이션 이전에 계층이 갑자기 반환 됩니다. 모델 값을 애니메이션에서 최종 값으로 설정 하면 레이어가 애니메이션의 끝 부분에 그대로 유지 됩니다.
