...
구현은 다음과 같습니다. 끌어 온 행과 연결 된 데이터를 검색 하 고 인코딩한 다음, `NSItemProvider` 응용 프로그램에서 작업의 "drop" 부분을 처리 하는 방법을 결정 하는을 구성 합니다 (예:이 예제에서는 데이터 형식을 처리할 수 있는지 여부 `PlainText` ).
...
끌기 대리자에는 끌기 동작을 사용자 지정 하기 위해 구현할 수 있는 여러 가지 선택적 메서드가 있습니다. 예를 들어 서식 있는 텍스트 뿐만 아니라 일반 텍스트와 같은 그리기의 벡터 및 비트맵 버전을 사용할 수 있는 여러 데이터 표현을 제공 합니다. 동일한 앱 내에서 끌어서 놓을 때 사용할 사용자 지정 데이터 표현을 제공할 수도 있습니다.
...
Drop 작업은, 또는 중 하나일 수 있습니다 `Cancel` `Move` `Copy` .

Drop 의도는 새 행을 삽입 하거나 기존 행에 데이터를 추가/추가할 수 있습니다.

#### <a name="performdrop"></a>PerformDrop

`PerformDrop`메서드는 사용자가 작업을 완료할 때 호출 되 고, 삭제 된 데이터를 반영 하도록 테이블 뷰와 데이터 원본을 수정 합니다.
