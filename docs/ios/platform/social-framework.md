...
소셜 프레임 워크를 사용 하 여 Twitter에 연결 하려면 아래와 같이 장치 설정에서 계정을 구성 해야 합니다.
 ...
 [![Twitter 계정 설정](social-framework-images/twitter01.png)](social-framework-images/twitter01.png#lightbox)
...
소셜 프레임 워크에는 `SLComposeViewController` 트 윗을 편집 하 고 보내는 시스템 제공 뷰를 제공 하는 라는 컨트롤러가 포함 되어 있습니다. 다음 스크린샷은이 뷰의 예를 보여 줍니다.

 [![이 스크린샷에서는 SLComposeViewController의 예를 보여 줍니다.](social-framework-images/twitter02.png)](social-framework-images/twitter02.png#lightbox)

Twitter에서를 사용 하려면 `SLComposeViewController` `FromService` 아래와 같이를 사용 하 여 메서드를 호출 하 여 컨트롤러의 인스턴스를 만들어야 합니다 `SLServiceType.Twitter` .

```csharp
var slComposer = SLComposeViewController.FromService (SLServiceType.Twitter);
```

인스턴스가 반환 된 후에 `SLComposeViewController` 는 Twitter에 게시할 UI를 제공 하는 데 사용할 수 있습니다. 그러나이 경우에는 다음을 호출 하 여 소셜 네트워크 (이 경우 Twitter)의 가용성을 확인 해야 합니다 `IsAvailable` .
...
다음 코드에서는를 사용 하 여 `SLComposeViewController` 트 윗을 보내는 데 사용 되는 뷰를 표시 하는 방법을 보여 줍니다.
...
소셜 프레임 워크에는 소셜 네트워크에 대 한 HTTP 요청 만들기 지원도 포함 됩니다. `SLRequest`특정 소셜 네트워크의 API를 대상으로 하는 데 사용 되는 클래스의 요청을 캡슐화 합니다.
...
이 예제에서는 반환 된 결과를 URL에를 포함 하 여 마지막 10 개 항목으로 제한 합니다 `?count=10` . 마지막으로,이 요청을 Twitter 계정 (위에서 로드 됨)에 연결 하 고 Twitter에 대 한 호출을 수행 하 여 데이터를 인출 합니다.
...
[![원시 JSON 데이터 표시의 예](social-framework-images/twitter03.png)](social-framework-images/twitter03.png#lightbox)
...
[![Facebook 계정 설정](social-framework-images/facebook01.png)](social-framework-images/facebook01.png#lightbox)
...
Facebook과 함께 사용 하는 경우은 `SLComposeViewController` Twitter 예제와 거의 동일 하 게 표시 되는 보기를 표시 합니다 .이 경우에는 **facebook** 을 제목으로 표시 합니다.

[![SLComposeViewController 표시](social-framework-images/facebook02.png)](social-framework-images/facebook02.png#lightbox)

### <a name="calling-facebook-graph-api"></a>Facebook Graph API 호출

Twitter 예제와 마찬가지로 소셜 프레임 워크의 개체는 `SLRequest` Facebook의 GRAPH API와 함께 사용할 수 있습니다. 예를 들어 다음 코드는 위에 지정 된 코드를 확장 하 여 Xamarin 계정에 대 한 graph API의 정보를 반환 합니다.
...
이 문서에서는 소셜 프레임 워크를 사용 하 여 Twitter 및 Facebook과 상호 작용 하는 방법을 살펴보았습니다. 장치 설정에서 각 소셜 네트워크에 대 한 계정을 구성 하는 위치를 살펴보았습니다. 또한를 사용 하 여 `SLComposeViewController` 소셜 네트워크에 게시 하기 위한 통합 된 보기를 제공 하는 방법에 대해서도 설명 합니다. 또한 `SLRequest` 각 소셜 네트워크의 API를 호출 하는 데 사용 되는 클래스를 검사 했습니다.
