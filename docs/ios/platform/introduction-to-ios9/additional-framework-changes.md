...
[![iOS 9 로고](additional-framework-changes-images/ios9-sml.png)](additional-framework-changes-images/ios9.png#lightbox)
...
- **AVPlayerViewController** - `AVPlayer` iPad에서 크기 조정 가능한 부동 PIP 창에 비디오를 표시 하는 데 사용 되는 컨트롤러를 관리 합니다.
...
- 새 추적 범주 및 특성이 `HKQuantityTypeIdentifier` 클래스 (예: `UVExposure` ) 및 `HKCategoryTypeIdentifier` 클래스 (예:)에 추가 되었습니다 `OvulationTestResult` . 
...
- `EvaluateAccessControl` `EvaluatePolicy` 이제 [LAContext](xref:LocalAuthentication.LAContext) 클래스의 및 메서드를 사용 하 여 이전에 성공한 잠금 해제 시도에서 Touch ID 일치를 재사용할 수 있습니다.
...
- **Iscredentialset** - `true` 자격 증명이 현재 설정 되어 있으면를 반환 합니다.
...
이러한 동작 변경으로 인해 `TouchesMoved` X/Y 좌표가 변경 되지 않은 경우에도 이벤트를 더 자주 호출 하도록 iOS 앱을 준비 해야 합니다.

자세한 내용은 [3D 터치 가이드 소개](~/ios/platform/3d-touch.md) 를 참조 하세요.

### <a name="document-open-in-place-functionality"></a>문서 열기 기능

`FinishedLaunching (application, launchOptions)` `WillFinishLaunching (Application, launchOptions)` 이제 [uiapplicationdelegate](xref:UIKit.UIApplicationDelegate) 클래스의 또는 메서드를 사용 하 여 문서를 열고 현재 위치의 문서를 수정할 수 있습니다 (복사 작업을 수행 하는 것이 아님).

새 열기 기능을 지원 하려면 `LSSupportsOpeningDocumentsInPlace` 값이 인 xamarin.ios 앱의 **info.plist** 파일에 키를 추가 합니다 `YES` .
...
새 `NSLayoutAnchor` 및 `NSLayoutDimension` 레이아웃 앵커 클래스는 [uiview](xref:UIKit.UIView) 클래스의 새 앵커 속성 (예: 및)을 사용 `LeadingAnchor` 하 여 `WidthAnchor` iOS 9에서 레이아웃을 보다 쉽게 만들 수 있습니다.
...
[Uiusernotificationaction](xref:UIKit.UIUserNotificationAction) 클래스에는 `Behavior` 알림에서 텍스트 입력을 지 원하는 데 사용할 수 있는 새 속성이 있습니다.

### <a name="uiapplicationdelegate-changes"></a>UIApplicationDelegate 변경 내용

Apple에서는 공식적으로 사용 되지 않지만 `FinishedLaunching (UIApplication application)` [Uiapplicationdelegate](xref:UIKit.UIApplicationDelegate) 클래스의 메서드에 대 한 모든 호출을 `FinishedLaunching (UIApplication application, NSDictionary launchOptions)` 또는 메서드로 대체 하는 것이 좋습니다 `WillFinishLaunching (UIApplication application, NSDictionary launchOptions)` .
...
- 사용자 지정 가능한 새 `UIFieldBehavior` 클래스를 사용 하 여 다양 한 필드 형식을 지원할 수 있습니다.
- 추가 첨부 파일 형식이 클래스에 추가 되었습니다 `UIAttachmentBehavior` .
