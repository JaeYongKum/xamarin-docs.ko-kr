...
[![적합성 및 체력 단련 앱 예제](workout-apps-images/workout01.png)](workout-apps-images/workout01.png#lightbox)
...
모든 체력 단련 앱의 핵심은 _Workout Session_ `HKWorkoutSession` 사용자가 시작 하 고 중지할 수 있는 체력 단련 세션 ()입니다. 체력 단련 세션 API는 구현 하기 쉬우며 다음과 같은 체력 단련 앱에 몇 가지 이점을 제공 합니다.
...
1. **솔루션 탐색기**에서 시청 확장의 도우미 앱 파일을 두 번 클릭 `Info.plist` 하 여 편집용으로 엽니다.
2. **원본** 뷰로 전환 합니다. 

    [![원본 뷰](workout-apps-images/plist01.png)](workout-apps-images/plist01.png#lightbox)
3. 라는 새 키를 추가 하 `WKBackgroundModes` 고 **형식을** 로 설정 합니다 `Array` . 

    [![WKBackgroundModes 라는 새 키를 추가 합니다.](workout-apps-images/plist02.png)](workout-apps-images/plist02.png#lightbox)
4. 의 **형식과** 값을 사용 하 여 배열에 새 항목을 추가 합니다 `String` `workout-processing` . 

    [![문자열 유형 및 체력 처리 값을 사용 하 여 배열에 새 항목을 추가 합니다.](workout-apps-images/plist03.png)](workout-apps-images/plist03.png#lightbox)
5. 변경 내용을 파일에 저장합니다.
...
[![진행 중인 세션을 시작 하는 세 가지 주요 단계](workout-apps-images/workout02.png)](workout-apps-images/workout02.png#lightbox)
...
2. 아래쪽으로 스크롤하고 **HealthKit 사용**을 선택 합니다. 
    ...
    [![HealthKit 사용을 선택 합니다.](workout-apps-images/auth01.png)](workout-apps-images/auth01.png#lightbox)
3. 변경 내용을 파일에 저장합니다.
...
는 체력 단련 `HKWorkoutConfiguration` 유형 (예:) `HKWorkoutActivityType.Running` 및 체력 (예:)을 지정 하는 체력 단련 구성 개체 ()를 사용 하 여 체력 단련 세션이 만들어집니다 `HKWorkoutSessionLocationType.Outdoor` .
...
작업 일지 세션 중에 발생할 수 있는 이벤트를 처리 하려면 앱에서 체력 세션 대리자 인스턴스를 만들어야 합니다. 프로젝트에 새 클래스를 추가 하 고 클래스의 기반으로 `HKWorkoutSessionDelegate` 합니다. 옥외 실행의 예는 다음과 같습니다.
...
이 클래스는 체력 세션의 상태가 변경 될 때 발생 하는 여러 이벤트를 만듭니다 `DidChangeToState` `DidFail` . 
...
[![얼굴 위에 표시 되는 작은 녹색 실행 남자 아이콘](workout-apps-images/workout03.png)](workout-apps-images/workout03.png#lightbox)
...
[![데이터 컬렉션 및 컨트롤 다이어그램](workout-apps-images/workout04.png)](workout-apps-images/workout04.png#lightbox)
...
메서드를 사용 하 여 데이터를 가져올 시작 날짜를 설정 하는 조건자를 만듭니다 `GetPredicateForSamples` . 메서드를 사용 하 여 HealthKit 정보를 가져오는 장치 집합을 만듭니다 `GetPredicateForObjectsFromDevices` .이 경우 로컬 Apple Watch ( `HKDevice.LocalDevice` )만 있습니다. 두 조건자는 `NSCompoundPredicate` 메서드를 사용 하 여 복합 조건자 ()로 결합 됩니다 `CreateAndPredicate` .

`HKAnchoredObjectQuery`원하는 데이터 요소 (이 경우 `HKQuantityTypeIdentifier.ActiveEnergyBurned` 활성 에너지 레코딩한 데이터 요소)에 대해 새로운가 생성 되 고 반환 되는 데이터의 양에 제한이 적용 되지 않습니다 ( `HKSampleQuery.NoLimit` ). HealthKit에서 앱으로 반환 되는 데이터를 처리 하기 위해 업데이트 처리기가 정의 됩니다. 
...
이 코드는 배열에서 반환 된 모든 샘플 ()에 대해 반복 하 여 `HKSample` `addedObjects` 수량 샘플 ()으로 캐스팅 `HKQuantitySample` 합니다. 그런 다음 샘플의 double 값을 줄 ()로 가져와, `HKUnit.Joule` 체력을 위해 구운 활성 에너지의 합계를 누적 하 고 사용자 인터페이스를 업데이트 합니다.
...
HealthKit에서 만든 이벤트를 관찰 하기 위해 앱은의 메서드를 재정의 합니다 `DidGenerateEvent` `HKWorkoutSessionDelegate` .
...
이 코드는 마커 이벤트 ()의 새 인스턴스를 만들고이를 `HKWorkoutEvent` 이벤트의 전용 컬렉션에 저장 합니다 .이는 나중에 다시 사용 세션에 기록 되 고 haptics를 통해 이벤트의 사용자에 게 알립니다.
...
HealthKit에서 생성 되는 일시 중지 및 재개 이벤트는의 메서드를 재정의 하 여 처리할 수 있습니다 `DidGenerateEvent` `HKWorkoutSessionDelegate` .
...
> [!IMPORTANT]
> 일시 중지 된 동작 및 동작 다시 시작 이벤트는 RunningWorkout 활동 형식 ()에 대해서만 지원 됩니다 `HKWorkoutActivityType.Running` .

이러한 이벤트는의 메서드를 재정의 하 여 처리할 수 있습니다 `DidGenerateEvent` `HKWorkoutSessionDelegate` .
...
[![체력 단련 세션 다이어그램 종료 및 저장](workout-apps-images/workout05.png)](workout-apps-images/workout05.png#lightbox)
...
진행 중인 세션을 종료 하려면를 전달 하는의 메서드를 호출 합니다 `EndWorkoutSession` `HKHealthStore` `HKWorkoutSession` .
...
그러면 장치 센서가 표준 모드로 다시 설정 됩니다. HealthKit가 체력을 종료 하면의 메서드에 대 한 콜백을 받게 됩니다 `DidChangeToState` `HKWorkoutSessionDelegate` .
...
앱이 진행 중인 세션을 종료 한 후에는 체력 ()을 만들고 `HKWorkout` HealthKit 데이터 저장소 ()에 (이벤트와 함께) 저장 해야 합니다 `HKHealthStore` .
...
이 코드는 개체에 대해 구운 총 에너지 양과 체력에 대 한 거리가 필요한를 만듭니다 `HKQuantity` . 체력을 정의 하는 메타 데이터 사전이 만들어지고, 체력 위치가 다음과 같이 지정 됩니다.

```csharp
metadata.Add (new NSString ("HKMetadataKeyIndoorWorkout"), new NSString ("NO"));
```

새 개체는와 같은를 사용 하 여 `HKWorkout` 생성 됩니다 `HKWorkoutActivityType` . 즉 `HKWorkoutSession` , 시작 및 종료 날짜, 이벤트 목록 (위 섹션에서 누적), 구운 에너지, 총 거리 및 메타 데이터 사전입니다. 이 개체는 Health Store 및 처리 된 모든 오류에 저장 됩니다.  
...
모든 watchOS 3 체력 청구 앱은 부모 iOS 10 기반 체력 앱을 포함 하며, iOS 10에 새로 추가 된이 iOS 앱을 사용 하 여 사용자 개입 없이 Apple Watch 체력을 체력으로 전환 하 고 백그라운드 실행 모드에서 watchOS 앱을 실행할 수 있습니다 (자세한 내용은 위에서 [실행 되는 배경 정보](#about-background-running) 참조).

WatchOS 앱이 실행 되는 동안 부모 iOS 앱과의 메시징 및 통신에 WatchConnectivity를 사용할 수 있습니다.

이 프로세스의 작동 원리를 살펴보세요.

[![iPhone 및 Apple Watch 통신 다이어그램](workout-apps-images/workout06.png)](workout-apps-images/workout06.png#lightbox)

1. IPhone 앱은 개체를 만들고 `HKWorkoutConfiguration` 체력 및 위치를 설정 합니다.
2. `HKWorkoutConfiguration`개체는 Apple Watch 버전의 앱으로 전송 되 고, 아직 실행 되지 않은 경우에는 시스템에 의해 시작 됩니다.
...
그런 다음 `HKWorkoutConfiguration` 일반적인 방법으로을 만들고의 메서드를 사용 하 여 `StartWatchApp` `HKHealthStore` Apple Watch에 보내고 앱과 체력 단련 세션을 시작 합니다.
...
를 사용 하 여 `HKWorkoutConfiguration` 새를 만들고 `HKWorkoutSession` 사용자 지정의 인스턴스를 연결 합니다 `HKWorkoutSessionDelegate` . 사용자의 HealthKit Health 스토어에 대해 체력 단련 세션이 시작 됩니다.
...
3. **watchOS 3 `OutdoorRunDelegate.cs` ** - `HKWorkoutSessionDelegate` 체력 이벤트를 처리 하는 사용자 지정입니다.
...
`ViewController.cs`진행 중인 앱의 부모 iOS 10 버전에 있는 파일에는 다음 코드가 포함 됩니다.
...
`ExtensionDelegate.cs`WatchOS 3 버전의 체력 앱에 있는 파일에는 다음 코드가 포함 됩니다.
...
`OutdoorRunDelegate.cs`WatchOS 3 버전의 체력 앱에 있는 파일에는 다음 코드가 포함 됩니다.
