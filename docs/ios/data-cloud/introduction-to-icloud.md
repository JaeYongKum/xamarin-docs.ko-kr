...
이 스크린샷에서는 사용 중인 샘플을 보여 줍니다. ICloud에서 변경 알림이 수신 되 면 화면 아래쪽의 스크롤 텍스트 보기에 인쇄 되 고 입력 필드에서 업데이트 됩니다.
 ...
 [![장치 간 메시지 흐름](introduction-to-icloud-images/icloud-kv-arrows.png)](introduction-to-icloud-images/icloud-kv-arrows.png#lightbox)
...
응용 프로그램은에 관찰자를 추가 하 여 iCloud에 의해 값이 변경 될 때 알림을 받을 수도 있습니다 `NSNotificationCenter.DefaultCenter` .
...
이 다이어그램은 모두 어떻게 통합 되는지 보여 줍니다. 각 장치에는 로컬 저장소 (UbiquityContainer)에 저장 된 데이터가 있으며 운영 체제의 iCloud 디먼은 클라우드에서 데이터를 보내고 받는 작업을 담당 합니다. 동시 액세스를 방지 하려면 UbiquityContainer에 대 한 모든 파일 액세스를 FilePresenter/FileCoordinator를 통해 수행 해야 합니다. `UIDocument`클래스는 사용자를 위해이를 구현 합니다 .이 예제에서는 UIDocument를 사용 하는 방법을 보여 줍니다.

 [![문서 저장소 개요](introduction-to-icloud-images/icloud-overview.png)](introduction-to-icloud-images/icloud-overview.png#lightbox)

ICloudUIDoc 예제는 `UIDocument` 단일 텍스트 필드를 포함 하는 간단한 서브 클래스를 구현 합니다. 에서 렌더링 되는 텍스트는 `UITextView` iCloud에서 빨간색으로 표시 된 알림 메시지를 사용 하 여 다른 장치로 전파 됩니다. 예제 코드는 충돌 확인과 같은 고급 iCloud 기능을 처리 하지 않습니다.

이 스크린샷은 샘플 응용 프로그램을 보여 줍니다. 텍스트를 변경 하 고 **UpdateChangeCount** 를 누르면 문서가 iCloud를 통해 다른 장치로 동기화 됩니다.

 [![이 스크린샷에서는 텍스트를 변경 하 고 UpdateChangeCount를 누르는 후의 샘플 응용 프로그램을 보여 줍니다.](introduction-to-icloud-images/iclouduidoc.png)](introduction-to-icloud-images/iclouduidoc.png#lightbox)
...
1. **Icloud 문서 찾기 및 열기** - `NSFileManager` 및 `NSPredicate` 를 사용 하 여 icloud 문서를 찾아 엽니다.

1. **ICloud 문서 표시** - `UIDocument` UI 컨트롤과 상호 작용할 수 있도록에서 속성을 노출 합니다.
 ...
 **Threads** `AppDelegate.FinishedLaunching` 에 대 한 초기 호출에서 스레드는 `GetUrlForUbiquityContainer` 주 스레드가 차단 되지 않도록 다른 스레드에서 수행 됩니다.

 **Notificationcenter** -완료와 같은 비동기 작업을 수행할 때 알림에 등록 `NSMetadataQuery.StartQuery` 합니다.

 **완료 처리기** -과 같은 비동기 작업 완료 시 실행할 메서드를 전달 `UIDocument.Open` 합니다.
...
ICloud에 UIDocument를 추가 하려면를 직접 호출 `UIDocument.Save` 하거나 (새 문서에만 해당)를 사용 하 여 기존 파일을 이동할 수 있습니다 `NSFileManager.DefaultManager.SetUbiquitious` . 예제 코드는이 코드를 사용 하 여 어디 컨테이너에 직접 새 문서를 만듭니다. 여기에는 두 가지 완료 처리기 (작업에 대 한 `Save` 작업 및 다른 하나는 열기)가 있습니다.
...
문서에 대 한 후속 변경 내용은 직접 "저장" 되지 않으며, 대신을 사용 하 여 변경 되었음을 알리고 `UIDocument` `UpdateChangeCount` , 디스크에 저장 작업을 자동으로 예약 합니다.
...
사용자는 설정을 통해 응용 프로그램 외부에서 "어디 container"의 **documents** 디렉터리에 있는 iCloud 문서를 관리할 수 있습니다. 파일 목록을 보고 삭제로 살짝 밀기 할 수 있습니다. 응용 프로그램 코드는 사용자가 문서를 삭제 하는 상황을 처리할 수 있어야 합니다. **문서** 디렉터리에 내부 응용 프로그램 데이터를 저장 하지 마세요.
 ...
 [![ICloud 문서 관리 워크플로](introduction-to-icloud-images/icloudstorage.png)](introduction-to-icloud-images/icloudstorage.png#lightbox)

사용자는 장치에서 iCloud 지원 응용 프로그램을 제거 하려고 할 때 다른 경고를 수신 하 여 해당 응용 프로그램과 관련 된 iCloud 문서의 상태를 알립니다.

 [![사용자가 장치에서 iCloud 지원 응용 프로그램을 제거 하려고 하는 경우의 샘플 대화 상자](introduction-to-icloud-images/icloud-delete1.png)](introduction-to-icloud-images/icloud-delete1.png#lightbox)

 [![사용자가 장치에서 iCloud 지원 응용 프로그램을 제거 하려고 하는 경우의 샘플 대화 상자](introduction-to-icloud-images/icloud-delete2.png)](introduction-to-icloud-images/icloud-delete2.png#lightbox)
...
이 처럼 많은 양의 데이터를 저장 하는 응용 프로그램은 백업 되지 않은 사용자 디렉터리 중 하나에 저장 해야 합니다 (예: 캐시 또는 tmp)를 사용 하거나 `NSFileManager.SetSkipBackupAttribute` 를 사용 하 여 백업 작업 중에 iCloud가 무시 하도록 해당 파일에 플래그를 적용 합니다.
