...
[![이러한 방식이 함께 작동 하는 방식을 대략적으로 보여 주는 다이어그램](architecture-images/aot.png)](architecture-images/aot-large.png#lightbox)
...
먼저 선택기를 통해 수행 되는 c #에 대 한 목표 C를 노출 하는 방법이 있어야 합니다. 선택기는 개체 또는 클래스로 전송 되는 메시지입니다. 이 작업은 목표-C를 사용 하 여 [objc_msgSend](~/cross-platform/macios/binding/overview.md) 함수를 통해 수행 됩니다.
...
- 기존 목표-C 클래스를 래핑하는 모든 클래스의 경우에는 [] 특성을 포함 하는 모든 관리 되는 멤버를 대상으로 하는 모든 관리 되는 멤버를 미러링 하는 새 목표-c 클래스를 만듭니다 `Export` .
...
관리 코드에는를 사용 하 여 `[Register]` `[Export]` 개체를 목표에 노출 해야 한다는 것을 알 수 있도록 등록 기관에서 사용 하는 특성 및이 포함 될 수 있습니다.
`[Register]`특성은 생성 된 기본 이름이 적절 하지 않은 경우 생성 된 목표-C 클래스의 이름을 지정 하는 데 사용 됩니다. NSObject에서 파생 된 모든 클래스는 목표-C에 자동으로 등록 됩니다.
Required 특성에는 `[Export]` 생성 된 목표-C 클래스에서 사용 되는 선택기 인 문자열이 포함 됩니다.
...
- **정적 등록 기관** – 정적 등록자는 빌드 중에 목표 C 코드를 생성 합니다 .이 코드는 정적 라이브러리로 컴파일되고 실행 파일에 연결 됩니다. 이렇게 하면 더 빨리 시작할 수 있지만 빌드 시간 동안 시간이 오래 걸립니다. 이는 기본적으로 장치 빌드에 사용 됩니다. 정적 등록자는 `--registrar:static` `mtouch` 다음과 같이 프로젝트의 빌드 옵션에서 특성으로 전달 하 여 iOS 시뮬레이터에서 사용할 수도 있습니다.

    [![추가 mtouch 인수 설정](architecture-images/image1.png)](architecture-images/image1.png#lightbox)
...
모든 Xamarin.ios 실행 파일의 진입점은 mono를 초기화 하는 라는 함수에 의해 제공 됩니다 `xamarin_main` .

프로젝트 형식에 따라 다음이 수행 됩니다.

- 일반 iOS 및 tvOS 응용 프로그램의 경우 Xamarin 앱에서 제공 하는 관리 되는 Main 메서드를 호출 합니다. 이 관리 되는 Main 메서드는 `UIApplication.Main` 목표-C의 진입점인를 호출 합니다. UIApplication. Main은 목표-C의 메서드에 대 한 바인딩입니다 `UIApplicationMain` .
- 확장의 경우  `NSExtensionMain` Apple 라이브러리에서 제공 하는 네이티브 함수 ( `NSExtensionmain` WatchOS 확장의 경우)가 호출 됩니다. 이러한 프로젝트는 실행 프로젝트가 아닌 클래스 라이브러리 이기 때문에 실행할 관리 되는 기본 메서드가 없습니다.
...
Xamarin.ios에서 호출 되는 생성기는 [`btouch`](https://github.com/xamarin/xamarin-macios/blob/master/src/btouch.cs) 이러한 정의 파일을 사용 하 고 .net 도구를 사용 하 여 [임시 어셈블리로 컴파일합니다](https://github.com/xamarin/xamarin-macios/blob/master/src/btouch.cs#L318). 그러나이 임시 어셈블리는 목표-C 코드를 호출할 수 없습니다. 그러면 생성기에서 임시 어셈블리를 읽고 런타임에 사용할 수 있는 c # 코드를 생성 합니다.
예를 들어, 사용자가 정의 .cs 파일에 임의 특성을 추가 하면 출력 코드에 표시 되지 않습니다. 생성기는이에 대해 알지 않으므로 `btouch` 임시 어셈블리에서이를 검색 하 여 출력을 찾을 수 없습니다.

Xamarin.iOS.dll 만들어지면 mtouch는 모든 구성 요소를 함께 번들로 만듭니다.
...
- AOT 컴파일
- 네이티브 실행 파일에 연결 된 일련의 정적 라이브러리 (각 어셈블리에 대해 하나씩)를 출력 하는 네이티브 실행 파일을 만듭니다. 기본 실행 파일은 시작 관리자 코드, 등록자 코드 (정적) 및 AOT 컴파일러의 모든 출력으로 구성 됩니다.
