...
[모델 렌더링에 대 한 지침](~/graphics-games/monogame/3d/part1.md) 을 읽은 사용자는 MonoGame에서 3d 모델 렌더링에 익숙할 것입니다. `Model`클래스는 파일 (예: fbx)에 정의 된 데이터를 사용 하 고 정적 데이터를 처리할 때 3d 그래픽을 렌더링 하는 효과적인 방법입니다. 일부 게임에는 런타임에 동적으로 정의 하거나 조작 하는 3D 형상이 필요 합니다. 이러한 경우 *꼭 짓 점* 배열을 사용 하 여 기 하 도형을 정의 하 고 렌더링할 수 있습니다. 꼭 짓 점은 기 하 도형을 정의 하는 데 사용 되는 정렬 된 목록의 일부인 3D 공간에 대 한 일반적인 용어입니다. 일반적으로 꼭 짓 점은 일련의 삼각형을 정의 하는 방식으로 정렬 됩니다.

꼭 짓 점이 3D 개체를 만드는 데 사용 되는 방식을 시각화 하는 데 도움이 되도록 다음 구를 살펴보겠습니다.

![꼭 짓 점을 사용 하 여 3D 개체를 만드는 방법을 시각화 하려면이 구를 고려 합니다.](part2-images/image1.png)

위와 같이 구는 여러 삼각형으로 명확 하 게 구성 됩니다. 구의 와이어 프레임을 확인 하 여 꼭지점이 폼 삼각형에 어떻게 연결 되는지 확인할 수 있습니다.

![구의 와이어 프레임을 확인 하 여 꼭지점이 폼 삼각형에 연결 되는 방식을 확인 합니다.](part2-images/image2.png)
...
![완성 된 프로젝트에는 꼭 짓 점 배열을 사용 하 여 바둑판 층이 포함 됩니다.](part2-images/image3.png)
...
![화면에 그려진 6 개의 로봇 모델](part2-images/image4.png)

이 프로젝트의 끝에는 로봇을 사용 하 여 사용자 지정 꼭 짓 점 렌더링을 결합 하 게 `Model` 되므로 로봇 렌더링 코드를 삭제 하지 않습니다. 대신, 이제 호출을 지워 6 개 `Game1.Draw` 로봇의 그리기를 제거 합니다. 이렇게 하려면 **Game1.cs** 파일을 열고 메서드를 찾습니다 `Draw` . 다음 코드를 포함 하도록 수정 합니다.
...
![그러면 게임이 빈 파란색 화면으로 표시 됩니다.](part2-images/image5.png)
...
각 형식의 이름은 포함 하는 구성 요소를 나타냅니다. 예를 들어에는 `VertexPositionColor` 위치 및 색에 대 한 값이 포함 됩니다. 각 구성 요소를 살펴보겠습니다.

- Position – 모든 꼭 짓 점 형식에는 `Position` 구성 요소가 포함 됩니다. `Position`값은 꼭지점이 3d 공간 (X, Y, Z)에서 배치 되는 위치를 정의 합니다.
...
그런 다음에서 꼭 짓 점을 정의 `Game1.Initialize` 합니다. 이 문서의 앞부분에서 참조 하는 제공 된 템플릿에는 메서드가 포함 되어 있지 않으므로 `Game1.Initialize` 전체 메서드를에 추가 해야 합니다 `Game1` .
...
![꼭 짓 점이 표시 되는 모양을 시각화 하려면 다음 다이어그램을 참조 하세요.](part2-images/image6.png)
...
먼저 `BasicEffect` 위치 및 조명과 같은 렌더링을 위한 매개 변수를 보관할 인스턴스를 정의 해야 합니다. 이렇게 하려면 `BasicEffect` `Game1` 필드가 정의 된 아래 클래스에 멤버를 추가 합니다 `floorVerts` .
...
그런 다음, 메서드를 수정 `Initialize` 하 여 효과를 정의 합니다.
...
다음에서를 호출 해야 합니다 `DrawGround` `Game1.Draw` .
...
![앱이 실행 될 때이를 표시 합니다.](part2-images/image7.png)

위의 코드에 있는 세부 정보 중 일부를 살펴보겠습니다.

### <a name="view-and-projection-properties"></a>보기 및 프로젝션 속성

`View`및 `Projection` 속성은 장면을 보는 방법을 제어 합니다. 모델 렌더링 코드를 다시 추가할 때 나중에이 코드를 수정할 예정입니다. 특히는 `View` 카메라의 위치와 방향을 제어 하 고 `Projection` *보기의 필드* 를 제어 합니다 .이 필드는 카메라를 확대/축소 하는 데 사용할 수 있습니다.

### <a name="techniques-and-passes"></a>기술 및 통과

효과에 대 한 속성을 할당 하면 실제 렌더링을 수행할 수 있습니다.

이 연습에서는 속성을 변경 하지 `CurrentTechnique` 않지만 고급 게임에는 다른 방식으로 그리기를 수행할 수 있는 단일 효과 (예: 색 값이 적용 되는 방법)가 있을 수 있습니다. 이러한 각 렌더링 모드는 렌더링 전에 할당 될 수 있는 기술로 표시 될 수 있습니다. 또한 각 기법을 제대로 렌더링 하려면 여러 단계가 필요할 수 있습니다. 광선 표면 또는 이유 같이 복잡 한 시각적 개체를 렌더링 하는 경우에는 여러 패스가 필요할 수 있습니다.

주의 해야 할 중요 한 점은 `foreach` 루프에서 기본의 복잡성에 관계 없이 동일한 c # 코드를 사용 하 여 모든 효과를 렌더링 하도록 하는 것입니다 `BasicEffect` .
...
그런 다음 메서드를 수정 `DrawGround` 합니다. 를에 할당 하 `effect.TextureEnabled` 고를로 설정 하는 것만 필요 합니다 `true` `effect.Texture` `checkerboardTexture` .
...
마지막으로, `Game1.Initialize` 꼭지점에 질감 좌표를 할당 하도록 메서드를 수정 해야 합니다.
...
![이제 평면에서 바둑판 패턴을 표시 합니다.](part2-images/image8.png)
...
![이 다이어그램은 정규화 된 좌표를 시각화 하는 데 도움이 됩니다.](part2-images/image9.png)
...
![이로 인해 질감이 20 번 반복 됩니다.](part2-images/image10.png)

## <a name="rendering-vertices-with-models"></a>모델을 사용 하 여 꼭 짓 점 렌더링

이제 평면이 제대로 렌더링 되었으므로 모델을 다시 추가 하 여 모든 항목을 함께 볼 수 있습니다. 먼저 모델 코드를 `Game1.Draw` 수정 된 위치를 사용 하 여 메서드에 다시 추가 합니다.
...
또한 `Vector3` 에서 `Game1` 카메라의 위치를 나타내는를 만듭니다. 선언 아래에 필드를 추가 합니다 `checkerboardTexture` .
...
다음으로 메서드에서 지역 변수를 제거 합니다 `cameraPosition` `DrawModel` .
...
마찬가지로 메서드에서 지역 변수를 제거 합니다 `cameraPosition` `DrawGround` .
...
![모델과 그라운드 모두 동시에 표시 됩니다.](part2-images/image11.png)
...
![이 코드는이 뷰를 생성 합니다.](part2-images/image3.png)

## <a name="summary"></a>요약

이 연습에서는 꼭 짓 점 배열을 사용 하 여 사용자 지정 렌더링을 수행 하는 방법을 살펴보았습니다. 이 경우 꼭 짓 점 기반 렌더링과 질감 및를 결합 하 여 바둑판 층을 만들었으며 `BasicEffect` 여기에 제공 된 코드는 3d 렌더링의 기반으로 사용 됩니다. 또한 동일한 장면의 모델과 함께 버텍스 기반 렌더링을 혼합할 수 있음을 보여 주었습니다.
