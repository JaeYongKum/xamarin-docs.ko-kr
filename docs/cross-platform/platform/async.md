---
title: 비동기 지원 개요
description: '이 문서에서는 비동기 코드를 더 쉽게 작성할 수 있도록 c # 5에 도입 된 async 및 wait 개념을 사용한 프로그래밍에 대해 설명 합니다.'
ms.prod: xamarin
...
_C # 5에는 비동기 프로그래밍을 간소화 하는 두 개의 키워드 (비동기 및 대기)가 도입 되었습니다. 이러한 키워드를 사용 하 여 작업 병렬 라이브러리를 활용 하는 간단한 코드를 작성 하 여 다른 스레드에서 장기 실행 작업 (예: 네트워크 액세스)을 실행 하 고 완료 시 결과에 쉽게 액세스할 수 있습니다. 최신 버전의 Xamarin.ios 및 Xamarin Android는 async 및 wait를 지원 합니다 .이 문서에서는 Xamarin과 함께 새 구문을 사용 하는 방법에 대 한 설명과 예제를 제공 합니다._
...
이 샘플 응용 프로그램은 기본 스레드를 차단 하지 않고 간단한 비동기 웹 요청을 만든 다음 다운로드 된 html 및 문자 개수로 UI를 업데이트 합니다.
 ...
 [![이 샘플 응용 프로그램은 주 스레드를 차단 하지 않고 간단한 비동기 웹 요청을 만든 다음 다운로드 한 html 및 문자 개수로 UI를 업데이트 합니다.](async-images/AsyncAwait_427x368.png)](async-images/AsyncAwait.png#lightbox)
...
`async`키워드는 메서드 선언 (또는 람다 또는 무명 메서드에서)에 배치 되어 비동기적으로 실행 될 수 있는 코드를 포함 함을 표시 합니다. 즉, 호출자의 스레드를 차단 하지 않습니다.

로 표시 된 메서드는 `async` 하나 이상의 wait 식 또는 문을 포함 해야 합니다. `await`메서드에 문이 없으면 동기적으로 실행 됩니다 (한정자가 없는 경우와 동일 `async` ). 이로 인해 컴파일러 경고가 발생 하 고 오류는 발생 하지 않습니다.

### <a name="return-types"></a>반환 형식

비동기 메서드는 또는를 반환 해야 합니다 `Task` `Task<TResult>` `void` .

`Task`메서드에서 다른 값을 반환 하지 않는 경우 반환 형식을 지정 합니다.

`Task<TResult>`메서드에서 값을 반환 해야 하는지 여부를 지정 합니다 `TResult` . 여기서은 반환 되는 형식 (예: `int` )입니다.

`void`반환 형식은 주로 필요로 하는 이벤트 처리기에 사용 됩니다. Void를 반환 하는 비동기 메서드를 호출 하는 코드는 결과에 사용할 수 없습니다 `await` .
...
비동기 메서드 내에서 발생 하는 예외는 태스크에 저장 되 고 작업이 진행 되 면 throw 됩니다 `await` . 이러한 예외는 catch 블록 내에서 catch 하 고 처리할 수 있습니다.

## <a name="cancellation"></a>취소

완료 하는 데 시간이 오래 걸리는 비동기 메서드는 취소를 지원 해야 합니다. 일반적으로 취소는 다음과 같이 호출 됩니다.

- `CancellationTokenSource`개체를 만듭니다.
- `CancellationTokenSource.Token`인스턴스가 취소 가능한 비동기 메서드에 전달 됩니다.
- 메서드를 호출 하 여 취소를 요청 `CancellationTokenSource.Cancel` 합니다.
...
다음 메서드는 ed 작업을 사용 하 여 메서드를 코딩 하는 방법을 보여 줍니다 `async` `await` .
...
IOS 샘플 응용 프로그램에서는이 예제가 약간 다르게 기록 되어 다른 방법을 보여 줍니다. 익명 대리자를 사용 하는 대신이 예제에서는 `async` 일반 이벤트 처리기와 같이 할당 된 이벤트 처리기를 선언 합니다.
