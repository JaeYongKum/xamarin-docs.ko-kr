...
두 가지 모두 자식 컨트롤을 숨기 거 나 표시 하는 기능을 지원 합니다. WPF에서는 세 가지 상태 속성 `IsVisible` (visible, hidden 및 축소)입니다. Xamarin.ios에서는 속성을 통해 표시 하거나 숨길 수 `IsVisible` 있습니다.
...
를 정의 하지 않으면 `ResourceDictionary` 런타임 오류가 발생 합니다.
