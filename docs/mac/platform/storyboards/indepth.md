...
스토리 보드는 `.storyboard` 컴파일되어 제공 될 때 xamarin.ios 앱의 번들에 포함 되는 리소스 파일 (확장명 포함)입니다. 앱에 대 한 시작 Storyboard를 정의 하려면 해당 파일을 편집 하 `Info.plist` 고 드롭다운 상자에서 **주 인터페이스** 를 선택 합니다. 
...
필요에 따라 메서드를 사용 하 여 `InstantiateInitialController` Interface Builder에서 초기 컨트롤러가 할당 된 뷰 컨트롤러를 로드할 수 있습니다.
...
또한 `NSViewControllers` 이제는 창의 _응답자 체인_의 일부입니다.
...
트리거된 Segue를 사용 하 여 명명 된 Segue (Interface Builder의 **식별자** 속성을 통해)를 지정 하 고 사용자가 단추를 클릭 하거나 `PerformSegue` 코드에서 메서드를 호출 하는 등의 이벤트에 의해 트리거될 수 있습니다.
...
Segue의 소스로 작동 하는 뷰 컨트롤러에서 메서드를 재정의 하 고, `PrepareForSegue` Segue를 실행 하 고 지정 된 뷰 컨트롤러를 표시 하기 전에 초기화를 수행 해야 합니다.
...
필요에 따라 메서드를 재정의 `ShouldPerformSegue` 하 고 Segue가 c # 코드를 통해 실제로 실행 되는지 여부를 제어할 수 있습니다. 수동으로 표시 된 뷰 컨트롤러의 경우 메서드를 호출 `DismissController` 하 여 더 이상 필요 하지 않은 경우 표시에서 제거 합니다.
...
- 특성을 사용 하 여 `Register` 이 클래스를 목표-C/macOS에 노출 합니다.
- 메서드를 재정의 `Perform` 하 여 사용자 지정 Segue의 작업을 실제로 수행 합니다.
- 창의 `ContentViewController` 컨트롤러를 Segue 대상 (대상)에 의해 정의 된 것으로 바꿉니다.
- 메서드를 사용 하 여 메모리를 확보 하기 위해 원래 뷰 컨트롤러를 제거 합니다 `RemoveFromParentViewController` .
...
2. 속성에는 `Storyboard` 창 컨트롤러를 로드 한 storyboard가 포함 되 고, 그렇지 `null` 않으면 스토리 보드에서 로드 되지 않습니다.
3. 메서드를 호출 `DismissController` 하 여 지정 된 창을 닫고 뷰에서 제거할 수 있습니다.

뷰 컨트롤러와 마찬가지로 창 컨트롤러는 `PerformSegue` 을 구현 `PrepareForSegue` 하 고 `ShouldPerformSegue` 및 메서드를 Segue 작업의 소스로 사용할 수 있습니다.
...
뷰 컨트롤러에서 이벤트를 재정의 하는 대신 `MouseDown` , storyboard를 사용할 때 사용자 입력 이벤트를 처리 하기 위해 제스처 인식기를 사용 해야 합니다.
...
1. **솔루션 탐색기**에서 프로젝트 이름을 마우스 오른쪽 단추로 클릭 하 고 **Add**  >  **새 파일**  >  추가 ...를 선택 합니다. **Mac**  >  **스토리 보드**. 새 스토리 보드의 **이름을** 입력 하 고 **새로 만들기** 단추를 클릭 합니다. 
