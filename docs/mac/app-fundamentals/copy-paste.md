...
Xamarin.ios 응용 프로그램에서 c # 및 .NET으로 작업 하는 경우 목표-C에서 작업 하는 개발자와 동일한 대/소문자 구분 (복사 및 붙여넣기) 지원에 액세스할 수 있습니다.
...
3. `MacCopyPaste` **프로젝트 이름** 으로를 입력 하 고 다른 모든 항목을 기본값으로 유지 합니다. 다음을 클릭합니다. 
...
다음 `NSDocument` 으로 응용 프로그램의 사용자 인터페이스에 대 한 백그라운드 저장소 역할을 하는 사용자 지정 클래스를 추가 합니다. 여기에는 단일 이미지 뷰가 포함 되어 있으며, 뷰에서 이미지를 기본 대/표시로 복사 하는 방법과 기본 대지의 이미지를 가져와서 이미지 뷰에 표시 하는 방법을 알고 있습니다.

**Solution Pad** 에서 xamarin.ios 프로젝트를 마우스 오른쪽 단추로 클릭 하 고 **Add**  >  **새 파일 추가 ...** 를 선택 합니다.
...
다음 코드에서는 이미지를 사용할 수 있는 경우 기본 대지의 이미지 데이터가 있는지 테스트 하는 속성을 제공 합니다 `true` `false` .
...
먼저 위에서 만든 클래스의 인스턴스를 노출 합니다 `ImageDocument` .
...
, 및를 사용 하 여 `Export` `WillChangeValue` `DidChangeValue` `Document` Xcode에서 키-값 코딩 및 데이터 바인딩을 허용 하도록 속성을 설정 했습니다.
...
주 창이 로드 되 고 표시 되 면 클래스의 인스턴스를 만들고 `ImageDocument` 다음 코드를 사용 하 여 UI의 이미지를 연결 합니다.
...
마지막으로, 복사 하 여 붙여넣기 도구 모음 항목을 클릭 하는 사용자에 대 한 응답으로 클래스의 인스턴스를 호출 `ImageDocument` 하 여 실제 작업을 수행 합니다.
...
각 메뉴 항목에 대해 현재, 최상위, 키 창을 가져와 클래스로 캐스팅 합니다 `ImageWindow` .
...
이번에는 현재 최상위 창을 가져오고 해당 클래스 인스턴스를 사용 `ImageDocument` 하 여 필요한 이미지 데이터가 있는지 확인 합니다. 그런 다음 메서드를 사용 `MenuWillHighlightItem` 하 여이 상태를 기반으로 각 항목을 활성화 하거나 비활성화 합니다.
...
먼저 편집 메뉴에서 메뉴 항목의 자동 사용 및 사용 안 함을 사용 하지 않도록 설정 합니다. 다음으로 위에서 만든 클래스의 인스턴스를 연결 `EditMenuDelegate` 합니다.
...
![애플리케이션 실행](copy-paste-images/run01.png "애플리케이션 실행")
...
`NSPasteboard`클래스는 응용 프로그램 간에 정보를 교환 하거나 지정 된 앱 내에서 정보를 교환 하기 위한 표준화 된 메커니즘을 제공 합니다. 대지의 주요 기능은 복사 및 붙여넣기 작업을 처리 하기 위한 것입니다.
...
필요에 따라 `CreateWithUniqueName` 클래스의 메서드를 호출 `NSPasteboard` 하 여 고유 하 게 명명 된 대지의를 만들 수 있습니다.
...
항목을 표시 하는 경우에는 인터페이스를 준수 하는 개체에서 사용할 수 있는 하나 이상의 다른 표현을 제공 하도록 지정 합니다 `NSPasteboardItemDataProvider` . 이러한 개체는 수신 앱에서 요청 하는 요청에 대 한 추가 표현을 제공 합니다.
...
`ChangeCount` `ClearContents` 클래스의 및 메서드를 사용 `NSPasteboard` 하 여 지정 된 대지의 변경 횟수를 수정 합니다.
...
일반적으로 위의 예제에서 수행한 것 처럼 일반 대 면에만 씁니다. 메서드에 보내는 모든 개체는 `WriteObjects` 인터페이스를 따라야 *합니다* `INSPasteboardWriting` . ,,,, 및와 같은 몇 가지 기본 제공 클래스는 `NSString` `NSImage` `NSURL` `NSColor` `NSAttributedString` `NSPasteboardItem` 이 인터페이스를 자동으로 준수 합니다.

사용자 지정 데이터 클래스를 대지의에 작성 하는 경우에는 인터페이스를 준수 `INSPasteboardWriting` 하거나 클래스의 인스턴스에 래핑해야 합니다 `NSPasteboardItem` (아래의 [사용자 지정 데이터 형식](#Custom_Data_Types) 섹션 참조).
...
메서드를 사용 하 여 대지의 데이터를 읽습니다 `ReadObjectsForClasses` . 두 매개 변수가 필요 합니다.

1. `NSObject`대지의 읽기를 원하는 기반 클래스 형식의 배열입니다. 가장 적합 한 데이터 형식으로 순서를 지정 해야 합니다.
...
예를 들어 다음 코드는가 `NSImage` 일반 대지의에 있는지 확인 하 고,이 경우 이미지 웰에 표시 합니다.
...
1. 메서드를 한 번 호출 하 `ReadObjectsForClasses` 고 원하는 모든 표현의 배열을 기본 순서로 제공 합니다.
2. 매번 다른 형식의 배열을 요청 하는 메서드를 여러 번 호출 `ReadObjectsForClasses` 합니다.
...
`CanReadObjectForClasses`대지의 메서드를 호출 하 여 지정 된 형식이 포함 되어 있는지 확인 합니다.

예를 들어 다음 코드는 일반 대지의 인스턴스가 포함 되어 있는지 여부를 확인 합니다 `NSImage` .
...
지정 된 Xamarin.ios 앱의 기능을 기반으로 하는 경우에는 지정 된 조건 집합을 충족 하는 경우에만 (예: 특정 데이터 형식의 파일이 나 Url을 가리키는 경우)에만 대지의 Url을 읽을 수 있습니다. 이 경우 또는 메서드의 두 번째 매개 변수를 사용 하 여 추가 검색 조건을 지정할 수 있습니다 `CanReadObjectForClasses` `ReadObjectsForClasses` .
...
이 경우에서 상속 하 `NSObject` 고 몇 가지 인터페이스 (및)를 준수 하도록 데이터 사용자 지정 클래스를 디자인 해야 `INSCoding` `INSPasteboardWriting` `INSPasteboardReading` 합니다. 필요에 따라를 사용 `NSPasteboardItem` 하 여 복사 하거나 붙여넣을 데이터를 캡슐화 할 수 있습니다.
...
사용자 지정 데이터 클래스를 작성 하거나 대//문자에서 읽으려면 먼저 및 인터페이스를 준수 해야 합니다 `INSPastebaordWriting` `INSPasteboardReading` . 또한이 클래스는에서 상속 하 `NSObject` 고 인터페이스를 준수 해야 합니다 `INSCoding` .
...
자세한 내용은 [Xamarin.ios 내부](~/mac/internals/how-it-works.md) 설명서의 [c # 클래스/메서드를 목표로 제공-c](~/mac/internals/how-it-works.md) 섹션을 참조 하세요. c `Register` `Export` # 클래스를 객관적인 개체 및 UI 요소에 연결 하는 데 사용 되는 및 특성에 대해 설명 합니다.
...
먼저의 기본 목표-C 메서드에서 _빈_ 생성자를 노출 합니다 `init` .

다음으로, `NSCoding` 의 내보낸 이름으로 붙여넣을 때 대지의 새 개체 인스턴스를 만드는 데 사용 되는 호환 생성자를 노출 합니다 `initWithCoder` .

이 생성자는를 사용 하 여에 `NSCoder` 게 작성 될 때에 의해 생성 되는을 사용 `NSKeyedArchiver` 하 고 키/값 쌍 데이터를 추출 하 여 데이터 클래스의 속성 필드에 저장 합니다.

#### <a name="writing-to-the-pasteboard"></a>대지의에 쓰기

인터페이스를 준수 하 여 `INSPasteboardWriting` 두 개의 메서드를 노출 하 고 필요에 따라 세 번째 메서드를 표시 하 여 클래스를 대지의에 쓸 수 있도록 해야 합니다.
...
사용자 지정 형식에 대해 사용자 고유의 UTI을 만듭니다. "" (는 앱 식별자와 마찬가지로 역방향 표기법). 또한 클래스는 표준 문자열을 대지의 ()에 쓸 수 있습니다 `public.text` . 
...
형식의 경우 `public.text` 간단한 형식의 개체를 반환 `NSString` 합니다. 사용자 지정 `com.xamarin.image-info` 형식의 경우 및 인터페이스를 사용 하 여 `NSKeyedArchiver` `NSCoder` 사용자 지정 데이터 클래스를 키/값 쌍 보관으로 인코딩합니다. 인코딩을 실제로 처리 하려면 다음 메서드를 구현 해야 합니다.
...
현재 옵션은 `WritingPromised` 사용할 수 있으며, 지정 된 형식이 약속 한 경우에만 사용할 수 있으며 실제로는 대/면에 기록 되지 않습니다. 자세한 내용은 위의 [약속 데이터](#Promised_Data) 섹션을 참조 하세요.
...
인터페이스를 준수 하 여 `INSPasteboardReading` 사용자 지정 데이터 클래스를 대지의에서 읽을 수 있도록 세 가지 메서드를 노출 해야 합니다.
...
형식에 대해 `com.xamarin.image-info` `NSKeyedArchiver` `initWithCoder:` 클래스에 추가 된 생성자를 호출 하 여에 대 한 클래스를 작성할 때를 사용 하 여 만든 키/값 쌍을 디코드 하도록 대지의 지시를 합니다.
...
사용자 지정 클래스 만들기를 보증 하지 않는 사용자 지정 항목을에 써야 하는 경우 또는 필요한 경우에만 공통 형식으로 데이터를 제공 하려는 경우가 있습니다. 이러한 경우에는를 사용할 수 있습니다 `NSPasteboardItem` .

는 대/소문자에 작성 된 데이터에 대 한 세분화 된 `NSPasteboardItem` 제어를 제공 하며 임시 액세스용으로 설계 된 데이터에 대 한 세분화 된 제어를 제공 합니다 .이는 대/소문자에 쓰여진 후에 삭제 해야 합니다.

#### <a name="writing-data"></a>데이터 쓰기

사용자 지정 데이터를에 쓰려면 `NSPasteboardItem` 사용자 지정를 제공 해야 `NSPasteboardItemDataProvider` 합니다. 프로젝트에 새 클래스를 추가 하 고 **ImageInfoDataProvider.cs**를 호출 합니다. 파일을 편집 하 여 다음과 같이 만듭니다.
...
사용자 지정 데이터 클래스를 사용 하는 것 처럼 및 지시문을 사용 하 여 `Register` `Export` 목표에 노출 해야 합니다. C. 클래스는에서 상속 해야 하 `NSPasteboardItemDataProvider` 고 및 메서드를 구현 해야 합니다 `FinishedWithDataProvider` `ProvideDataForType` .

메서드를 사용 하 여 다음과 `ProvideDataForType` 같이에 래핑되는 데이터를 제공 합니다 `NSPasteboardItem` .
...
이 경우 이미지에 대 한 두 가지 정보 (이름 및 ImageType)를 저장 하 고 단순 문자열 ()에 기록 `public.text` 합니다.
